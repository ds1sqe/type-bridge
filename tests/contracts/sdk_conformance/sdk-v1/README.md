...
nine runtime proof rows. It is not the complete sdk journey and it does
...
wire accepted by `scripts/ci/compare_sdk_conformance.py`.
...
When `TYPE_BRIDGE_SDK_REPORT` is present, producers require an absolute
...
```bash
python scripts/ci/compare_sdk_conformance.py \
  tmp/sdk/python.json \
  tmp/sdk/node.json \
  tmp/sdk/rust.json
```
