...
`sdk-v1` checkpoint. It compares real Python, Node, Rust, and C live
...
nine-row baseline regression. V2 adds C and the C13-C23, C28, C31, G02, G05,
...
- `proof-fragment-allowlist-v1.json`: the exact producer IDs, source-path sets,
  ...
  binding; and
- `scripts/ci/compare_sdk_conformance_v2.py`: the fail-closed comparator.

Each live runner owns its output path through
`TYPE_BRIDGE_SDK_REPORT_V2`. The parent directory must already exist, the
...
to use `TYPE_BRIDGE_SDK_REPORT` independently.
...
`TYPE_BRIDGE_SDK_V2_PROOF_RUN_NONCE`. Each emitter receives one
create-new destination through `TYPE_BRIDGE_SDK_V2_PROOF_FRAGMENT`; a
report runner receives the resulting one-or-more paths through
`TYPE_BRIDGE_SDK_V2_PROOF_FRAGMENTS`, encoded with the platform path-list
