...
`workforce_v3_fingerprints` Rust test from the exact live-producer projection
...
candidate report validation; it does not promote any manifest capability.
...
- `catalog-v3.json`: the exact 21-row ledger, four live report-producer
  identities, the exact 17-case candidate transition, and the finalized
...
The Phase-7 implementation uses two closed intermediate wires before that
publication. A binding supplement uses
`typebridge.workforce-v3-live-supplement/v1` for exactly eight lanes not already
owned by the accepted Phase-2/Phase-5 reports. The live compositor validates
...
`typebridge.workforce-v3-live-observations/v1` with exactly 14 lanes. The final
