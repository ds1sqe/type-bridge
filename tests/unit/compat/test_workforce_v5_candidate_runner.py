"""Command and cleanup contracts for the four-binding Workforce V5 runner."""
...
SPEC = importlib.util.spec_from_file_location(
    "workforce_v5_candidate_runner", CI / "run_workforce_v5_candidate.py"
)
...
def test_assembly_runs_each_real_composer_and_one_fan_in(
    ...
    assert (
        sum(
            any(item.endswith("compose_workforce_v5_evidence.py") for item in command)
    ...
    assert (
        sum(
            any(item.endswith("assemble_workforce_conformance_v5.py") for item in command)
            for command in commands
        )
        == 4
    )
    assert any(item.endswith("compare_workforce_conformance_v5.py") for item in commands[-1])
...
def test_provider_free_fan_in_names_each_binding(
    ...
    fan_in = [
        command for command in commands if any("compare_workforce_v5_" in item for item in command)
    ]
    assert len(fan_in) == 2
...
def test_runner_names_all_four_live_producer_paths() -> None:
    source = (CI / "run_workforce_v5_candidate.py").read_text(encoding="utf-8")
    for marker in (
