"""Frozen source-contract tests for the four-binding workforce-v3 journey."""
...
EXPECTED_SELECTED_PROOFS = [
    ("workforce.crud.entity-batch-insert-put", "direct_runtime", "entity_batch_insert_put"),
    (
    ...
    ("workforce.crud.unkeyed-entity", "direct_runtime", "unkeyed_entity_iid_lifecycle"),
    (
    ...
    ("workforce.manager.filter", "direct_runtime", "manager_field_token_filter"),
    ("workforce.model.constraints", "diagnostic", "projected_constraint_validation"),
    (
    ...
    ("workforce.projection.field-name-identity", "artifact", "field_name_identity"),
    (
        "workforce.projection.token-package-fencing",
        "diagnostic",
        "token_package_fencing",
    ),
    ("workforce.runtime.cancellation", "direct_runtime", "data_operation_cancellation"),
    (
        "workforce.runtime.connection-policy",
        "direct_runtime",
        "complete_connection_policy",
    ),
    ("workforce.runtime.explicit-close", "lifecycle", "data_resource_lifecycle"),
    (
        "workforce.runtime.timeout-resource-limits",
        "direct_runtime",
        "data_operation_resource_limits",
    ),
    ("workforce.schema.generate.atomic", "artifact", "atomic_multibinding_generation"),
    (
...
EXPECTED_FRAGMENT_PRODUCERS: dict[str, dict[str, dict[str, Any]]] = {
    "python": {
        "python.generated-data-v3-proof": {
            ...
            "lanes": DATA_LANES,
            "test_id": "runtime_projection::tests::workforce_v3_python_data_plane_fragment",
        },
    ...
    "node": {
        "node.generated-data-v3-proof": {
            ...
            "lanes": DATA_LANES,
            "test_id": "runtime_projection::tests::workforce_v3_node_data_plane_fragment",
        },
    ...
    "rust": {
        "type-bridge-rust.generated-data-v3-proof": {
            ...
            "lanes": DATA_LANES,
            "test_id": "transaction::tests::workforce_v3_rust_data_plane_fragment",
        },
        "type-bridge-rust.generated-package-v3-proof": {
            ...
            "lanes": PACKAGE_LANES,
            "test_id": "rust_acceptance::workforce_v3_generated_package_integrity",
        },
    },
    "c": {
        "type-bridge-c.generated-data-v3-proof": {
            ...
            "lanes": DATA_LANES,
            "test_id": "runtime::tests::workforce_v3_c_data_plane_fragment",
        },
        "type-bridge-c.generated-package-v3-proof": {
            ...
            "lanes": PACKAGE_LANES,
            "test_id": "c_emitter::workforce_v3_generated_package_integrity",
        },
    },
}
...
def test_workforce_v3_report_schema_freezes_four_bindings_and_exact_21_rows() -> None:
    ...
    assert fixture["properties"]["id"] == {"const": "workforce-v3"}
    ...
    assert catalog["expected_fingerprints"] == {
        ...
        "projections": {
            ...
            for binding, digest in {
                ...
                "node": "8e7e267546da6dcee85ba335293b8eb9a82c2612d9260810f63a043b7ff4c7ed",
                "rust": "e4a801726b98d2248c245ff9f6fe9a332dc7982a04fcae355ef76ebcbe8ef54e",
                "c": "2ba2a74aa11e3df1923999bf451ea5431bef00d4f9c2654eab1bf2ee498a182c",
            }.items()
        },
    }
...
def test_workforce_v3_journey_freezes_21_complete_observation_shapes() -> None:
    ...
    assert journey["format"] == "typebridge.workforce-journey/v3"
    assert journey["fixture_id"] == "workforce-v3"
...
def test_workforce_v3_fragment_contract_freezes_two_producers_and_seven_lanes() -> None:
    schema = _load("proof-fragment-schema-v1.json")
    assert schema["additionalProperties"] is False
    assert schema["properties"]["format"] == {"const": "typebridge.workforce-v3-proof-fragment/v1"}
    ...
    assert allowlist["format"] == "typebridge.workforce-v3-proof-fragment-allowlist/v1"
...
def test_workforce_v3_environment_and_nonce_names_are_independent_from_v2() -> None:
    readme = (CONTRACT_ROOT / "README.md").read_text(encoding="utf-8")

    assert "TYPE_BRIDGE_WORKFORCE_REPORT_V3" in readme
    assert "TYPE_BRIDGE_WORKFORCE_V3_PROOF_RUN_NONCE" in readme
    assert "TYPE_BRIDGE_WORKFORCE_V3_PROOF_FRAGMENT" in readme
    assert "TYPE_BRIDGE_WORKFORCE_V3_PROOF_FRAGMENTS" in readme
    assert "TYPE_BRIDGE_WORKFORCE_REPORT_V2" not in readme
    assert "TYPE_BRIDGE_WORKFORCE_V2_PROOF_RUN_NONCE" not in readme
