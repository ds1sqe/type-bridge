"""Command-ledger contract for the four-binding Workforce V3 runner."""
...
SPEC = importlib.util.spec_from_file_location(
    "workforce_v3_candidate_runner", CI / "run_workforce_v3_candidate.py"
)
...
def test_phase_report_ledger_uses_all_three_validated_fan_ins(
    ...
    for marker in (
        "run_phase2_projection_parity.py",
        "run_phase2_projection_live.py",
        "run_phase5_manager_filter_live.py",
    ):
...
def test_assembly_runs_four_compositors_four_assemblers_and_one_fan_in(
    ...
    assert sum(any("compose_workforce_v3" in item for item in command) for command in commands) == 4
    assert (
        sum(
            any("assemble_workforce_conformance_v3" in item for item in command)
            for command in commands
        )
        == 4
    )
    assert any("compare_workforce_conformance_v3" in item for item in commands[-1])


def test_runner_names_all_live_and_proof_producers() -> None:
    source = (CI / "run_workforce_v3_candidate.py").read_text(encoding="utf-8")
    for marker in (
        ...
        "workforce_v3_python_data_plane_fragment",
        "workforce_v3_node_data_plane_fragment",
        "workforce_v3_rust_data_plane_fragment",
        "workforce_v3_c_data_plane_fragment",
    ):
