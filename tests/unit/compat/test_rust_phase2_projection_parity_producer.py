...
NEGATIVE = PRODUCER.with_name("phase2_foreign_negative.rs")
HARNESS = PRODUCER.parents[1] / "rust_acceptance.rs"


def test_rust_phase2_producer_uses_real_generated_projection_paths() -> None:
...
def test_rust_phase2_foreign_fence_is_derived_from_runtime_diagnostics() -> None:
    source = PRODUCER.read_text()
...
def test_rust_phase2_producer_does_not_import_expected_parity_evidence() -> None:
    source = PRODUCER.read_text()
    for forbidden in (
        "expected_report",
...
def test_rust_phase2_harness_proves_nominal_foreign_fence_and_canonical_report() -> None:
    ...
    assert "generated_rust_phase2_projection_parity_producer" in harness
