"""Source and lifecycle contract for the C Projected exact-live producer."""
...
HARNESS = ROOT / "type-bridge-core/crates/schema-codegen/tests/c_projected_live.rs"
CONSUMER = ROOT / "type-bridge-core/crates/schema-codegen/tests/c_projected_live/consumer.c"
SETUP = ROOT / "tests/support/provider/main.rs"
HELPER = ROOT / "type-bridge-core/crates/schema-codegen/tests/support/c_provider.rs"
...
def _sources() -> tuple[str, str, str]:
    return (
        HARNESS.read_text(encoding="utf-8") + HELPER.read_text(encoding="utf-8"),
        CONSUMER.read_text(encoding="utf-8"),
        SETUP.read_text(encoding="utf-8"),
    )


def test_c_projected_live_producer_is_independent_and_exactly_authority_bound() -> None:
    ...
    assert 'const REPORT_FORMAT: &str = "typebridge.projected-live-report/v1"' in harness
    ...
    assert "projected_foreign_schema_package_open" in consumer


def test_c_projected_live_consumer_owns_data_crud_and_omits_ordered_persistence() -> None:
    harness, consumer, setup = _sources()
...
def test_c_projected_live_observation_ledger_is_exact_and_runtime_checked() -> None:
    _, consumer, _ = _sources()
    emitted = re.findall(r'puts\("TYPE_BRIDGE_C_PROJECTED_LIVE_FACT\\t([^\\]+)\\t', consumer)
    assert emitted == [
    ...
    for model in ("person", "robot", "interaction", "plainzhactivity", "event", "container"):
        assert f"projected_{model}_database_count(" in consumer
    assert "interaction_actor_is(interaction, expected_kinds[index]" in consumer
    assert "projected_plainzhactivity_database_get_by_iid(" in consumer
    assert "plain != NULL && diagnostics == NULL" in consumer
    assert "plain == NULL" in consumer
    assert "projected_container_item_player_as_event(" in consumer
    assert "memcmp(hydrated_iid.data, iids[8].bytes" in consumer


def test_c_projected_live_lifecycle_is_fail_closed_and_publishes_last() -> None:
    harness, consumer, setup = _sources()
    ...
    assert consumer.rfind("type_bridge_schema_package_close") < consumer.rfind(
        "TYPE_BRIDGE_C_PROJECTED_LIVE_FACT"
    )


def test_c_projected_live_requires_exact_ports_database_and_provider_version() -> None:
    harness, consumer, setup = _sources()
    for name in (
        "TYPE_BRIDGE_PROJECTED_LIVE_REPORT",
        "TYPE_BRIDGE_PROJECTED_LIVE_ADDRESS",
        "TYPE_BRIDGE_PROJECTED_LIVE_HTTP_PORT",
