"""Fail-closed tests for artifact-bound Sdk V6 fan-in."""
...
SCRIPT = ROOT / "scripts/ci/compare_sdk_conformance_v6.py"
SPEC = importlib.util.spec_from_file_location("sdk_v6_comparator", SCRIPT)
...
def _report(root: Path, binding: str) -> dict[str, object]:
    ...
    return {
        ...
        "acceptance_report_sha256": "8" * 64,
        "artifacts": artifacts,
        "generated_surface": {
            "format": COMPARATOR.SURFACE_FORMATS[binding],
            "artifact-id": (
                artifacts["generated-package"]["artifact-id"]
                if binding == "c"
...
def test_accepts_exact_five_pending_artifact_promotions(tmp_path: Path) -> None:
    root = _stage(tmp_path)
    _set_pending(root)
    comparison = COMPARATOR.compare_reports(_reports(root), root)
    assert comparison["authority_state"] == "artifact"
    ...
    for capability in manifest["capabilities"]:
        if capability["case_ids"][0] in COMPARATOR.CASES:
            capability["binding_profile"] = (
                ...
                if capability["case_ids"][0] == "sdk.distribution.standalone-cli"
