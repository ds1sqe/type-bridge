"""Command-ledger and publication tests for the Workforce V1/V2 runner."""
...
SPEC = importlib.util.spec_from_file_location(
    "workforce_v1_v2_candidate_runner", CI / "run_workforce_v1_v2_candidate.py"
)
...
def test_fragment_ledger_covers_each_binding_and_exact_validator(
    ...
    assert (
        sum(
            any("workforce_v2_proof_fragments.py" in item for item in command)
    ...
    source = (CI / "run_workforce_v1_v2_candidate.py").read_text(encoding="utf-8")
    for marker in (
        ...
        "workforce_v2_rust_deterministic_proof_fragment",
        "workforce_v2_c_deterministic_proof_fragment",
    ):
...
def test_publication_is_one_create_new_nested_candidate(tmp_path: Path) -> None:
