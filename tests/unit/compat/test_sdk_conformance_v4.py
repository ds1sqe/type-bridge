"""Fail-closed tests for the Sdk V4 comparator."""
...
SCRIPT = ROOT / "scripts/ci/compare_sdk_conformance_v4.py"
SPEC = importlib.util.spec_from_file_location("sdk_v4_comparator", SCRIPT)
...
def _stage_contracts(destination: Path) -> Path:
    ...
    (target / "sdk-v4").mkdir(parents=True)
    ...
    for name in [
        ...
        shutil.copy2(source / "sdk-v4" / name, target / "sdk-v4" / name)
    return destination
...
def test_loads_exact_catalog() -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ("manifest_transition_cases", ["sdk.runtime.cancellation"], "transition_scope_drift"),
