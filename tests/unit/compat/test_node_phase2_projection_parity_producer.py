"""Fail-closed tests for Node's provider-free Phase-2 report producer."""
...
def test_acceptance_runs_real_producer_then_committed_comparator() -> None:
    ...
    producer_call = 'resolve(STAGE, "phase2_parity_check.mjs")'
    comparator_call = 'resolve(ROOT, "scripts/ci/compare_phase2_projection_parity.py")'
    ...
    assert "compare_phase2_projection_parity" not in producer
    ...
    assert "generated_phase2_foreign" in producer
...
def test_producer_does_not_require_generated_packages_for_publication_helpers() -> None:
    ...
    environment.pop("TYPE_BRIDGE_PHASE2_PARITY_REPORT", None)
    source = """
const producer = await import(process.argv[1]);
...
"""
    ...
    assert completed.stdout == ("typebridge.phase2-projected-parity-report/v1\ntypedb-3.12.1/v1")
