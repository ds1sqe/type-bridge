"""Provider-free acceptance for Python's generated Sdk V5 codec."""
...
PRODUCER = ROOT / "type-bridge-core/crates/schema-codegen/tests/acceptance/sdk_v5_codec.py"
...
def test_generated_python_sdk_v5_canonical_codec(tmp_path: Path) -> None:
    ...
    first = Path(
        os.environ.get(
            "TYPE_BRIDGE_SDK_V5_PYTHON_CORPUS",
            tmp_path / "python-sdk-v5-corpus.json",
        )
    )
    repeated = tmp_path / "python-sdk-v5-corpus-repeat.json"
    operational = Path(
        os.environ.get(
            "TYPE_BRIDGE_SDK_V5_PYTHON_OPERATIONAL_EVIDENCE",
            tmp_path / "python-sdk-v5-operational.json",
        )
    )
    repeated_operational = tmp_path / "python-sdk-v5-operational-repeat.json"
