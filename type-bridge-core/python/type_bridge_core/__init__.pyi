"""Type stubs for the ``type_bridge_core`` Rust extension module.

Hand-maintained to mirror the PyO3 bindings in
``crates/python/src/lib.rs`` (and ``transpiler.rs``). Shipped with the package
(``py.typed``) so downstream consumers and this repo share one source of type
truth for the extension. Only the symbols imported directly by Python source
carry precise signatures; the remaining extension classes and functions are
reached dynamically and stay visible through the module-level ``__getattr__``
fallback rather than being enumerated here.

``_ArchivedTypeSchema``'s ``entities`` / ``relations`` / ``attributes`` / ``functions`` /
``structs`` are PyO3 ``#[getter]`` properties, so they are declared with
``@property``; declaring them as plain methods makes type checkers reject
``schema.attributes[...]`` and ``"x" in schema.attributes``.
"""

from collections.abc import Mapping, Sequence
from os import PathLike
from typing import Any, Literal, Never, Protocol, TypedDict, final, overload

class ValueCoercer:
    def __init__(self) -> None: ...
    def coerce(self, value: Any, target_type: str) -> Any: ...
    def coerce_batch(self, pairs: list[tuple[Any, str]]) -> Any: ...
    def format_typeql(self, value: Any, value_type: str) -> str: ...

class QueryCompiler:
    def __init__(self) -> None: ...
    def compile(self, clauses: list[Any]) -> str: ...
    def compile_dicts(self, clauses: list[Any]) -> str: ...
    def parse(self, input: str) -> Any: ...

class _ArchivedTypeSchema:
    @staticmethod
    def from_typeql(input: str) -> _ArchivedTypeSchema: ...
    @staticmethod
    def from_json(json: str) -> _ArchivedTypeSchema: ...
    def to_json(self) -> str: ...
    def is_abstract(self, type_name: str) -> bool: ...
    def get_all_owned_attributes(self, name: str) -> list[Any]: ...
    def get_all_plays_roles(self, name: str) -> list[Any]: ...
    def get_all_relates(self, name: str) -> list[Any]: ...
    def validate_query(self, clauses: list[Any]) -> Any: ...
    @property
    def entities(self) -> Mapping[str, Any]: ...
    @property
    def relations(self) -> Mapping[str, Any]: ...
    @property
    def attributes(self) -> Mapping[str, Any]: ...
    @property
    def functions(self) -> Mapping[str, Any]: ...
    @property
    def structs(self) -> Mapping[str, Any]: ...

class ValidationEngine:
    def __init__(self) -> None: ...
    def validate_type_name(self, name: str, context: str) -> bool: ...
    def validate_variable_name(self, name: str, context: str) -> bool: ...
    def validate_pattern(self, pattern: Any) -> bool: ...
    def validate_statement(self, statement: Any) -> bool: ...
    def validate_query(self, clauses: list[Any], schema: _ArchivedTypeSchema) -> Any: ...
    def add_rule(self, rule_dict: Any) -> None: ...
    def load_rules(self, json_str: str) -> list[str]: ...
    def export_rules(self) -> str: ...
    def clear_rules(self) -> None: ...
    def rule_count(self) -> int: ...
    def validate_entity(
        self, entity_data: Any, schema: _ArchivedTypeSchema | None = ...
    ) -> Any: ...

def coerce_value(value: Any, target_type: str) -> Any: ...
def format_value(value: Any) -> str: ...
def parse_typeql_query(input: str) -> Any: ...
def _render_models_json(input: str, target: str, options_json: str | None = ...) -> str: ...
def toml_to_typeql(toml_text: str) -> str: ...
def run_v2_cli(arguments: list[str]) -> int: ...

class PyAdoptionRevision: ...

class PyAdoptionDirectoryEntry:
    @property
    def name(self) -> str: ...
    def name_bytes(self) -> bytes: ...
    def is_file(self) -> bool: ...
    def is_directory(self) -> bool: ...
    def is_symlink(self) -> bool: ...

class PyAdoptionDirectoryAuthority:
    @staticmethod
    def open(path: str) -> PyAdoptionDirectoryAuthority: ...
    def directory_revision(self) -> PyAdoptionRevision: ...
    def require_directory_revision(self, revision: PyAdoptionRevision) -> None: ...
    def entries(
        self,
        relative: str,
        maximum_entries: int,
        expected_directory: PyAdoptionDirectoryEntry | None = ...,
    ) -> list[PyAdoptionDirectoryEntry]: ...
    def inspect(
        self,
        relative: str,
        expected_parent: PyAdoptionDirectoryEntry | None = ...,
    ) -> PyAdoptionDirectoryEntry | None: ...
    def read_bounded(
        self,
        relative: str,
        limit: int,
        expected: PyAdoptionDirectoryEntry | None = ...,
    ) -> bytes: ...
    def write_atomic_no_replace(self, name: str, contents: bytes) -> None: ...
    def validate_publication_name(self, name: str) -> None: ...
    def remove_if_matches(
        self,
        name: str,
        expected: PyAdoptionDirectoryEntry,
        expected_bytes: bytes,
    ) -> bool: ...

# Prepared V2 query facade — opaque authority handle plus canonical
# plan/envelope bytes; limits accept any int and are range-checked
# against the unsigned 64-bit wire contract.
class QueryV2Authority:
    def __init__(self, declared_schema: bytes, scope: str, profile: str) -> None: ...
    @staticmethod
    def query_only(
        database: PyRustDatabase,
        declared_schema: bytes,
        scope: str,
        profile: str,
    ) -> QueryV2Authority: ...

type QueryV2ValueType = Literal[
    "string",
    "long",
    "double",
    "boolean",
    "date",
    "datetime",
    "datetime_tz",
    "decimal",
    "duration",
]
type QueryV2TypeKind = Literal["entity", "relation", "attribute"]
type QueryV2Comparator = Literal[
    "equal",
    "not_equal",
    "less",
    "less_or_equal",
    "greater",
    "greater_or_equal",
]
type QueryV2OrderDirection = Literal["ascending", "descending"]
type QueryV2Reducer = Literal["count", "max", "mean", "median", "min", "std", "sum"]
type QueryV2Scalar = str | int | float | bool

class _QueryV2ExactFloat(Protocol):
    """Static marker accepted by ordinary ``float`` values but not ``int``."""

    def hex(self) -> str: ...

@final
class _QueryV2AuthorityIdentity:
    def __new__(cls) -> Never: ...
    def same_authority(self, other: _QueryV2AuthorityIdentity) -> bool: ...

@final
class _QueryV2BindingHandle:
    def __new__(cls) -> Never: ...

@final
class _QueryV2InputHandle:
    def __new__(cls) -> Never: ...

@final
class _QueryV2OperandHandle:
    def __new__(cls) -> Never: ...

@final
class _QueryV2PatternHandle:
    def __new__(cls) -> Never: ...

@final
class _QueryV2OrderHandle:
    def __new__(cls) -> Never: ...

@final
class _QueryV2ReduceAssignmentHandle:
    def __new__(cls) -> Never: ...

@final
class _QueryV2LocalReturnHandle:
    def __new__(cls) -> Never: ...

@final
class _QueryV2LocalFunctionHandle:
    def __new__(cls) -> Never: ...

@final
class _QueryV2DocumentFieldHandle:
    def __new__(cls) -> Never: ...

@final
class AuthoredQueryInvocation:
    def __new__(cls) -> Never: ...
    @property
    def canonical_bytes(self) -> bytes: ...
    @property
    def operation(self) -> Literal["rows", "count", "exists"]: ...
    @property
    def plan_fingerprint(self) -> str: ...
    @property
    def authority_identity(self) -> _QueryV2AuthorityIdentity: ...
    @property
    def required_transport_capabilities(self) -> tuple[str, ...]: ...

@final
class AuthoredQueryPlan:
    def __new__(cls) -> Never: ...
    @property
    def canonical_bytes(self) -> bytes: ...
    @property
    def format(self) -> Literal["typebridge.query-plan/v2"]: ...
    @property
    def fingerprint(self) -> str: ...
    @property
    def required_capabilities(self) -> tuple[str, ...]: ...
    @property
    def authority_identity(self) -> _QueryV2AuthorityIdentity: ...
    def rows(self, rows: Sequence[Sequence[QueryV2Scalar | None]]) -> AuthoredQueryInvocation: ...
    def documents(
        self, rows: Sequence[Sequence[QueryV2Scalar | None]]
    ) -> AuthoredQueryInvocation: ...
    def count(self, rows: Sequence[Sequence[QueryV2Scalar | None]]) -> AuthoredQueryInvocation: ...
    def exists(self, rows: Sequence[Sequence[QueryV2Scalar | None]]) -> AuthoredQueryInvocation: ...

@final
class QueryPlanBuilder:
    def __init__(self, authority: QueryV2Authority) -> None: ...
    def binding(self, variable: str) -> _QueryV2BindingHandle: ...
    def input(
        self, public_name: str, value_type: QueryV2ValueType, optional: bool
    ) -> _QueryV2InputHandle: ...
    def binding_operand(self, binding: _QueryV2BindingHandle) -> _QueryV2OperandHandle: ...
    @overload
    def literal_operand(
        self,
        value_type: Literal[
            "string",
            "date",
            "datetime",
            "datetime_tz",
            "decimal",
            "duration",
        ],
        value: str,
    ) -> _QueryV2OperandHandle: ...
    @overload
    def literal_operand(self, value_type: Literal["long"], value: int) -> _QueryV2OperandHandle: ...
    @overload
    def literal_operand(
        self, value_type: Literal["double"], value: _QueryV2ExactFloat
    ) -> _QueryV2OperandHandle: ...
    @overload
    def literal_operand(
        self, value_type: Literal["boolean"], value: bool
    ) -> _QueryV2OperandHandle: ...
    def input_operand(self, input: _QueryV2InputHandle) -> _QueryV2OperandHandle: ...
    def isa(
        self,
        binding: _QueryV2BindingHandle,
        type_kind: QueryV2TypeKind,
        type_label: str,
        include_subtypes: bool,
    ) -> _QueryV2PatternHandle: ...
    def has(
        self,
        owner: _QueryV2BindingHandle,
        attribute: _QueryV2BindingHandle,
        attribute_label: str,
    ) -> _QueryV2PatternHandle: ...
    def links(
        self,
        relation: _QueryV2BindingHandle,
        relation_label: str,
        roles: Sequence[str],
        players: Sequence[_QueryV2BindingHandle],
    ) -> _QueryV2PatternHandle: ...
    def value(
        self,
        comparator: QueryV2Comparator,
        left: _QueryV2OperandHandle,
        right: _QueryV2OperandHandle,
    ) -> _QueryV2PatternHandle: ...
    def not_(self, patterns: Sequence[_QueryV2PatternHandle]) -> _QueryV2PatternHandle: ...
    def or_(self, branches: Sequence[Sequence[_QueryV2PatternHandle]]) -> _QueryV2PatternHandle: ...
    def try_(self, patterns: Sequence[_QueryV2PatternHandle]) -> _QueryV2PatternHandle: ...
    def reachable(
        self,
        source: _QueryV2BindingHandle,
        target: _QueryV2BindingHandle,
        relation_label: str,
        role_from: str,
        role_to: str,
        min_depth: int,
        max_depth: int,
    ) -> _QueryV2PatternHandle: ...
    @overload
    def function_call(
        self,
        assigned: _QueryV2BindingHandle,
        arguments: Sequence[_QueryV2OperandHandle],
        function_name: str,
        local_function: None = ...,
    ) -> _QueryV2PatternHandle: ...
    @overload
    def function_call(
        self,
        assigned: _QueryV2BindingHandle,
        arguments: Sequence[_QueryV2OperandHandle],
        function_name: None,
        local_function: _QueryV2LocalFunctionHandle,
    ) -> _QueryV2PatternHandle: ...
    @overload
    def function_call(
        self,
        assigned: _QueryV2BindingHandle,
        arguments: Sequence[_QueryV2OperandHandle],
        *,
        local_function: _QueryV2LocalFunctionHandle,
    ) -> _QueryV2PatternHandle: ...
    def order(
        self, binding: _QueryV2BindingHandle, direction: QueryV2OrderDirection
    ) -> _QueryV2OrderHandle: ...
    @overload
    def reduce_assignment(
        self,
        assigned: _QueryV2BindingHandle,
        reducer: Literal["count"],
        input: None = ...,
    ) -> _QueryV2ReduceAssignmentHandle: ...
    @overload
    def reduce_assignment(
        self,
        assigned: _QueryV2BindingHandle,
        reducer: Literal["max", "mean", "median", "min", "std", "sum"],
        input: _QueryV2BindingHandle,
    ) -> _QueryV2ReduceAssignmentHandle: ...
    @overload
    def local_return(
        self,
        reducer: Literal["count"],
        input: _QueryV2BindingHandle,
        value_type: Literal["long"],
    ) -> _QueryV2LocalReturnHandle: ...
    @overload
    def local_return(
        self,
        reducer: Literal["sum"],
        input: _QueryV2BindingHandle,
        value_type: Literal["long", "double"],
    ) -> _QueryV2LocalReturnHandle: ...
    def local_function(
        self,
        name: str,
        bindings: Sequence[_QueryV2BindingHandle],
        parameter_bindings: Sequence[_QueryV2BindingHandle],
        parameter_labels: Sequence[str],
        body: Sequence[_QueryV2PatternHandle],
        returns: _QueryV2LocalReturnHandle,
    ) -> _QueryV2LocalFunctionHandle: ...
    def match(self, patterns: Sequence[_QueryV2PatternHandle]) -> None: ...
    def select(self, bindings: Sequence[_QueryV2BindingHandle]) -> None: ...
    def require(self, bindings: Sequence[_QueryV2BindingHandle]) -> None: ...
    def distinct(self) -> None: ...
    def reduce(
        self,
        assignments: Sequence[_QueryV2ReduceAssignmentHandle],
        groups: Sequence[_QueryV2BindingHandle],
    ) -> None: ...
    def sort(self, terms: Sequence[_QueryV2OrderHandle]) -> None: ...
    def offset(self, rows: int) -> None: ...
    def limit(self, rows: int) -> None: ...
    def document_binding(
        self, key: str, binding: _QueryV2BindingHandle
    ) -> _QueryV2DocumentFieldHandle: ...
    def document_attribute_list(
        self, key: str, owner: _QueryV2BindingHandle, attribute_label: str
    ) -> _QueryV2DocumentFieldHandle: ...
    def finalize_rows(self, columns: Sequence[_QueryV2BindingHandle]) -> AuthoredQueryPlan: ...
    def finalize_documents(
        self, fields: Sequence[_QueryV2DocumentFieldHandle]
    ) -> AuthoredQueryPlan: ...

class _QueryV2ErrorTextPathSegment(TypedDict):
    kind: Literal["field", "identifier"]
    value: str

class _QueryV2ErrorIndexPathSegment(TypedDict):
    kind: Literal["index"]
    value: int

type _QueryV2ErrorPathSegment = _QueryV2ErrorTextPathSegment | _QueryV2ErrorIndexPathSegment

class _QueryV2ErrorTextDetail(TypedDict):
    kind: Literal["text", "long"]
    value: str

class _QueryV2ErrorBooleanDetail(TypedDict):
    kind: Literal["boolean"]
    value: bool

class _QueryV2ErrorTextListDetail(TypedDict):
    kind: Literal["text_list"]
    value: list[str]

type _QueryV2ErrorDetail = (
    _QueryV2ErrorTextDetail | _QueryV2ErrorBooleanDetail | _QueryV2ErrorTextListDetail
)

class QueryV2Error(ValueError):
    category: Literal[
        "invalid_contract",
        "unsupported_capability",
        "resource_limit",
        "integrity",
    ]
    code: str
    message: str
    path: list[_QueryV2ErrorPathSegment]
    details: Mapping[str, _QueryV2ErrorDetail]

class PendingQueryV2Remote:
    def request_bytes(self) -> bytes: ...
    def decode_reply(self, response: bytes | bytearray) -> str:
        """Claim once, snapshot a bounded reply, and verify its request binding."""
        ...

def query_v2_authority(declared_schema: bytes, scope: str, profile: str) -> QueryV2Authority: ...
def query_v2_authority_from_schema_authority(
    schema_authority: bytes,
    semantic_fingerprint: bytes,
) -> QueryV2Authority: ...
def query_v2_query_only_authority(
    database: PyRustDatabase,
    declared_schema: bytes,
    scope: str,
    profile: str,
) -> QueryV2Authority:
    """Bind a local-only authority to one exact database with no migration controls."""
    ...

def query_v2_execute_local(
    database: PyRustDatabase,
    authority: QueryV2Authority,
    plan: bytes,
    invocation_json: str,
    deadline_ms: int | None = ...,
) -> str:
    """Runs with the GIL released; the optional deadline bounds the round trip."""
    ...

def query_v2_remote_capabilities(advertisement: bytes) -> list[str]:
    """Inspect an advertisement already trusted through TLS or out-of-band pinning."""
    ...

def query_v2_prepare_remote(
    authority: QueryV2Authority,
    plan: bytes,
    invocation_json: str,
    advertisement: bytes,
    max_items: int,
    max_bytes: int,
    max_collection_members: int,
    deadline_ms: int | None = ...,
) -> PendingQueryV2Remote:
    """Bind a trusted executor advertisement and absolute expiry into one request."""
    ...

# Version gate — SSOT in crates/core/src/version.rs, re-exported here.
class VersionError(Exception):
    """Raised for TypeDB version window violations, band mismatches, probe failures, or parse errors."""

    ...

def min_supported_version() -> str:
    """Return the minimum supported TypeDB version as ``"3.11.0"``."""
    ...

def max_supported_line() -> str:
    """Return the maximum supported TypeDB line as ``"major.minor"`` (e.g. ``"3.12"``)."""
    ...

def band(version: str) -> int | None:
    """Return the protocol band for a TypeDB version string, or ``None`` if unmapped.

    Raises ``VersionError`` when the version string cannot be parsed.
    """
    ...

def check_supported(driver: str, server: str) -> None:
    """Assert that driver and server versions are compatible (window + band).

    Raises ``VersionError`` on window violations or band mismatches.
    """
    ...

def embedded_driver_version() -> str:
    """Return the typedb-driver version compiled into the Rust runtime.

    Returns the band-8 (3.11.x) pin for back-compat. Use
    ``embedded_driver_versions()`` to get all compiled-in bands.
    """
    ...

def embedded_driver_versions() -> dict[int, str]:
    """Return all driver versions compiled into the Rust runtime, keyed by protocol band.

    Returns a dict mapping ``int`` band → ``str`` version for every band
    feature compiled into this build.  The default build returns
    ``{8: "3.11.5", 9: "3.12.3"}``.
    """
    ...

def check_server_supported(server: str) -> None:
    """Assert this build's embedded runtime can serve ``server``.

    Uses band-set membership (not equality): the server passes when its protocol
    band is among the bands compiled into this build.  The band set is derived
    from ``embedded_driver_versions()`` — never a hardcoded literal.

    Raises ``VersionError`` when the server is outside the support window or
    its band is not compiled into this build.
    """
    ...

def server_version(
    address: str,
    http_port: int = 8000,
    tls: bool = False,
    tls_root_ca: str | PathLike[str] | None = ...,
) -> str:
    """Query the TypeDB HTTP API for the server version.

    Returns the detected version as a string (e.g. ``"3.12.3"``).
    Raises ``VersionError`` when the endpoint is unreachable or the response
    cannot be parsed, and ``ValueError`` for invalid custom-root configuration.
    """
    ...

class _CustomRootCaSnapshot:
    """Private owner for one Rust-validated immutable CA snapshot."""

    def __init__(self, configured_path: str | PathLike[str]) -> None: ...
    @property
    def path(self) -> PathLike[str]: ...
    def cleanup(self) -> None: ...

class PyRustDatabase:
    """Rust-backed TypeDB database connection."""

    @staticmethod
    def connect(
        address: str,
        database: str,
        username: str | None = ...,
        password: str | None = ...,
        http_port: int = 8000,
        server_version: str | None = ...,
        tls: bool | None = ...,
        tls_root_ca: str | PathLike[str] | None = ...,
    ) -> PyRustDatabase: ...
    def close(self) -> None: ...
    def database_exists(self) -> bool: ...
    def database_exists_controlled(
        self,
        *,
        timeout_milliseconds: int | None = ...,
        cancellation: MigrationCancellation | None = ...,
    ) -> bool: ...
    def create_database(self) -> None: ...
    def create_database_controlled(
        self,
        *,
        timeout_milliseconds: int | None = ...,
        cancellation: MigrationCancellation | None = ...,
    ) -> None: ...
    def create_database_outcome(self) -> Literal["created", "already_exists"]: ...
    def create_database_outcome_controlled(
        self,
        *,
        timeout_milliseconds: int | None = ...,
        cancellation: MigrationCancellation | None = ...,
    ) -> Literal["created", "already_exists"]: ...
    def delete_database(self) -> None: ...
    def delete_database_outcome(self) -> Literal["deleted", "already_absent"]: ...
    def inspect_database_pair(
        self,
    ) -> Literal["absent", "standalone_managed", "owned_pair", "owned_journal_orphan"]: ...
    def inspect_database_pair_controlled(
        self,
        *,
        timeout_milliseconds: int | None = ...,
        cancellation: MigrationCancellation | None = ...,
    ) -> Literal["absent", "standalone_managed", "owned_pair", "owned_journal_orphan"]: ...
    def plan_database_delete(self) -> PyManagedDatabaseDeletionPlan: ...
    def plan_database_delete_controlled(
        self,
        *,
        timeout_milliseconds: int | None = ...,
        cancellation: MigrationCancellation | None = ...,
    ) -> PyManagedDatabaseDeletionPlan: ...
    def server_version(self) -> str | None: ...
    def transaction(self, transaction_type: str = "read") -> PyRustTransactionContext: ...
    def __getattr__(self, name: str) -> Any: ...

class PyManagedDatabaseDeletionPlan:
    """Owned single-use pair-aware managed database deletion plan."""

    def inspected_state(
        self,
    ) -> Literal["absent", "standalone_managed", "owned_pair", "owned_journal_orphan"]: ...
    def execute(
        self,
    ) -> Literal[
        "already_absent",
        "deleted_standalone_managed",
        "deleted_owned_pair",
        "deleted_owned_journal_orphan",
    ]: ...
    def execute_controlled(
        self,
        *,
        timeout_milliseconds: int | None = ...,
        cancellation: MigrationCancellation | None = ...,
    ) -> Literal[
        "already_absent",
        "deleted_standalone_managed",
        "deleted_owned_pair",
        "deleted_owned_journal_orphan",
    ]: ...
    def close(self) -> None: ...

class PyRustTransactionContext:
    """Rust-backed TypeDB transaction context."""

    def __getattr__(self, name: str) -> Any: ...

# Internal native typed-match seam. Public authoring types live in
# ``type_bridge.typed``; these classes intentionally expose no state getters.
class _QueryDescriptorRegistry:
    def __init__(self) -> None: ...
    def register_entity(self, descriptor: Mapping[str, Any]) -> Mapping[str, Any]: ...
    def register_relation(self, descriptor: Mapping[str, Any]) -> Mapping[str, Any]: ...
    def entity(self, type_name: str) -> Mapping[str, Any]: ...
    def relation(self, type_name: str) -> Mapping[str, Any]: ...
    def snapshot(self) -> list[Mapping[str, Any]]: ...
    def schema_info(self) -> Mapping[str, Any]: ...

class DynamicValue:
    @staticmethod
    def string(value: str) -> DynamicValue: ...
    @staticmethod
    def long(value: int) -> DynamicValue: ...
    @staticmethod
    def double(value: float) -> DynamicValue: ...
    @staticmethod
    def boolean(value: bool) -> DynamicValue: ...
    @staticmethod
    def date(value: str) -> DynamicValue: ...
    @staticmethod
    def datetime(value: str) -> DynamicValue: ...
    @staticmethod
    def datetime_tz(value: str) -> DynamicValue: ...
    @staticmethod
    def decimal(value: str) -> DynamicValue: ...
    @staticmethod
    def duration(value: str) -> DynamicValue: ...

@final
class QueryExecutionResourceLimits:
    def __init__(
        self,
        timeout_milliseconds: int | None = ...,
        items: int | None = ...,
        bytes: int | None = ...,
        graph_nodes: int | None = ...,
        attribute_values: int | None = ...,
        collection_members: int | None = ...,
        role_players: int | None = ...,
        statements: int | None = ...,
    ) -> None: ...
    @property
    def timeout_milliseconds(self) -> int: ...
    @property
    def items(self) -> int: ...
    @property
    def bytes(self) -> int: ...
    @property
    def graph_nodes(self) -> int: ...
    @property
    def attribute_values(self) -> int: ...
    @property
    def collection_members(self) -> int: ...
    @property
    def role_players(self) -> int: ...
    @property
    def statements(self) -> int: ...

@final
class QueryCancellation:
    def __init__(self) -> None: ...
    def cancel(self) -> None: ...
    @property
    def is_cancelled(self) -> bool: ...

class MatchFunctionArgumentHandle: ...

class MatchFunctionHandle:
    def call(self, arguments: Sequence[MatchFunctionArgumentHandle]) -> MatchFunctionCallHandle: ...

class MatchFunctionValueHandle:
    def function_argument(self) -> MatchFunctionArgumentHandle: ...

class MatchFunctionCallHandle:
    def function_argument(self) -> MatchFunctionArgumentHandle: ...
    def compare_field(self, operator: str, field: MatchFieldHandle) -> MatchPredicateHandle: ...
    def compare_value(
        self, operator: str, value: MatchFunctionValueHandle
    ) -> MatchPredicateHandle: ...
    def compare_call(
        self, operator: str, other: MatchFunctionCallHandle
    ) -> MatchPredicateHandle: ...

class MatchBindingHandle:
    def iid(self, iid: str) -> MatchPredicateHandle: ...
    def iid_in(self, iids: list[str]) -> MatchPredicateHandle: ...
    def function_argument(self) -> MatchFunctionArgumentHandle: ...
    def field(self, field_name: str) -> MatchFieldHandle: ...
    def field_owned_by(self, owner_type: str, field_name: str) -> MatchFieldHandle: ...
    def role(self, role_name: str) -> MatchRoleHandle: ...
    def role_owned_by(self, owner_type: str, role_name: str) -> MatchRoleHandle: ...
    def one(self) -> MatchSelectionHandle: ...
    def collect(self) -> MatchSelectionHandle: ...

class MatchFieldHandle:
    def presence(self, present: bool) -> MatchPredicateHandle: ...
    def compare_value(self, operator: str, value: DynamicValue) -> MatchPredicateHandle: ...
    def compare_field(self, operator: str, other: MatchFieldHandle) -> MatchPredicateHandle: ...
    def order(self, direction: str, missing: str) -> MatchOrderHandle: ...

class MatchRoleHandle:
    def connects(self, player: MatchBindingHandle) -> MatchPredicateHandle: ...

class MatchPredicateHandle:
    def and_(self, other: MatchPredicateHandle) -> MatchPredicateHandle: ...
    def or_(self, other: MatchPredicateHandle) -> MatchPredicateHandle: ...
    def not_(self) -> MatchPredicateHandle: ...

class MatchOrderHandle: ...

class MatchSelectionHandle:
    def distinct(self, enabled: bool = True) -> MatchSelectionHandle: ...
    def order_by(self, order: MatchOrderHandle) -> MatchSelectionHandle: ...

class MatchShapeHandle: ...

class ValidatedMatchResultHandle:
    def __new__(cls) -> Never: ...
    def row_count(self) -> int: ...
    def row(self, index: int) -> ValidatedMatchRowHandle: ...
    def page_entry_count(self) -> int: ...
    def page_entry(self, index: int) -> ValidatedMatchRowHandle: ...
    def page_offset(self) -> int: ...
    def page_limit(self) -> int: ...
    def page_total(self) -> int | None: ...
    def count_value(self) -> int: ...
    def exists_value(self) -> bool: ...
    def reduction_row_count(self) -> int: ...
    def reduction_value_count(self, row: int) -> int: ...
    def reduction_value_kind(self, row: int, index: int) -> str: ...
    def reduction_value(self, row: int, index: int) -> int | float | None: ...
    def reduction_group(self, row: int) -> ValidatedMatchThingHandle: ...
    def reduction_group_value(self, row: int) -> object: ...
    def reduction_group_values(self, row: int) -> list[object]: ...

class ValidatedMatchRowHandle:
    def __new__(cls) -> Never: ...
    def slot_count(self) -> int: ...
    def slot(self, index: int) -> ValidatedMatchSlotHandle: ...

class ValidatedMatchSlotHandle:
    def __new__(cls) -> Never: ...
    def name(self) -> str | None: ...
    def is_collection(self) -> bool: ...
    def thing_count(self) -> int: ...
    def thing(self, index: int) -> ValidatedMatchThingHandle: ...

class ValidatedMatchThingHandle:
    def __new__(cls) -> Never: ...
    def iid(self) -> str: ...
    def declared_type_name(self) -> str: ...
    def concrete_type_name(self) -> str: ...
    def kind(self) -> str: ...
    def attribute_count(self) -> int: ...
    def attribute(self, index: int) -> ValidatedMatchAttributeHandle: ...
    def role_count(self) -> int: ...
    def role(self, index: int) -> ValidatedMatchRoleHandle: ...

class ValidatedMatchAttributeHandle:
    def __new__(cls) -> Never: ...
    def field_name(self) -> str: ...
    def value_count(self) -> int: ...
    def value_type(self, index: int) -> str: ...
    def value(self, index: int) -> object: ...

class ValidatedMatchRoleHandle:
    def __new__(cls) -> Never: ...
    def role_name(self) -> str: ...
    def player_count(self) -> int: ...
    def player(self, index: int) -> ValidatedMatchRolePlayerHandle: ...

class ValidatedMatchRolePlayerHandle:
    def __new__(cls) -> Never: ...
    def iid(self) -> str: ...
    def declared_type_name(self) -> str: ...
    def concrete_type_name(self) -> str: ...
    def kind(self) -> str: ...
    def attribute_count(self) -> int: ...
    def attribute(self, index: int) -> ValidatedMatchAttributeHandle: ...

class MatchQueryHandle:
    @property
    def is_closed(self) -> bool: ...
    def close(self) -> None: ...
    def fork(self) -> MatchQueryHandle: ...
    def add_hidden(self, binding: MatchBindingHandle) -> MatchQueryHandle: ...
    def where_predicate(self, predicate: MatchPredicateHandle) -> MatchQueryHandle: ...
    def allow_cross_join(
        self, left: MatchBindingHandle, right: MatchBindingHandle
    ) -> MatchQueryHandle: ...
    def fetch_rows_diagnostic(
        self,
        order: list[MatchOrderHandle],
        offset: int,
        limit: int,
        cardinality: str,
    ) -> str: ...
    def execute_fetch_rows_owned(
        self,
        database: PyRustDatabase,
        order: list[MatchOrderHandle],
        offset: int,
        limit: int,
        cardinality: str,
    ) -> ValidatedMatchResultHandle: ...
    def execute_fetch_rows_borrowed(
        self,
        transaction: PyRustTransactionContext,
        order: list[MatchOrderHandle],
        offset: int,
        limit: int,
        cardinality: str,
    ) -> ValidatedMatchResultHandle: ...
    def execute_page_by_owned(
        self,
        database: PyRustDatabase,
        root: MatchBindingHandle,
        order: list[MatchOrderHandle],
        offset: int,
        limit: int,
        include_total: bool,
    ) -> ValidatedMatchResultHandle: ...
    def execute_page_by_borrowed(
        self,
        transaction: PyRustTransactionContext,
        root: MatchBindingHandle,
        order: list[MatchOrderHandle],
        offset: int,
        limit: int,
        include_total: bool,
    ) -> ValidatedMatchResultHandle: ...
    def execute_count_by_owned(
        self, database: PyRustDatabase, root: MatchBindingHandle
    ) -> ValidatedMatchResultHandle: ...
    def execute_count_by_borrowed(
        self, transaction: PyRustTransactionContext, root: MatchBindingHandle
    ) -> ValidatedMatchResultHandle: ...
    def execute_exists_by_owned(
        self, database: PyRustDatabase, root: MatchBindingHandle
    ) -> ValidatedMatchResultHandle: ...
    def execute_exists_by_borrowed(
        self, transaction: PyRustTransactionContext, root: MatchBindingHandle
    ) -> ValidatedMatchResultHandle: ...
    def page_by_diagnostic(
        self,
        root: MatchBindingHandle,
        order: list[MatchOrderHandle],
        offset: int,
        limit: int,
        include_total: bool,
    ) -> str: ...
    def count_by_diagnostic(self, root: MatchBindingHandle) -> str: ...
    def exists_by_diagnostic(self, root: MatchBindingHandle) -> str: ...
    def reduce_by_diagnostic(
        self,
        root: MatchBindingHandle,
        group: MatchBindingHandle | None,
        reducers: Sequence[str],
        inputs: Sequence[MatchFieldHandle | None],
    ) -> str: ...
    def execute_reduce_by_owned(
        self,
        database: PyRustDatabase,
        root: MatchBindingHandle,
        group: MatchBindingHandle | None,
        reducers: Sequence[str],
        inputs: Sequence[MatchFieldHandle | None],
    ) -> ValidatedMatchResultHandle: ...
    def execute_reduce_by_borrowed(
        self,
        transaction: PyRustTransactionContext,
        root: MatchBindingHandle,
        group: MatchBindingHandle | None,
        reducers: Sequence[str],
        inputs: Sequence[MatchFieldHandle | None],
    ) -> ValidatedMatchResultHandle: ...
    def reduce_by_field_diagnostic(
        self,
        root: MatchBindingHandle,
        group: MatchFieldHandle,
        reducers: Sequence[str],
        inputs: Sequence[MatchFieldHandle | None],
    ) -> str: ...
    def execute_reduce_by_field_owned(
        self,
        database: PyRustDatabase,
        root: MatchBindingHandle,
        group: MatchFieldHandle,
        reducers: Sequence[str],
        inputs: Sequence[MatchFieldHandle | None],
    ) -> ValidatedMatchResultHandle: ...
    def execute_reduce_by_field_borrowed(
        self,
        transaction: PyRustTransactionContext,
        root: MatchBindingHandle,
        group: MatchFieldHandle,
        reducers: Sequence[str],
        inputs: Sequence[MatchFieldHandle | None],
    ) -> ValidatedMatchResultHandle: ...
    def reduce_by_fields_diagnostic(
        self,
        root: MatchBindingHandle,
        groups: Sequence[MatchFieldHandle],
        reducers: Sequence[str],
        inputs: Sequence[MatchFieldHandle | None],
    ) -> str: ...
    def execute_reduce_by_fields_owned(
        self,
        database: PyRustDatabase,
        root: MatchBindingHandle,
        groups: Sequence[MatchFieldHandle],
        reducers: Sequence[str],
        inputs: Sequence[MatchFieldHandle | None],
    ) -> ValidatedMatchResultHandle: ...
    def execute_reduce_by_fields_borrowed(
        self,
        transaction: PyRustTransactionContext,
        root: MatchBindingHandle,
        groups: Sequence[MatchFieldHandle],
        reducers: Sequence[str],
        inputs: Sequence[MatchFieldHandle | None],
    ) -> ValidatedMatchResultHandle: ...

class MatchSessionHandle:
    def __init__(self, registry: _QueryDescriptorRegistry) -> None: ...
    @property
    def is_closed(self) -> bool: ...
    def close(self) -> None: ...
    def exact(self, type_name: str) -> MatchBindingHandle: ...
    def subtypes(self, type_name: str) -> MatchBindingHandle: ...
    def function_by_id(self, function_id: str) -> MatchFunctionHandle: ...
    def function_value(
        self, attribute_type_key: str, value: DynamicValue
    ) -> MatchFunctionValueHandle: ...
    def reachable(
        self,
        relation_type: str,
        role_from: str,
        role_to: str,
        source: MatchBindingHandle,
        target: MatchBindingHandle,
        min_depth: int,
        max_depth: int,
    ) -> MatchPredicateHandle: ...
    def positional(self, slots: list[MatchSelectionHandle]) -> MatchShapeHandle: ...
    def named(
        self, names: list[str], selections: list[MatchSelectionHandle]
    ) -> MatchShapeHandle: ...
    def named_checked(
        self,
        declarations: list[tuple[str, str, bool]],
        names: list[str],
        selections: list[MatchSelectionHandle],
    ) -> MatchShapeHandle: ...
    def query(self, shape: MatchShapeHandle) -> MatchQueryHandle: ...

@final
class PyProjectedModelManager:
    def __new__(cls) -> Never: ...
    def insert[T](self, instance: T) -> T: ...
    def insert_many[T](self, instances: Sequence[T]) -> list[T]: ...
    def put[T](self, instance: T) -> T: ...
    def put_many[T](self, instances: Sequence[T]) -> list[T]: ...
    def update[T](self, instance: T) -> T: ...
    def update_many[T](self, instances: Sequence[T]) -> list[T]: ...
    def resolve_iid[T](self, instance: T) -> str | None: ...
    def delete(self, instance_or_iid: object) -> None: ...
    def delete_many(self, iids: Sequence[str]) -> None: ...
    def filter(self, **filters: object) -> PyProjectedModelManager: ...
    def canonical_filter(self) -> PyProjectedManagerFilter: ...
    def all(self) -> list[object]: ...
    def legacy_all(self) -> list[object]: ...
    def first(self) -> object | None: ...
    def count(self) -> int: ...
    def exists(self) -> bool: ...
    def get_by_iid(self, iid: str) -> object | None: ...

@final
class PyProjectedManagerFilter:
    def __new__(cls) -> Never: ...
    def where_field(
        self,
        owner: type[object],
        field_name: str,
        metadata_json: str,
        comparison: str,
        value: object,
    ) -> PyProjectedManagerFilter: ...
    def reject_unissued_field(self) -> PyProjectedManagerFilter: ...
    def all(self) -> list[object]: ...
    def first(self) -> object | None: ...
    def count(self) -> int: ...
    def exists(self) -> bool: ...

@final
class PyRuntimeProjection:
    def __init__(
        self,
        projection_json: str,
        semantic_fingerprint_json: str,
        projection_fingerprint_json: str,
        models: Sequence[tuple[type[object], type[object] | None]],
        schema_authority: bytes | None = ...,
        structs: Sequence[type[object]] = ...,
    ) -> None: ...
    def connect_direct(
        self,
        endpoint: str,
        database: str,
        username: str = ...,
        password: str = ...,
        http_port: int = ...,
        tls_mode: str = ...,
        tls_root_ca: str | PathLike[str] | None = ...,
        connection_limits: QueryExecutionResourceLimits | None = ...,
        answer_limits: QueryExecutionResourceLimits | None = ...,
        cancellation: QueryCancellation | None = ...,
    ) -> PyRustDatabase: ...
    def manager_for_database(
        self,
        model: type[object],
        database: PyRustDatabase,
    ) -> PyProjectedModelManager: ...
    def manager_for_transaction(
        self,
        model: type[object],
        transaction: PyRustTransactionContext,
    ) -> PyProjectedModelManager: ...
    def validate_attribute_value(
        self,
        model: type[object],
        value: object,
    ) -> None: ...
    def validate_field_value(
        self,
        model: type[object],
        field_name: str,
        value: object,
    ) -> None: ...
    def validate_create(self, model: type[object], instance: object) -> None: ...
    def encode_attribute(self, model: type[object], instance: object) -> bytes: ...
    def decode_attribute(self, model: type[object], data: bytes) -> object: ...
    def encode_create(self, model: type[object], instance: object) -> bytes: ...
    def decode_create(self, model: type[object], data: bytes) -> object: ...
    def encode_reference(self, model: type[object], instance: object) -> bytes: ...
    def decode_reference(self, model: type[object], data: bytes) -> object: ...
    def encode_snapshot(self, model: type[object], instance: object) -> bytes: ...
    def decode_snapshot(self, model: type[object], data: bytes) -> object: ...
    def encode_struct(self, structure: type[object], instance: object) -> bytes: ...
    def decode_struct(self, structure: type[object], data: bytes) -> object: ...
    def encode_archive(self, records: Sequence[bytes]) -> bytes: ...
    def decode_archive(self, data: bytes) -> list[bytes]: ...
    def encode_archive_controlled(
        self,
        records: Sequence[bytes],
        *,
        cancellation: QueryCancellation | None = ...,
        timeout_milliseconds: int | None = ...,
        max_input_bytes: int | None = ...,
        max_output_bytes: int | None = ...,
        max_depth: int | None = ...,
        max_records: int | None = ...,
        max_members: int | None = ...,
    ) -> bytes: ...
    def decode_archive_controlled(
        self,
        data: bytes,
        *,
        cancellation: QueryCancellation | None = ...,
        timeout_milliseconds: int | None = ...,
        max_input_bytes: int | None = ...,
        max_output_bytes: int | None = ...,
        max_depth: int | None = ...,
        max_records: int | None = ...,
        max_members: int | None = ...,
    ) -> list[bytes]: ...
    def query_builder_match_entity(
        self,
        model: type[object],
        variable: str,
        filters: Mapping[str, object],
    ) -> str: ...
    def query_builder_insert_entity(self, instance: object, variable: str) -> str: ...
    def query_builder_match_relation(
        self,
        model: type[object],
        variable: str,
        role_players: Mapping[str, str] | None = ...,
    ) -> str: ...
    def match_session(self) -> MatchSessionHandle: ...
    def match_session_with_resources(
        self,
        resources: QueryExecutionResourceLimits,
        cancellation: QueryCancellation,
    ) -> MatchSessionHandle: ...
    def hydrate_thing(self, thing: ValidatedMatchThingHandle) -> object: ...

@final
class RemoteModelQueryContext:
    def __new__(cls) -> Never: ...

@final
class PendingRemoteModelQuery:
    def __new__(cls) -> Never: ...
    @property
    def is_closed(self) -> bool: ...
    def close(self) -> None: ...
    def request_bytes(self) -> bytes: ...
    def decode_reply(self, response: bytes | bytearray) -> ValidatedMatchResultHandle: ...

def query_v2_remote_model_context(
    authority: QueryV2Authority,
    advertisement: bytes | bytearray,
    max_items: int,
    max_bytes: int,
    max_collection_members: int,
    max_graph_nodes: int,
    max_attribute_values: int,
    max_role_players: int,
    deadline_ms: int | None = ...,
) -> RemoteModelQueryContext: ...
def query_v2_remote_model_context_with_resources(
    authority: QueryV2Authority,
    advertisement: bytes | bytearray,
    resources: QueryExecutionResourceLimits,
    cancellation: QueryCancellation,
) -> RemoteModelQueryContext: ...
def query_v2_prepare_remote_model_rows(
    query: MatchQueryHandle,
    context: RemoteModelQueryContext,
    order: list[MatchOrderHandle],
    offset: int,
    limit: int,
    cardinality: Literal["exactly_one", "bounded_many"],
) -> PendingRemoteModelQuery: ...
def query_v2_prepare_remote_model_page(
    query: MatchQueryHandle,
    context: RemoteModelQueryContext,
    root: MatchBindingHandle,
    order: list[MatchOrderHandle],
    offset: int,
    limit: int,
    include_total: bool,
) -> PendingRemoteModelQuery: ...
def query_v2_prepare_remote_model_count(
    query: MatchQueryHandle,
    context: RemoteModelQueryContext,
    root: MatchBindingHandle,
) -> PendingRemoteModelQuery: ...
def query_v2_prepare_remote_model_exists(
    query: MatchQueryHandle,
    context: RemoteModelQueryContext,
    root: MatchBindingHandle,
) -> PendingRemoteModelQuery: ...
def query_v2_prepare_remote_model_reduce(
    query: MatchQueryHandle,
    context: RemoteModelQueryContext,
    root: MatchBindingHandle,
    group: MatchBindingHandle | None,
    reducers: Sequence[Literal["count", "sum", "min", "max", "mean", "median", "std"]],
    inputs: Sequence[MatchFieldHandle | None],
) -> PendingRemoteModelQuery: ...
def query_v2_prepare_remote_model_reduce_by_field(
    query: MatchQueryHandle,
    context: RemoteModelQueryContext,
    root: MatchBindingHandle,
    group: MatchFieldHandle,
    reducers: Sequence[Literal["count", "sum", "min", "max", "mean", "median", "std"]],
    inputs: Sequence[MatchFieldHandle | None],
) -> PendingRemoteModelQuery: ...
def query_v2_prepare_remote_model_reduce_by_fields(
    query: MatchQueryHandle,
    context: RemoteModelQueryContext,
    root: MatchBindingHandle,
    groups: Sequence[MatchFieldHandle],
    reducers: Sequence[Literal["count", "sum", "min", "max", "mean", "median", "std"]],
    inputs: Sequence[MatchFieldHandle | None],
) -> PendingRemoteModelQuery: ...

class MatchRequestError(Exception):
    category: str
    sdk_category: str
    query_category: str | None
    code: str
    message: str
    path: list[dict[str, object]]
    details: dict[str, object]

def revalidate_match_diagnostic(registry: _QueryDescriptorRegistry, diagnostic: str) -> str: ...
def validate_match_order_term_count(actual: int) -> None: ...

@final
class MigrationIdentity:
    def __init__(self, app_label: str, name: str) -> None: ...
    @property
    def app_label(self) -> str: ...
    @property
    def name(self) -> str: ...

@final
class MigrationHistoryEntry:
    @property
    def id(self) -> MigrationIdentity: ...
    @property
    def parents(self) -> list[MigrationIdentity]: ...
    @property
    def manifest_digest(self) -> str: ...
    @property
    def step_count(self) -> int: ...
    @property
    def safety(self) -> str: ...
    @property
    def reversible(self) -> bool: ...

@final
class MigrationPreviewEntry:
    @property
    def id(self) -> MigrationIdentity: ...
    @property
    def safety(self) -> str: ...
    @property
    def step_count(self) -> int: ...
    @property
    def transaction_group_count(self) -> int: ...
    @property
    def backfill_count(self) -> int: ...
    @property
    def reversible(self) -> bool: ...

@final
class MigrationVerificationFinding:
    @property
    def kind(
        self,
    ) -> Literal[
        "applied_ledger",
        "live_semantics",
        "desired_divergence",
        "pending_migrations",
        "capabilities",
    ]: ...
    @property
    def pending(self) -> list[MigrationIdentity]: ...
    @property
    def diagnostic_code(self) -> str | None: ...

@final
class MigrationVerificationReport:
    @property
    def clean(self) -> bool: ...
    @property
    def findings(self) -> list[MigrationVerificationFinding]: ...
    @property
    def applied_frontier(self) -> list[MigrationIdentity]: ...

@final
class MigrationApprovalSet:
    def __len__(self) -> int: ...

class MigrationApprovalBuilder:
    def approve(self, index: int) -> None: ...
    def finish(self) -> MigrationApprovalSet: ...

class MigrationCancellation:
    def __init__(self) -> None: ...
    def cancel(self) -> None: ...
    @property
    def cancelled(self) -> bool: ...

@final
class MigrationExecutionResources:
    def __init__(self, transaction_groups: int, backfill_observations: int) -> None: ...
    @property
    def transaction_groups(self) -> int: ...
    @property
    def backfill_observations(self) -> int: ...

@final
class MigrationPlan:
    @property
    def execution_authorized(self) -> bool: ...
    def execute(self, database: PyRustDatabase, holder: str) -> MigrationExecutionReport: ...
    def execute_controlled(
        self,
        database: PyRustDatabase,
        holder: str,
        *,
        timeout_milliseconds: int | None = ...,
        resources: MigrationExecutionResources | None = ...,
        cancellation: MigrationCancellation | None = ...,
    ) -> MigrationExecutionReport: ...

@final
class MigrationExecutionReport:
    @property
    def direction(self) -> Literal["apply", "rollback"]: ...
    @property
    def status(
        self,
    ) -> Literal["applied", "rolled_back", "retry_safe", "requires_explicit_recovery"]: ...
    @property
    def migration_id(self) -> MigrationIdentity | None: ...
    @property
    def position_kind(
        self,
    ) -> (
        Literal["transaction_group", "backfill_step", "manifest_checkpoint", "rollback_step"] | None
    ): ...
    @property
    def position_ordinal(self) -> int | None: ...
    @property
    def diagnostic_category(self) -> str | None: ...
    @property
    def diagnostic_code(self) -> str | None: ...
    @property
    def backfills(self) -> list[MigrationBackfillObservation]: ...

@final
class MigrationBackfillObservation:
    @property
    def migration_id(self) -> MigrationIdentity: ...
    @property
    def operation_ordinal(self) -> int: ...
    @property
    def manifest_step_index(self) -> int: ...
    @property
    def plan_fingerprint(self) -> str: ...
    @property
    def direction(self) -> Literal["forward", "reverse"]: ...
    @property
    def matched(self) -> int: ...
    @property
    def changed(self) -> int: ...
    @property
    def skipped(self) -> int: ...
    @property
    def transaction_groups(self) -> int: ...

@final
class MigrationPreview:
    @property
    def direction(self) -> Literal["apply", "rollback"]: ...
    @property
    def execution_authorized(self) -> bool: ...
    def migration_count(self) -> int: ...
    def migration(self, index: int) -> MigrationPreviewEntry | None: ...
    def approval_builder(self) -> MigrationApprovalBuilder: ...
    def authorize(self, approvals: MigrationApprovalSet) -> MigrationPlan: ...

@final
class MigrationCatalog:
    def fingerprint_json(self) -> str: ...
    def __len__(self) -> int: ...
    def is_empty(self) -> bool: ...
    def heads(self) -> list[MigrationIdentity]: ...
    def entry(self, index: int) -> MigrationHistoryEntry | None: ...
    def verify(self, database: PyRustDatabase) -> MigrationVerificationReport: ...
    def applied_migrations(self, database: PyRustDatabase) -> list[MigrationIdentity]: ...
    def preview_apply(
        self,
        applied: list[MigrationIdentity],
        targets: list[MigrationIdentity] | None = ...,
    ) -> MigrationPreview: ...
    def preview_rollback(
        self,
        applied: list[MigrationIdentity],
        removals: list[MigrationIdentity],
    ) -> MigrationPreview: ...

def open_migration_catalog(
    schema_authority: bytes,
    history_bundle: bytes,
) -> MigrationCatalog: ...

@final
class PyMigrationStateReader:
    def __new__(cls) -> Never: ...
    def load_applied(self) -> list[dict[str, object]]: ...
    def load_runs(self) -> list[dict[str, object]]: ...

def migration_state_reader(db: PyRustDatabase) -> PyMigrationStateReader: ...
def __getattr__(name: str) -> Any: ...
