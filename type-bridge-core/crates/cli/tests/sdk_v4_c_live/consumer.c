...
int main(void) {
  ...
  const char *database_name = getenv("TYPE_BRIDGE_SDK_V4_DATABASE");
  ...
  CHECK(tb_sdkv4_schema_package_open(&package, &diagnostics));
  ...
  if (type_bridge_migration_plan_execute_with_options(plan, database, view("sdk-v4-c-limit"), &options, &initial_outcome, &diagnostics) == TYPE_BRIDGE_STATUS_OK) fail("zero group limit", TYPE_BRIDGE_STATUS_INVALID_ARGUMENT);
  ...
  preview = preview_apply(catalog, NULL, 0u, ids[0]);
  plan = authorize(preview, 0, approval_code);
  initial_outcome = execute_plan(plan, database, "sdk-v4-c-initial", &diagnostics);
  ...
  plan = authorize(preview, 0, approval_code);
  { type_bridge_migration_execution_outcome_t *outcome = execute_plan(plan, database, "sdk-v4-c-expand", &diagnostics); if (outcome == NULL) fail("expand migration", TYPE_BRIDGE_STATUS_INVALID_ARGUMENT); CHECK(type_bridge_migration_execution_outcome_close(&outcome)); }
  CHECK(type_bridge_migration_plan_close(&plan)); CHECK(type_bridge_migration_plan_close(&preview));
  ...
  if (execute_plan(plan, database, "sdk-v4-c-backfill-conflict", &diagnostics) != NULL) fail("conflicting backfill", TYPE_BRIDGE_STATUS_INVALID_ARGUMENT);
  ...
  forward_outcome = execute_plan(plan, database, "sdk-v4-c-backfill-forward", &diagnostics);
  ...
  plan = authorize(preview, 1, approval_code);
  rollback_outcome = execute_plan(plan, database, "sdk-v4-c-backfill-reverse", &diagnostics);
  ...
  if (scanf("%lu", &remaining_count) != 1) fail("remaining acknowledgement", TYPE_BRIDGE_STATUS_INVALID_ARGUMENT);
  {
    ...
    CHECK(type_bridge_migration_identity_new(view("sdkv4"), view("9999_unknown"), &unknown, &diagnostics));
    remove[0] = unknown;
  ...
  reapply_outcome = execute_plan(reapply_plan, database, "sdk-v4-c-backfill-reapply", &diagnostics);
