//! Exact-live generated Python Workforce V4 report evidence.
...
const CONSUMER: &str = include_str!("workforce_v4_python_live/consumer.py");
...
fn generated_python_emits_complete_v4_report_on_3_12_3() {
    ...
    fs::rename(
        ...
        temporary.path().join("workforce_v4_generated"),
    )
    ...
    let database = format!("workforce_v4_python_{}", std::process::id());
    ...
    let report_path = temporary.path().join("python-workforce-v4-report.json");
    ...
    if let Some(directory) = std::env::var_os("TYPE_BRIDGE_WORKFORCE_V4_REPORT_DIR") {
        fs::copy(
            &report_path,
            PathBuf::from(directory).join("python-workforce-v4-report.json"),
        )
