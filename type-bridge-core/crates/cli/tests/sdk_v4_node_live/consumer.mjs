...
const databaseName = process.env.TYPE_BRIDGE_SDK_V4_DATABASE;
...
let resourceCode;
try {
  limitedPlan.executeControlled(database, "sdk-v4-node-limit", {
...
const initialReport = initialPlan.execute(database, "sdk-v4-node-initial");
...
expandPlan.execute(database, "sdk-v4-node-expand");
...
let conflictCode;
try {
  backfillPlan.execute(database, "sdk-v4-node-backfill-conflict");
...
const forwardReport = backfillPlan.execute(database, "sdk-v4-node-backfill-forward");
...
const rollbackReport = rollbackPlan.execute(database, "sdk-v4-node-backfill-reverse");
...
let unknownCode;
try {
  catalog.previewRollback(appliedAfterRollback, [{ appLabel: "sdkv4", name: "9999_unknown" }]);
...
const reapplyReport = backfillPlan.execute(database, "sdk-v4-node-backfill-reapply");
