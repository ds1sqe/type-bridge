//! Exact-live internal C ABI 1.6 Sdk V4 report evidence.
...
const CONSUMER: &str = include_str!("sdk_v4_c_live/consumer.c");
...
async fn generated_c_emits_complete_v4_report_on_3_12_3() {
    ...
    run(
        Command::new("cargo")
            .current_dir(&core)
            .args(["build", "-p", "type-bridge-c"]),
        "ABI 1.6 shared library build",
    );
    let generated = temporary.path().join("generated/c");
    let executable = temporary.path().join("sdk-v4-c-consumer");
    ...
    let database = format!("sdk_v4_c_{}", std::process::id());
    ...
    let report_path = temporary.path().join("c-sdk-v4-report.json");
    ...
    if let Some(directory) = std::env::var_os("TYPE_BRIDGE_SDK_V4_REPORT_DIR") {
        fs::copy(
            &report_path,
            PathBuf::from(directory).join("c-sdk-v4-report.json"),
        )
