...
async fn main() {
    let database_name =
        env::var("TYPE_BRIDGE_SDK_V4_DATABASE").expect("isolated database name is provided");
    ...
    assert_eq!(
        database
            ...
            .await
            .expect("initial pair inspection"),
        type_bridge::ManagedDatabasePairState::Absent,
    );
    let create = database
        .create_database()
        .await
        .expect("managed database creates");
    let repeat_create = database
        .create_database()
        .await
        .expect("create is normalized");
    let pair_state = database
        .inspect_database_pair()
        .await
        .expect("pair inspects");
    ...
    let limited = plan
        .execute_controlled(&database, "sdk-v4-rust-limit", &limited_control)
        .await
    ...
    let initial_report = initial_plan
        .execute(&database, "sdk-v4-rust-initial")
        .await
        .expect("initial schema applies");

    let expand_preview = catalog
        .preview_apply(vec![initial_id.clone()], Some(vec![expand_id.clone()]))
    ...
    expand_plan
        .execute(&database, "sdk-v4-rust-expand")
        .await
    ...
    let backfill_plan = backfill_preview
        .authorize(
            &backfill_approvals
                .finish()
                .expect("backfill approvals freeze"),
        )
        .expect("backfill plan authorizes");
    let conflict = backfill_plan
        .execute(&database, "sdk-v4-rust-backfill-conflict")
        .await
    ...
    let backfill_report = backfill_plan
        .execute(&database, "sdk-v4-rust-backfill-forward")
        .await
    ...
    assert!(
        retry_preview.is_empty(),
        "completed backfill plans no repeat work"
    );
    let retry_changed = 0;
    ...
    let rollback_plan = rollback_preview
        .authorize(
            &rollback_approvals
                .finish()
                .expect("rollback approvals freeze"),
        )
        .expect("rollback plan authorizes");
    let rollback_report = rollback_plan
        .execute(&database, "sdk-v4-rust-backfill-reverse")
        .await
    ...
    let reapply_report = backfill_plan
        .execute(&database, "sdk-v4-rust-backfill-reapply")
        .await
