//! Exact-live generated Node Workforce V4 report evidence.
...
const CONSUMER: &str = include_str!("workforce_v4_node_live/consumer.mjs");
...
fn generated_node_emits_complete_v4_report_on_3_12_3() {
    ...
    let database = format!("workforce_v4_node_{}", std::process::id());
    ...
    let report_path = temporary.path().join("node-workforce-v4-report.json");
    ...
    if let Some(directory) = std::env::var_os("TYPE_BRIDGE_WORKFORCE_V4_REPORT_DIR") {
        fs::copy(
            &report_path,
            PathBuf::from(directory).join("node-workforce-v4-report.json"),
        )
