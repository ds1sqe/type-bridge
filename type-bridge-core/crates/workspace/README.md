...
```toml
[dependencies]
type-bridge-workspace = "2.2.0"
```

Begin with `TypeBridgeConfigSpec` and `TypeBridgeWorkspace` in the
[crate API](https://docs.rs/type-bridge-workspace/2.2.0). Provide explicit
...
2.2.0 and requires Rust 1.88+. Each workspace selects one frozen
...
[API documentation](https://docs.rs/type-bridge-workspace/2.2.0) ·
