...
const CONSUMER: &str = include_str!("../../schema-codegen/tests/c_phase2_parity/consumer.c");
const SEMANTIC_PROFILE: &str = "typedb-3.12.1/v1";
const REPORT_FORMAT: &str = "typebridge.phase2-projected-parity-report/v1";
const FACT_PREFIX: &str = "TYPE_BRIDGE_C_PHASE2_FACT\t";
const REPORT_ENV: &str = "TYPE_BRIDGE_PHASE2_PARITY_REPORT_C";
...
impl Stage {
    fn new() -> Self {
        ...
        let path = env::temp_dir().join(format!(
            "typebridge-c-phase2-parity-{}-{sequence}",
            std::process::id(),
        ));
        fs::create_dir(&path).expect("unique C Phase-2 parity stage creates");
        Self(path)
    }
...
impl Drop for Stage {
    fn drop(&mut self) {
        fs::remove_dir_all(&self.0).expect("C Phase-2 parity stage removes");
    }
}
...
fn authority_for(
    ...
    let context = ManagedDeltaContext::new(
        ManagedScopeId::new(scope).expect("Phase-2 managed scope is valid"),
        profile,
        available,
    );
...
fn emitted_fixture() -> EmittedFixture {
    ...
    assert_ne!(foreign_source, SOURCE, "foreign Phase-2 schema must differ");
    let (local_resolved, local_authority) =
        authority_for(SOURCE, "workforce-v3.yaml", "workforce-v3-c-phase2");
    let (foreign_resolved, foreign_authority) = authority_for(
        &foreign_source,
        "workforce-v3-foreign.yaml",
        "workforce-v3-c-phase2-foreign",
    );
    ...
    let (local_projection, local) = emit(&local_resolved, &local_authority, "phase2");
    let (foreign_projection, foreign) =
        emit(&foreign_resolved, &foreign_authority, "phase2_foreign");
    assert_ne!(
...
fn assert_projection_contract(projection: &RuntimeProjection, package: &GeneratedPackage) {
    ...
    let header = std::str::from_utf8(
        package
            .get("include/phase2/models.h")
            .expect("Phase-2 generated header exists"),
    )
    .expect("Phase-2 generated header is UTF-8");
    assert!(!header.contains("phase2_actor_create_open("));
    let person_args = header
        .split_once("typedef struct phase2_person_create_args_v1_t {")
        .and_then(|(_, tail)| tail.split_once("} phase2_person_create_args_v1_t;"))
        .map(|(body, _)| body)
    ...
    assert!(!header.contains("phase2_plainzhactivity_participant_player_from_robot("));
    assert!(!header.contains("phase2_event_subject_player_from_robot("));
    let employment_args = header
        .split_once("typedef struct phase2_employment_create_args_v1_t {")
        .and_then(|(_, tail)| tail.split_once("} phase2_employment_create_args_v1_t;"))
        .map(|(body, _)| body)
...
fn render_consumer(fixture: &EmittedFixture) -> String {
    ...
    for (placeholder, kind, label) in [
        ...
        replacements.insert(
            placeholder,
            token_symbol("phase2", "model", model_ordinal(local, kind, label)),
        );
    }
    ...
    for (placeholder, owner, role) in [
        ...
        replacements.insert(
            placeholder,
            token_symbol("phase2", "role", role_ordinal(local, owner, role)),
        );
    }
    ...
    assert!(
        ...
        "C Phase-2 consumer retained an unresolved token placeholder",
    );
    source
}
...
fn native_library() -> PathBuf {
    ...
    [dependencies.join(&filename), profile.join(filename)]
        ...
        .expect("build type-bridge-c's shared library before the Phase-2 producer")
}
...
fn parse_observations(stdout: &[u8]) -> BTreeMap<String, Value> {
    let text = std::str::from_utf8(stdout).expect("C Phase-2 facts are UTF-8");
    ...
    for line in text.lines() {
        ...
        let (name, raw) = payload
            .split_once('\t')
            .expect("C Phase-2 fact has a name and observation");
        assert!(
            OBSERVATIONS.contains(&name),
            "unexpected C Phase-2 fact {name}"
        );
        let value: Value = serde_json::from_str(raw).expect("C Phase-2 fact is valid JSON");
        assert!(value.is_object(), "C Phase-2 fact must be an object");
        assert_eq!(
            to_canonical_json(&value).expect("C Phase-2 fact canonicalizes"),
            raw.as_bytes(),
            "C Phase-2 fact {name} is not canonical JSON",
        );
        assert!(
            observations.insert(name.to_owned(), value).is_none(),
            "C Phase-2 fact {name} was duplicated",
        );
    }
    assert_eq!(
        observations.keys().map(String::as_str).collect::<Vec<_>>(),
        OBSERVATIONS,
        "C Phase-2 fact ledger is incomplete",
    );
    observations
}
...
fn regular_source_identity(root: &Path, relative: &str) -> Value {
    let path = root.join(relative);
    let metadata = fs::symlink_metadata(&path).expect("Phase-2 authority is inspectable");
    assert!(
        metadata.is_file() && !metadata.file_type().is_symlink(),
        "Phase-2 authority must be a regular non-symlink file",
    );
    assert!(
        metadata.len() <= MAX_AUTHORITY_BYTES,
        "Phase-2 authority exceeds its byte limit",
    );
    let bytes = fs::read(&path).expect("Phase-2 authority is readable");
...
fn canonical_report(report: &Value) -> Vec<u8> {
    let mut bytes = to_canonical_json(report).expect("C Phase-2 report canonicalizes");
    bytes.push(b'\n');
    assert!(
        bytes.len() <= MAX_REPORT_BYTES,
        "C Phase-2 report exceeds its byte limit",
    );
    bytes
}
...
fn publish_report(path: &Path, report: &Value) {
    ...
    let parent = path.parent().expect("C Phase-2 report path has a parent");
    ...
    let mut file = OpenOptions::new()
        ...
        .create_new(true)
        .open(path)
        .expect("C Phase-2 report creates without replacement");
    file.write_all(&bytes).expect("C Phase-2 report writes");
    file.sync_all().expect("C Phase-2 report synchronizes");
}

fn validate_report(root: &Path, path: &Path) {
    let script = root.join("scripts/ci/compare_phase2_projection_parity.py");
    let program = concat!(
        "import importlib.util,pathlib,sys;",
        "p=pathlib.Path(sys.argv[1]);",
        "s=importlib.util.spec_from_file_location('phase2_comparator',p);",
    ...
    let output = Command::new("python3")
        ...
        .current_dir(root)
        .output()
        .expect("committed Phase-2 comparator launches");
    assert!(
        output.status.success(),
        "committed Phase-2 comparator rejected C report:\nstdout:\n{}\nstderr:\n{}",
...
#[test]
fn generated_c17_phase2_parity_producer_is_exact_and_comparator_accepted() {
    ...
    let consumer = stage.path().join("phase2-parity-consumer.c");
    fs::write(&consumer, render_consumer(&fixture)).expect("C Phase-2 consumer writes");
    ...
    for compiler in ["gcc", "clang"] {
        ...
        compiler_count += 1;
        let executable = stage.path().join(format!("phase2-parity-{compiler}"));
        ...
        assert!(
            output.status.success(),
            "{compiler} rejected the strict C17 Phase-2 producer:\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr),
        );
        ...
        assert!(
            output.status.success(),
            "{compiler}'s C Phase-2 producer failed with {}:\nstdout:\n{}\nstderr:\n{}",
            output.status,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr),
        );
    ...
    let observations = parse_observations(
        canonical_stdout
            .as_deref()
            .expect("one compiler emitted C Phase-2 facts"),
    );
    let report = build_report(&root, observations);
    let requested = env::var_os(REPORT_ENV).map(PathBuf::from);
    let output = requested.unwrap_or_else(|| stage.path().join("c-phase2-report.json"));
