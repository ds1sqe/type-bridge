#!/usr/bin/env python3
"""Produce Python's provider-free Phase-2 projected parity report."""
...
OUTPUT_ENV = "TYPE_BRIDGE_PHASE2_PARITY_REPORT"
    ...
    return _canonical(
        {
            ...
            "executor": {
                "epoch": "python-phase2-epoch-0001",
                "identity": "python-phase2-executor",
            },
...
def _wire_scalar(kind: str, value: object) -> dict[str, object]:
    ...
    if kind == "datetime_tz" and type(value) is datetime and value.tzinfo is not None:
        ...
        if offset != timedelta(0):
            raise AssertionError("Phase-2 fixture requires the authored UTC datetime")
        return _value(
            kind,
            {
    ...
    if kind == "duration" and type(value) is timedelta:
        if value.microseconds != 0 or value.days != 0 or value.seconds != 38:
            raise AssertionError("Phase-2 fixture requires its authored exact duration")
...
def _build_report(root: Path, package: Any, foreign: Any) -> dict[str, object]:
    ...
    provider_text_exposed = "phase2-provider-secret" in diagnostic_text
    ...
    return {
        "authority": {
            "journey": _source_identity(root, JOURNEY_RELATIVE, "Phase-2 journey"),
            "schema": _source_identity(root, SCHEMA_RELATIVE, "Phase-2 schema"),
        },
        "binding": "python",
...
def _load_packages() -> tuple[Any, Any]:
    ...
    try:
        import generated_phase2
        import generated_phase2_foreign
    except ImportError as error:
        raise ProducerError(
            "generated_package_import", "Phase-2 generated packages cannot be imported"
        ) from error
    return generated_phase2, generated_phase2_foreign
...
def main() -> int:
    try:
        ...
        print(f"Python Phase-2 parity producer rejected: {error}", file=sys.stderr)
        return 1
    return 0
