#!/usr/bin/env node
/** Produce Node's provider-free Projected projected parity report. */
...
export const REPORT_FORMAT = "typebridge.projected-parity-report/v1";
export const SEMANTIC_PROFILE = "typedb-3.12.1/v1";
export const OUTPUT_ENV = "TYPE_BRIDGE_PROJECTED_PARITY_REPORT";
...
function remoteAdvertisement() {
  return Buffer.from(
    canonicalString({
      ...
      executor: {
        epoch: "node-projected-epoch-0001",
        identity: "node-projected-executor",
      },
...
async function buildReport(root, package_, foreign) {
  ...
  const providerTextExposed = diagnosticText.includes("projected-provider-secret");
  requireCondition(
    !providerTextExposed,
  ...
  return {
    authority: {
      journey: sourceIdentity(root, JOURNEY_RELATIVE, "Projected journey"),
      schema: sourceIdentity(root, SCHEMA_RELATIVE, "Projected schema"),
    },
    binding: "node",
...
export async function main() {
  try {
    const [package_, foreign] = await Promise.all([
      import("./generated_projected/dist/index.js"),
      import("./generated_projected_foreign/dist/index.js"),
    ]);
    ...
    process.stderr.write(
      `Node Projected parity producer rejected: ${error instanceof Error ? error.message : String(error)}\n`,
    );
    return 1;
  }
}
