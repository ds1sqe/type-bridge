...
export const REPORT_FORMAT = "typebridge.phase5-manager-filter-live-report/v1";
...
export const OUTPUT_ENV = "TYPE_BRIDGE_PHASE5_MANAGER_LIVE_REPORT";
...
export const FOREIGN_PACKAGE = "generated_phase5_manager_foreign";
  ...
  for (const label of ["schema", "journey", "provider"]) {
    ...
    if (digest !== SOURCE_SHA256[label]) {
      throw new ProducerError(
        ...
        `Phase-5 ${label} is not the frozen exact-3.12.1 authority`,
      );
    }
...
function validateOutputTarget(path) {
  ...
  throw new ProducerError("output_exists", "report destination already exists");
}
...
function runManagerJourney(package_, database) {
  ...
  try {
    ...
    try {
      ...
      requireCondition(
        plan04.countBy(person) === 1n,
        "borrowed_read_reuse",
        "Plan04 sibling query did not remain usable",
      );
    } finally {
      session.close();
    }
...
export async function main(environment = process.env) {
  try {
    ...
    const records = personRecords(
      jsonObject(sources.journey, "Phase-5 journey"),
    );
    ...
    console.error(
      `Node Phase-5 manager-filter producer rejected: ${
