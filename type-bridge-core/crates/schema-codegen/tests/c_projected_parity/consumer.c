...
#define CHECK(condition)                                                       \
  do {                                                                         \
    if (!(condition)) {                                                        \
      fprintf(stderr, "C Projected parity evidence failed at line %d\n",       \
              __LINE__);                                                       \
      return __LINE__;                                                         \
    }                                                                          \
  } while (0)
...
#define REQUIRE(condition)                                                     \
  do {                                                                         \
    if (!(condition)) {                                                        \
      fprintf(stderr, "C Projected evidence requirement failed at line %d\n",  \
              __LINE__);                                                       \
      return 0;                                                                \
    }                                                                          \
  } while (0)
...
typedef struct person_values {
  projected_aliases *analyst;
  projected_aliases *mathematician;
...
static int open_person_values(const type_bridge_schema_package_t *package,
  ...
  REQUIRE(projected_aliases_open(package, VIEW("analyst"), &values->analyst,
                              diagnostics) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_aliases_open(package, VIEW("mathematician"),
                              &values->mathematician, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_foozuzubar_open(package, 7, &values->foo_bar, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_identifier_open(package, VIEW("data-ada"),
                                 &values->identifier, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_nickname_open(package, VIEW("Ada"), &values->nickname,
                               diagnostics) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_score_open(package, 38, &values->score, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_scorezuzugte_open(package, 40, &values->score_gte,
                                   diagnostics) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_valzubool_open(package, 0u, &values->boolean_value,
                                diagnostics) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_valzuconstrained_open(package, 38, &values->constrained,
                                       diagnostics) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_valzudate_open(package, VIEW("2026-08-12"), &values->date,
                                diagnostics) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_valzudatetime_open(package, VIEW("2026-08-12T09:30:00"),
                                    &values->datetime, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_valzudatetimezutzz_open(
              package, VIEW("2026-08-12T09:30:00Z"), &values->datetime_tz,
              diagnostics) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_valzudecimal_open(package, VIEW("38.5"), &values->decimal,
                                   diagnostics) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_valzudouble_open(package, UINT64_C(0x4043000000000000),
                                  &values->double_value, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_valzuduration_open(package, VIEW("PT38S"), &values->duration,
                                    diagnostics) == TYPE_BRIDGE_STATUS_OK);
  return *diagnostics == NULL;
}
...
static int close_person_values(person_values_t *values) {
  return projected_valzuduration_close(&values->duration) ==
         ...
         projected_valzudouble_close(&values->double_value) ==
             TYPE_BRIDGE_STATUS_OK &&
         projected_valzudecimal_close(&values->decimal) == TYPE_BRIDGE_STATUS_OK &&
         ...
         projected_aliases_close(&values->mathematician) == TYPE_BRIDGE_STATUS_OK &&
         projected_aliases_close(&values->analyst) == TYPE_BRIDGE_STATUS_OK;
}
...
static int create_text_is(const projected_person_create *create,
...
static int create_long_is(const projected_person_create *create,
                          const type_bridge_projected_token_v1_t *field,
                          int64_t expected) {
  type_bridge_projected_value_t *value = NULL;
  type_bridge_execution_diagnostics_t *diagnostics = NULL;
...
static int create_scalar_values_are_exact(const projected_person_create *create) {
  type_bridge_projected_value_t *value = NULL;
  type_bridge_execution_diagnostics_t *diagnostics = NULL;
...
static int hydrated_scalar_values_are_exact(const projected_person *person) {
  type_bridge_execution_diagnostics_t *diagnostics = NULL;
  projected_aliases *alias = NULL;
  projected_foozuzubar *foo_bar = NULL;
  projected_identifier *identifier = NULL;
  projected_nickname *nickname = NULL;
  projected_score *score = NULL;
  projected_scorezuzugte *score_gte = NULL;
  projected_valzubool *boolean_value = NULL;
  projected_valzuconstrained *constrained = NULL;
  ...
  REQUIRE(projected_person_aliases_count(person, &count, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          count == 2u && diagnostics == NULL);
  REQUIRE(projected_person_aliases_at(person, 0u, &alias, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_aliases_value(alias, &text, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          same_view(text, VIEW("analyst")) &&
          projected_aliases_close(&alias) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_aliases_at(person, 1u, &alias, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_aliases_value(alias, &text, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          same_view(text, VIEW("mathematician")) &&
          projected_aliases_close(&alias) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_foozuzubar(person, &foo_bar, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_foozuzubar_value(foo_bar, &integer, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          integer == 7 &&
          projected_foozuzubar_close(&foo_bar) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_identifier(person, &identifier, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_identifier_value(identifier, &text, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          same_view(text, VIEW("data-ada")) &&
          projected_identifier_close(&identifier) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_nickname(person, &nickname, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_nickname_value(nickname, &text, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          same_view(text, VIEW("Ada")) &&
          projected_nickname_close(&nickname) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_score(person, &score, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_score_value(score, &integer, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          integer == 38 &&
          projected_score_close(&score) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_scorezuzugte(person, &score_gte, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_scorezuzugte_value(score_gte, &integer, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          integer == 40 &&
          projected_scorezuzugte_close(&score_gte) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_valzubool(person, &boolean_value, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_valzubool_value(boolean_value, &boolean, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          boolean == 0u &&
          projected_valzubool_close(&boolean_value) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_valzuconstrained(person, &constrained, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_valzuconstrained_value(constrained, &integer, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          integer == 38 &&
          projected_valzuconstrained_close(&constrained) ==
              TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_valzudate(person, &date, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_valzudate_value(date, &text, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          same_view(text, VIEW("2026-08-12")) &&
          projected_valzudate_close(&date) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_valzudatetime(person, &datetime, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
  ...
  REQUIRE(projected_person_valzudatetimezutzz(person, &datetime_tz,
                                           &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_valzudatetimezutzz_value(datetime_tz, &text, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          same_view(text, VIEW("2026-08-12T09:30:00Z")) &&
          projected_valzudatetimezutzz_close(&datetime_tz) ==
              TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_valzudecimal(person, &decimal, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
  ...
  REQUIRE(projected_person_valzudouble(person, &double_value, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
  ...
  REQUIRE(projected_person_valzuduration(person, &duration, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
...
static int run_constraint_rejections(
  ...
  projected_nickname *nickname = NULL;
  projected_valzuconstrained *too_large = NULL;
  projected_aliases *fourth_alias = NULL;
  ...
  nickname = (projected_nickname *)(uintptr_t)1u;
  {
    type_bridge_status_t status = projected_nickname_open(
  ...
  nickname = (projected_nickname *)(uintptr_t)1u;
  REQUIRE(projected_nickname_open(package, VIEW("ada"), &nickname,
                               &diagnostics) ==
              TYPE_BRIDGE_STATUS_INVALID_ARGUMENT &&
          nickname == NULL &&
          diagnostic_is(diagnostics,
          ...
          close_diagnostics(&diagnostics));
  too_large = (projected_valzuconstrained *)(uintptr_t)1u;
  REQUIRE(projected_valzuconstrained_open(package, 81, &too_large,
                                       &diagnostics) ==
              TYPE_BRIDGE_STATUS_INVALID_ARGUMENT &&
  ...
  REQUIRE(projected_aliases_open(package, VIEW("programmer"), &fourth_alias,
                              &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
  ...
  REQUIRE(type_bridge_projected_create_open_v1(package, &descriptor, &create,
          ...
          diagnostic_is(diagnostics,
                        ...
                        "field_cardinality_violation") &&
          close_diagnostics(&diagnostics));
  REQUIRE(projected_aliases_close(&fourth_alias) == TYPE_BRIDGE_STATUS_OK);
...
static int person_ref_key_is(
    const projected_person_ref *person, type_bridge_byte_view_t expected,
    type_bridge_execution_diagnostics_t **diagnostics) {
  projected_identifier *key = NULL;
  type_bridge_byte_view_t actual = {NULL, 0u};
  int matched = projected_person_ref_identifier_key(person, &key, diagnostics) ==
                    TYPE_BRIDGE_STATUS_OK &&
                projected_identifier_value(key, &actual, diagnostics) ==
                    TYPE_BRIDGE_STATUS_OK &&
                same_view(actual, expected);
  if (key != NULL &&
      projected_identifier_close(&key) != TYPE_BRIDGE_STATUS_OK) {
    return 0;
  }
  return matched;
}
...
static int interaction_hydration_is_exact(
    const type_bridge_schema_package_t *package,
    const projected_identifier *identifier,
  ...
  if (type_bridge_projected_thing_open_v1(package, &descriptor, &thing,
      ...
      projected_interaction_actor((const projected_interaction *)thing, &actor,
                               &diagnostics) != TYPE_BRIDGE_STATUS_OK) {
    goto close;
  }
  if (expected_actor_kind == 0) {
    ...
    matched = actor != NULL &&
              projected_interaction_actor_player_as_person(
                  actor, &person, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
              person_ref_key_is(person, VIEW("data-ada"), &diagnostics);
    if (person != NULL &&
        projected_person_ref_close(&person) != TYPE_BRIDGE_STATUS_OK) {
      matched = 0;
    }
    ...
    int64_t value = 0;
    matched = actor != NULL &&
              projected_interaction_actor_player_as_robot(
              ...
              projected_robotzuid_value(key, &value, &diagnostics) ==
                  TYPE_BRIDGE_STATUS_OK &&
              value == 7;
    if (key != NULL && projected_robotzuid_close(&key) != TYPE_BRIDGE_STATUS_OK) {
      matched = 0;
    }
    if (robot != NULL &&
        projected_robot_ref_close(&robot) != TYPE_BRIDGE_STATUS_OK) {
      matched = 0;
    }
  }
...
close:
  if (actor != NULL &&
      projected_interaction_actor_player_close(&actor) != TYPE_BRIDGE_STATUS_OK) {
    matched = 0;
  }
  if (thing != NULL &&
...
static int run_role_and_package_evidence(
    ...
    const projected_person *local_person, const person_values_t *person_values) {
  type_bridge_execution_diagnostics_t *diagnostics = NULL;
  projected_person_ref *ada = NULL;
  projected_person_ref *dana = NULL;
  projected_identifier *dana_key = NULL;
  projected_robotzuid *robot_key = NULL;
  projected_robotzuid *negative_robot_key = NULL;
  projected_robot_ref *robot = NULL;
  projected_robot_ref *negative_robot = NULL;
  ...
  projected_networkzhlink_participant_player *ada_participant = NULL;
  projected_networkzhlink_participant_player *dana_participant = NULL;
  projected_networkzhlink_create *network_create = NULL;
  type_bridge_projected_thing_t *network_thing = NULL;
  projected_interaction_target_player *target = NULL;
  projected_interaction_actor_player *person_actor = NULL;
  projected_interaction_actor_player *robot_actor = NULL;
  projected_interaction_create *interaction_create = NULL;
  projected_event_ref *event = NULL;
  ...
  projected_identifier *interaction_person_identifier = NULL;
  projected_identifier *interaction_robot_identifier = NULL;
  projected_identifier *interaction_absent_identifier = NULL;
  projected_identifier *network_identifier = NULL;
  ...
  REQUIRE(projected_identifier_open(package, VIEW("data-dana"), &dana_key,
                                 &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          open_full_reference(
              package, &LOCAL_MODEL_PERSON, &LOCAL_FIELD_PERSON_IDENTIFIER,
              ...
              TYPE_BRIDGE_STATUS_OK);
  {
    projected_identifier *key = NULL;
    type_bridge_byte_view_t text = {NULL, 0u};
    REQUIRE(projected_person_ref_identifier_key(ada, &key, &diagnostics) ==
                TYPE_BRIDGE_STATUS_OK &&
            projected_identifier_value(key, &text, &diagnostics) ==
                TYPE_BRIDGE_STATUS_OK &&
            same_view(text, VIEW("data-ada")) &&
            projected_identifier_close(&key) == TYPE_BRIDGE_STATUS_OK);
    REQUIRE(projected_person_ref_identifier_key(dana, &key, &diagnostics) ==
                TYPE_BRIDGE_STATUS_OK &&
            projected_identifier_value(key, &text, &diagnostics) ==
                TYPE_BRIDGE_STATUS_OK &&
  ...
  REQUIRE(projected_robotzuid_open(package, 7, &robot_key, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          open_full_reference(
              ...
              TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_robotzuid_open(package, -7, &negative_robot_key,
                                &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          open_full_reference(
              package, &LOCAL_MODEL_ROBOT, &LOCAL_FIELD_ROBOT_ID,
  ...
  REQUIRE(projected_plainzhactivity_participant_player_from_person(
              ada, &plain_player, &diagnostics) == TYPE_BRIDGE_STATUS_OK);
  {
    ...
    REQUIRE(projected_plainzhactivity_create_open(
  ...
  }
  {
    ...
    projected_person_ref *person = NULL;
    REQUIRE(projected_plainzhactivity_participant(
                (const projected_plainzhactivity *)plain_thing, &hydrated,
                &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
  ...
  REQUIRE(projected_networkzhlink_participant_player_from_person(
              ada, &ada_participant, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          projected_networkzhlink_participant_player_from_person(
              dana, &dana_participant, &diagnostics) == TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_identifier_open(package, VIEW("data-link-forward"),
                                 &network_identifier, &diagnostics) ==
          TYPE_BRIDGE_STATUS_OK);
  {
    const projected_networkzhlink_participant_player *players[] = {
        ada_participant, dana_participant};
    projected_networkzhlink_create_role_participant_chunks_v1_t chunk = {0};
    projected_networkzhlink_create_args_v1_t args = {0};
    projected_networkzhlink_origin_player *origin = NULL;
    projected_networkzhlink_destination_player *destination = NULL;
    REQUIRE(projected_networkzhlink_origin_player_from_person(
                ada, &origin, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
            projected_networkzhlink_destination_player_from_person(
    ...
    REQUIRE(projected_networkzhlink_create_open(
            ...
            projected_networkzhlink_destination_player_close(&destination) ==
                TYPE_BRIDGE_STATUS_OK &&
            projected_networkzhlink_origin_player_close(&origin) ==
                TYPE_BRIDGE_STATUS_OK);
  }
  {
    const type_bridge_projected_value_t *identifier_values[] = {
  ...
  {
    size_t count = 0u;
    projected_networkzhlink_participant_player *player = NULL;
    projected_person_ref *person = NULL;
    REQUIRE(projected_networkzhlink_participant_count(
                (const projected_networkzhlink *)network_thing, &count,
                &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
            count == 2u);
    REQUIRE(projected_networkzhlink_participant_at(
                (const projected_networkzhlink *)network_thing, 0u, &player,
                &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
            projected_networkzhlink_participant_player_as_person(
                player, &person, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
            person_ref_key_is(person, VIEW("data-ada"), &diagnostics) &&
            projected_person_ref_close(&person) == TYPE_BRIDGE_STATUS_OK &&
            projected_networkzhlink_participant_player_close(&player) ==
                TYPE_BRIDGE_STATUS_OK);
    REQUIRE(projected_networkzhlink_participant_at(
                (const projected_networkzhlink *)network_thing, 1u, &player,
                &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
            projected_networkzhlink_participant_player_as_person(
                player, &person, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
  ...
  {
    const projected_networkzhlink_participant_player *duplicates[] = {
        ada_participant, ada_participant};
    projected_networkzhlink_create_role_participant_chunks_v1_t chunk = {0};
    projected_networkzhlink_create_args_v1_t args = {0};
    projected_networkzhlink_create *duplicate =
        (projected_networkzhlink_create *)(uintptr_t)1u;
    projected_networkzhlink_origin_player *origin = NULL;
    projected_networkzhlink_destination_player *destination = NULL;
    REQUIRE(projected_networkzhlink_origin_player_from_person(
                ada, &origin, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
  ...
  REQUIRE(projected_interaction_target_player_from_person(
          ...
          projected_identifier_open(package, VIEW("interaction-person"),
                                 &interaction_person_identifier,
                                 &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          projected_identifier_open(package, VIEW("interaction-robot"),
                                 &interaction_robot_identifier,
                                 &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          projected_identifier_open(package, VIEW("interaction-absent"),
              ...
              TYPE_BRIDGE_STATUS_OK);
  {
    ...
    REQUIRE(projected_interaction_create_open(
            ...
            create_role_string_key_is(
                ...
                &LOCAL_FIELD_PERSON_IDENTIFIER, VIEW("data-ada")));
    REQUIRE(projected_interaction_create_close(&interaction_create) ==
            TYPE_BRIDGE_STATUS_OK);
  ...
  REQUIRE(projected_event_ref_from_iid(package, VIEW("0x50"), &event,
                                    &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          projected_container_item_player_from_event(
              event, &event_item, &diagnostics) == TYPE_BRIDGE_STATUS_OK);
  {
    ...
    REQUIRE(projected_container_create_open(
    ...
    {
      const projected_container_item_player *too_many[] = {event_item, event_item,
                                                         event_item};
      projected_container_create *rejected =
          (projected_container_create *)(uintptr_t)1u;
  ...
  }
  {
    ...
    type_bridge_projected_thing_descriptor_v1_t descriptor = {0};
    size_t count = 0u;
    projected_container_item_player *item = NULL;
    ...
    REQUIRE(type_bridge_projected_thing_open_v1(
                ...
                TYPE_BRIDGE_STATUS_OK &&
            projected_container_item_count(
                (const projected_container *)container_thing, &count,
                &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
            count == 1u &&
            projected_container_item_at(
                (const projected_container *)container_thing, 0u, &item,
                &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
            ...
            projected_event_ref_close(&hydrated_event) == TYPE_BRIDGE_STATUS_OK &&
            projected_container_item_player_close(&item) ==
                TYPE_BRIDGE_STATUS_OK);
  }

  {
    projected_foreign_identifier *foreign_key = NULL;
    projected_foreign_person_ref *foreign_person = NULL;
    projected_plainzhactivity_participant_player *rejected =
        (projected_plainzhactivity_participant_player *)(uintptr_t)1u;
    size_t count = SIZE_MAX;
    REQUIRE(projected_foreign_identifier_open(
                foreign_package, VIEW("foreign-person"), &foreign_key,
                &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
            projected_foreign_person_ref_from_key(
                foreign_package, foreign_key, &foreign_person,
                &diagnostics) == TYPE_BRIDGE_STATUS_OK);
    REQUIRE(projected_plainzhactivity_participant_player_from_person(
                (const projected_person_ref *)foreign_person, &rejected,
            ...
            close_diagnostics(&diagnostics));
    REQUIRE(projected_foreign_person_aliases_count(
                (const projected_foreign_person *)local_person, &count,
                &diagnostics) == TYPE_BRIDGE_STATUS_EXECUTION_FAILED &&
            count == 0u &&
            diagnostic_is(diagnostics,
                          TYPE_BRIDGE_EXECUTION_DIAGNOSTIC_INTEGRITY,
                          "generated_token_package_mismatch") &&
            close_diagnostics(&diagnostics));
    REQUIRE(projected_foreign_person_ref_close(&foreign_person) ==
                TYPE_BRIDGE_STATUS_OK &&
            projected_foreign_identifier_close(&foreign_key) ==
                TYPE_BRIDGE_STATUS_OK);
  }

  REQUIRE(projected_container_create_close(&container_create) ==
              TYPE_BRIDGE_STATUS_OK &&
          type_bridge_projected_thing_close(&container_thing) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_container_item_player_close(&event_item) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_event_ref_close(&event) == TYPE_BRIDGE_STATUS_OK &&
          projected_interaction_create_close(&interaction_create) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_identifier_close(&interaction_absent_identifier) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_identifier_close(&interaction_robot_identifier) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_identifier_close(&interaction_person_identifier) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_interaction_actor_player_close(&robot_actor) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_interaction_actor_player_close(&person_actor) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_interaction_target_player_close(&target) ==
              TYPE_BRIDGE_STATUS_OK &&
          type_bridge_projected_thing_close(&network_thing) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_networkzhlink_create_close(&network_create) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_identifier_close(&network_identifier) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_networkzhlink_participant_player_close(&dana_participant) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_networkzhlink_participant_player_close(&ada_participant) ==
              TYPE_BRIDGE_STATUS_OK &&
          ...
          projected_plainzhactivity_participant_player_close(&plain_player) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_robot_ref_close(&negative_robot) == TYPE_BRIDGE_STATUS_OK &&
          projected_robot_ref_close(&robot) == TYPE_BRIDGE_STATUS_OK &&
          projected_robotzuid_close(&negative_robot_key) ==
              TYPE_BRIDGE_STATUS_OK &&
          projected_robotzuid_close(&robot_key) == TYPE_BRIDGE_STATUS_OK &&
          projected_person_ref_close(&dana) == TYPE_BRIDGE_STATUS_OK &&
          projected_identifier_close(&dana_key) == TYPE_BRIDGE_STATUS_OK &&
          projected_person_ref_close(&ada) == TYPE_BRIDGE_STATUS_OK &&
          diagnostics == NULL);
  return 1;
}
...
static int run_projection_evidence(void) {
  ...
  projected_person_create *person_create = NULL;
  type_bridge_projected_thing_t *person_thing = NULL;

  REQUIRE(projected_schema_package_open(&package, &package_diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          package != NULL && package_diagnostics == NULL);
  REQUIRE(projected_foreign_schema_package_open(&foreign_package,
                                             &package_diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
  ...
  {
    const projected_aliases *aliases[] = {values.analyst,
                                       values.mathematician};
    projected_person_create_field_aliases_chunks_v1_t aliases_chunk = {0};
    projected_person_create_args_v1_t args = {0};
    aliases_chunk.struct_size = sizeof(aliases_chunk);
    aliases_chunk.version = PROJECTED_CREATE_ARGS_VERSION;
    ...
    REQUIRE(projected_person_create_open(package, &args, &person_create,
                                      &diagnostics) ==
                TYPE_BRIDGE_STATUS_OK &&
  ...
  REQUIRE(hydrated_scalar_values_are_exact((const projected_person *)person_thing));
  REQUIRE(run_constraint_rejections(package, &values, &person_fields));
  REQUIRE(run_role_and_package_evidence(
      package, foreign_package, (const projected_person *)person_thing, &values));

  REQUIRE(type_bridge_projected_thing_close(&person_thing) ==
          TYPE_BRIDGE_STATUS_OK);
  REQUIRE(projected_person_create_close(&person_create) == TYPE_BRIDGE_STATUS_OK);
