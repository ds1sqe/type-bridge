...
pub const WORKFORCE_V2_PROOF_SCHEMA: &str =
    "tests/contracts/sdk_conformance/workforce-v2/proof-fragment-schema-v1.json";
pub const WORKFORCE_V2_PROOF_ALLOWLIST: &str =
    "tests/contracts/sdk_conformance/workforce-v2/proof-fragment-allowlist-v1.json";
pub const WORKFORCE_V2_JOURNEY: &str =
    "tests/contracts/sdk_conformance/workforce-v2/journey-v2.json";

const WORKFORCE_V2_PROOF_FORMAT: &str = "typebridge.workforce-v2-proof-fragment/v1";
const WORKFORCE_V2_ALLOWLIST_FORMAT: &str = "typebridge.workforce-v2-proof-fragment-allowlist/v1";
const WORKFORCE_V2_PROOF_MAX_BYTES: u64 = 64 * 1024;
const WORKFORCE_V2_PROOF_MAX_RESULTS: usize = 16;
const WORKFORCE_V2_PROOF_MAX_SOURCES: usize = 16;

pub type WorkforceV2ProofLane = (String, String);

#[derive(Clone, Debug, Eq, PartialEq)]
struct WorkforceV2ProofProducerAuthority {
    sources: Vec<String>,
    results: BTreeMap<WorkforceV2ProofLane, String>,
}
...
pub fn workforce_v2_source_identity(root: &Path, relative: &str) -> Result<Value, String> {
...
fn validate_proof_source_identity(
    ...
    let expected = workforce_v2_source_identity(root, relative)?;
    if value != &expected {
...
fn proof_lane(value: &Value, context: &str) -> Result<WorkforceV2ProofLane, String> {
...
fn workforce_v2_proof_authority(
    root: &Path,
    binding: &str,
) -> Result<BTreeMap<String, WorkforceV2ProofProducerAuthority>, String> {
    if !matches!(binding, "python" | "node" | "rust" | "c") {
        return Err(format!("unknown workforce-v2 binding {binding:?}"));
    }
    let raw = proof_regular_source(
        root,
        WORKFORCE_V2_PROOF_ALLOWLIST,
        "workforce-v2 proof allowlist",
    )?;
    let value: Value = serde_json::from_slice(&raw)
        .map_err(|error| format!("workforce-v2 proof allowlist is invalid JSON: {error}"))?;
    let allowlist = proof_object(
        &value,
        &["format", "semantic_profile", "bindings"],
        "allowlist",
    )?;
    if proof_string(&allowlist["format"], "allowlist format")? != WORKFORCE_V2_ALLOWLIST_FORMAT
        || proof_string(&allowlist["semantic_profile"], "allowlist profile")? != TEST_PROFILE
    {
        return Err("workforce-v2 proof allowlist authority is not v1".to_owned());
    }
    ...
    for (producer_index, producer) in producers.iter().enumerate() {
        ...
        {
            return Err("workforce-v2 proof allowlist producer ID is invalid".to_owned());
        }
        let sources = producer["sources"]
            .as_array()
            .filter(|sources| {
                !sources.is_empty() && sources.len() <= WORKFORCE_V2_PROOF_MAX_SOURCES
            })
        ...
        let results = producer["results"]
            .as_array()
            .filter(|results| {
                !results.is_empty() && results.len() <= WORKFORCE_V2_PROOF_MAX_RESULTS
            })
        ...
        for (result_index, result) in results.iter().enumerate() {
            ...
            {
                return Err("workforce-v2 proof allowlist test ID is invalid".to_owned());
            }
            if !claimed_lanes.insert(lane.clone()) {
                return Err("workforce-v2 proof allowlist lane is claimed twice".to_owned());
            }
        ...
        if authority
            .insert(
                ...
                WorkforceV2ProofProducerAuthority {
            ...
            .is_some()
        {
            return Err("workforce-v2 proof allowlist producer ID is duplicated".to_owned());
        }
...
pub fn workforce_v2_proof_lanes(
    root: &Path,
    binding: &str,
) -> Result<BTreeSet<WorkforceV2ProofLane>, String> {
    Ok(workforce_v2_proof_authority(root, binding)?
        .values()
...
fn load_proof_fragment(path: &Path) -> Result<Value, String> {
    ...
    if metadata.len() > WORKFORCE_V2_PROOF_MAX_BYTES {
        return Err(format!(
            "proof fragment {} exceeds {} bytes",
            path.display(),
            WORKFORCE_V2_PROOF_MAX_BYTES
        ));
    }
...
pub fn workforce_v2_proof_paths(value: &std::ffi::OsStr) -> Result<Vec<PathBuf>, String> {
    let paths = std::env::split_paths(value).collect::<Vec<_>>();
    if paths.is_empty() || paths.iter().any(|path| path.as_os_str().is_empty()) {
        return Err("workforce-v2 proof fragment path list is empty or malformed".to_owned());
    }
    Ok(paths)
}

pub fn validate_workforce_v2_proof_fragments(
    root: &Path,
    binding: &str,
    run_nonce: &str,
    fragment_paths: &[PathBuf],
) -> Result<BTreeMap<WorkforceV2ProofLane, Value>, String> {
    if !is_lower_hex_64(run_nonce) {
        return Err("workforce-v2 proof run nonce must be 64 lowercase hex characters".to_owned());
    }
    if fragment_paths.is_empty() {
        return Err("at least one workforce-v2 proof fragment is required".to_owned());
    }
    let producer_authority = workforce_v2_proof_authority(root, binding)?;
    let allowed = producer_authority
        .values()
        .flat_map(|producer| producer.results.keys().cloned())
        .collect::<BTreeSet<_>>();
    let expected_contract = serde_json::json!({
        "allowlist": workforce_v2_source_identity(root, WORKFORCE_V2_PROOF_ALLOWLIST)?,
        "journey": workforce_v2_source_identity(root, WORKFORCE_V2_JOURNEY)?,
        "proof_schema": workforce_v2_source_identity(root, WORKFORCE_V2_PROOF_SCHEMA)?,
    });
    ...
    for path in fragment_paths {
        ...
        if !normalized_paths.insert(normalized) {
            return Err("one workforce-v2 proof fragment path was supplied twice".to_owned());
        }
        ...
        if proof_string(&fragment["format"], "proof fragment format")? != WORKFORCE_V2_PROOF_FORMAT
            || proof_string(&fragment["semantic_profile"], "proof fragment profile")?
                != TEST_PROFILE
        {
            return Err("proof fragment format/profile is not workforce-v2 v1".to_owned());
        }
        ...
        if fragment["contract"] != expected_contract {
            return Err("proof fragment binds another workforce-v2 contract".to_owned());
        }
