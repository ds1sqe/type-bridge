 ...
 * the flat ABI-1.4 admission form from the exact embedded chunk tables. */
#include "src/models.c"

static int
phase4_flatten_resource(const type_bridge_chunked_byte_view_v1_t *resource,
                        uint8_t **out_storage,
...
type_bridge_status_t TYPE_BRIDGE_CALL phase4_schema_package_open_flat_v2(
  ...
  for (index = 0u; index < 7u; ++index) {
    if (!phase4_flatten_resource(resources[index], &storage[index],
                                 &views[index])) {
      goto cleanup;
    }
  }
