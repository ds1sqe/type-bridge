...
const REPORT_ENV: &str = "TYPE_BRIDGE_PROJECTED_RUST_REPORT";
...
fn hydration_transport(replies: Vec<GraphReply>) -> (HydrationTransport, Arc<Mutex<usize>>) {
    ...
    let advertisement_contract = RemoteCapabilities::new(
        capabilities,
        RemoteExecutorBinding::new("rust-projected-parity", "epoch-00000000001")
...
fn type_id(kind: TypeKind, label: &str) -> TypeId {
    TypeId::new(kind, label).expect("static Projected type identity is canonical")
}
...
fn attribute(label: &str, values: Vec<CompatibilityValueV2>) -> HydrationAttributeEvidenceV2 {
    HydrationAttributeEvidenceV2::new(
        AttributeId::new(label).expect("static Projected attribute is canonical"),
        values,
    )
}
...
fn role(
    ...
    HydrationRoleEvidenceV2::new(
        RoleId::new(relation_label, role_label).expect("static Projected role is canonical"),
        players,
    )
}
...
fn build_report(
    ...
    Ok(json!({
        "authority": {
            "journey": source_identity(root, JOURNEY_RELATIVE, "Projected journey")?,
            "schema": source_identity(root, SCHEMA_RELATIVE, "Projected schema")?,
        },
        "binding": "rust",
...
fn output_path() -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    if !path.is_absolute() || path.file_name().is_none() {
        return Err("Projected Rust report path must be an absolute file path".into());
    }
    Ok(path)
}
...
fn publish_report(path: &Path, report: &Value) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if payload.len() > MAX_REPORT_BYTES {
        return Err("canonical Projected Rust report exceeds its byte ceiling".into());
    }
    let parent = path
        .parent()
        .ok_or("Projected Rust report path has no parent")?;
    let parent_metadata = fs::symlink_metadata(parent)?;
    if parent_metadata.file_type().is_symlink() || !parent_metadata.is_dir() {
        return Err("Projected Rust report parent must be a non-symlink directory".into());
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !root.is_absolute() {
        return Err("Projected repository root must be absolute".into());
    }
