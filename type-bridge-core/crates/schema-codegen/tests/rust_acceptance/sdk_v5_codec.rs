...
fn publish_provider_free_corpus(records: &[Vec<u8>; 9], archive: &[u8]) -> std::io::Result<()> {
    let Some(path) = std::env::var_os("TYPE_BRIDGE_SDK_V5_CORPUS") else {
        return Ok(());
    };
    ...
    if !path.is_absolute() {
        return Err(std::io::Error::new(
            ...
            "Sdk V5 corpus path must be absolute",
        ));
    }
    let record_b64 = records
        ...
        .join(",");
    let payload = format!(
        "{{\"archive_b64\":\"{}\",\"binding\":\"rust\",\"format\":\"typebridge.sdk-v5-provider-free-corpus/v1\",\"record_b64\":[{}]}}",
        base64(archive),
        record_b64,
    );
...
fn publish_operational_evidence(payload: &str) -> std::io::Result<()> {
    let Some(path) = std::env::var_os("TYPE_BRIDGE_SDK_V5_OPERATIONAL_EVIDENCE") else {
        return Ok(());
    };
    let path = std::path::PathBuf::from(path);
    if !path.is_absolute() {
        return Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "Sdk V5 operational evidence path must be absolute",
        ));
    }
    use std::io::Write as _;
    let mut output = std::fs::OpenOptions::new()
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let val_datetime_tz = ValDatetimeTz::new(DateTimeTz::try_new("2026-08-22T12:34:56Z")?)?;
    ...
    let person_row = HydratedRow::new(
        ...
        vec![
            (
                ...
                vec![identifier_scalar.clone()],
            ),
            (
                PersonType::score.owns_id_json(),
                vec![score.value().into_encoded_scalar()],
            ),
            (
                PersonType::score__gte.owns_id_json(),
                vec![7_i64.into_encoded_scalar()],
            ),
            (
                PersonType::val_bool.owns_id_json(),
                vec![val_bool.value().into_encoded_scalar()],
            ),
            (
                PersonType::val_constrained.owns_id_json(),
                vec![val_constrained.value().into_encoded_scalar()],
            ),
            (
                PersonType::val_date.owns_id_json(),
                vec![val_date.value().into_encoded_scalar()],
            ),
            (
                PersonType::val_datetime.owns_id_json(),
                vec![val_datetime.value().into_encoded_scalar()],
            ),
            (
                PersonType::val_datetime_tz.owns_id_json(),
                vec![val_datetime_tz.value().into_encoded_scalar()],
            ),
            (
                PersonType::val_decimal.owns_id_json(),
                vec![val_decimal.value().into_encoded_scalar()],
            ),
            (
                PersonType::val_double.owns_id_json(),
                vec![val_double.value().into_encoded_scalar()],
            ),
            (
                PersonType::val_duration.owns_id_json(),
                vec![val_duration.value().into_encoded_scalar()],
            ),
        ],
        vec![],
    );
    ...
    let membership = SCHEMA.encode_create(MembershipCreate::new(MembershipMemberRef::Robot(
        robot_ref.clone(),
    ))?)?;
    ...
    let input_limited = CanonicalCodecOptions::new()
        .with_limits(CanonicalCodecLimits::new().with_max_input_bytes(archive.len() - 1));
    let input_error = expect_code(
        SCHEMA.decode_archive_controlled(&archive, &input_limited),
        "projected_codec_input_limit",
    );
    let output_limited = CanonicalCodecOptions::new()
        .with_limits(CanonicalCodecLimits::new().with_max_output_bytes(archive.len() - 1));
    ...
    let depth_limited =
        CanonicalCodecOptions::new().with_limits(CanonicalCodecLimits::new().with_max_depth(1));
    ...
    let operational = format!(
        concat!(
            ...
            "\"test_id\":\"rust_acceptance::generated_rust_sdk_v5_canonical_codec\"}}"
        ),
