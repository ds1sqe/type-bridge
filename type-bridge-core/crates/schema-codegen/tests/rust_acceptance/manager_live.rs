    ...
    for (label, relative, expected) in AUTHORITY_FILES {
        ...
        if digest != expected {
            return Err(ProducerError::new(
                ...
                format!("Manager {label} authority drifted"),
            ));
        }
        report.insert(
            label.to_owned(),
...
async fn manager_observation(database: &GeneratedDatabase<AppSchema>) -> ProducerResult<Value> {
    ...
    let journey = async {
        ...
        let person = live(session.exact::<Person>(), "bind sibling query person")?;
        let sibling_query = live(
            live(session.query(person), "select sibling query person")?.where_(
                person.field(PersonType::identifier).eq(generated(
                    Identifier::new("data-ada"),
                    "sibling query identity",
                )?),
            ),
            "filter sibling query person",
        )?;
        if live(
            sibling_query.count_by(person).await,
            "execute sibling query count",
        )? != 1
        {
            return Err(ProducerError::new(
                "borrowed_read_reuse",
                "sibling query query did not remain usable",
            ));
        }
        drop(sibling_query);
        drop(session);
...
async fn run() -> ProducerResult<()> {
    if SEMANTIC_SCHEMA_FINGERPRINT_JSON != EXPECTED_SEMANTIC_FINGERPRINT {
        return Err(ProducerError::new(
            ...
            "generated crate is not the exact Sdk V3 projection",
        ));
    }
...
async fn main() {
    if let Err(error) = run().await {
        eprintln!("Manager Rust manager live producer rejected: {error}");
        std::process::exit(1);
    }
}
