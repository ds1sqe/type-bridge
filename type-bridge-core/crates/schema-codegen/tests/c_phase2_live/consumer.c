...
#define CHECK(condition)                                                       \
  do {                                                                         \
    if (!(condition)) {                                                        \
      fprintf(stderr, "C Phase-2 live failure at line %d\n", __LINE__);      \
      return __LINE__;                                                         \
    }                                                                          \
  } while (0)
...
static uint32_t required_http_port(void) {
  const char *value = getenv("TYPE_BRIDGE_PHASE2_LIVE_HTTP_PORT");
  char *end = NULL;
...
static int close_person_inputs(person_inputs_t *values) {
  return phase2_valzuduration_close(&values->duration) ==
         ...
         phase2_valzudouble_close(&values->double_value) ==
             TYPE_BRIDGE_STATUS_OK &&
         phase2_valzudecimal_close(&values->decimal) ==
             TYPE_BRIDGE_STATUS_OK &&
...
static int open_person_create(
    ...
    phase2_person_create **out_create,
    type_bridge_execution_diagnostics_t **diagnostics) {
  person_inputs_t values = {0};
  phase2_person_create_args_v1_t args = {0};
  int ok = 0;
  if (phase2_identifier_open(package, view_of(identifier), &values.identifier,
                             diagnostics) != TYPE_BRIDGE_STATUS_OK ||
      phase2_score_open(package, score, &values.score, diagnostics) !=
          TYPE_BRIDGE_STATUS_OK ||
      phase2_foozuzubar_open(package, foo_bar, &values.foo_bar, diagnostics) !=
          TYPE_BRIDGE_STATUS_OK ||
      phase2_scorezuzugte_open(package, score_gte, &values.score_gte,
                               diagnostics) != TYPE_BRIDGE_STATUS_OK ||
      phase2_valzubool_open(package, boolean_value, &values.boolean_value,
                            diagnostics) != TYPE_BRIDGE_STATUS_OK ||
      phase2_valzuconstrained_open(package, constrained, &values.constrained,
                                   diagnostics) != TYPE_BRIDGE_STATUS_OK ||
      phase2_valzudate_open(package, view_of(date), &values.date, diagnostics) !=
          TYPE_BRIDGE_STATUS_OK ||
      phase2_valzudatetime_open(package, view_of(datetime), &values.datetime,
                                diagnostics) != TYPE_BRIDGE_STATUS_OK ||
      phase2_valzudatetimezutzz_open(package, view_of(datetime_tz),
                                     &values.datetime_tz, diagnostics) !=
          TYPE_BRIDGE_STATUS_OK ||
      phase2_valzudecimal_open(package, view_of(decimal), &values.decimal,
                               diagnostics) != TYPE_BRIDGE_STATUS_OK ||
      phase2_valzudouble_open(package, double_bits, &values.double_value,
                              diagnostics) != TYPE_BRIDGE_STATUS_OK ||
      phase2_valzuduration_open(package, view_of(duration), &values.duration,
                                diagnostics) != TYPE_BRIDGE_STATUS_OK) {
    goto close;
  }
  if (nickname != NULL &&
      phase2_nickname_open(package, view_of(nickname), &values.nickname,
                           diagnostics) != TYPE_BRIDGE_STATUS_OK) {
    goto close;
  }
  ...
  ok = phase2_person_create_open(package, &args, out_create, diagnostics) ==
           TYPE_BRIDGE_STATUS_OK &&
...
static int open_robot_create(
    ...
    int64_t constrained, phase2_robot_create **out_create,
    type_bridge_execution_diagnostics_t **diagnostics) {
  phase2_robotzuid *robot_id = NULL;
  phase2_valzuconstrained *value = NULL;
  phase2_robot_create_args_v1_t args = {0};
  int ok = phase2_robotzuid_open(package, key, &robot_id, diagnostics) ==
               TYPE_BRIDGE_STATUS_OK &&
           phase2_valzuconstrained_open(package, constrained, &value,
                                        diagnostics) == TYPE_BRIDGE_STATUS_OK;
  if (ok) {
    args.struct_size = sizeof(args);
    args.version = PHASE2_CREATE_ARGS_VERSION;
    ...
    ok = phase2_robot_create_open(package, &args, out_create, diagnostics) ==
             TYPE_BRIDGE_STATUS_OK &&
         *out_create != NULL && *diagnostics == NULL;
  }
  return phase2_valzuconstrained_close(&value) == TYPE_BRIDGE_STATUS_OK &&
         phase2_robotzuid_close(&robot_id) == TYPE_BRIDGE_STATUS_OK && ok;
}

static int person_ref_from_iid(
    const type_bridge_schema_package_t *package, const iid_copy_t *iid,
    phase2_person_ref **reference,
    type_bridge_execution_diagnostics_t **diagnostics) {
  return phase2_person_ref_from_iid(package, iid_view(iid), reference,
                                    diagnostics) == TYPE_BRIDGE_STATUS_OK &&
         *reference != NULL && *diagnostics == NULL;
}

static int observe_person_scalars(
    const phase2_person *person,
    type_bridge_execution_diagnostics_t **diagnostics) {
  ...
  int ok =
      phase2_person_valzubool(person, &boolean_value, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      phase2_valzubool_value(boolean_value, &bool_value, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      bool_value == 0u &&
      phase2_person_valzudate(person, &date, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      phase2_valzudate_value(date, &text, diagnostics) == TYPE_BRIDGE_STATUS_OK &&
      text_is(text, "2026-08-12") &&
      phase2_person_valzudatetime(person, &datetime, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      phase2_valzudatetime_value(datetime, &text, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      text_is(text, "2026-08-12T09:30:00") &&
      phase2_person_valzudatetimezutzz(person, &datetime_tz, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      phase2_valzudatetimezutzz_value(datetime_tz, &text, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      text_is(text, "2026-08-12T09:30:00Z") &&
      phase2_person_valzudecimal(person, &decimal, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      phase2_valzudecimal_value(decimal, &text, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      text_is(text, "38.5") &&
      phase2_person_valzudouble(person, &double_value, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      phase2_valzudouble_value(double_value, &bits, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      bits == UINT64_C(0x4043000000000000) &&
      phase2_person_valzuduration(person, &duration, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      phase2_valzuduration_value(duration, &text, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      text_is(text, "PT38S") &&
      phase2_person_score(person, &score, diagnostics) == TYPE_BRIDGE_STATUS_OK &&
      phase2_score_value(score, &long_value, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      long_value == 38 &&
      phase2_person_nickname(person, &nickname, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      phase2_nickname_value(nickname, &text, diagnostics) ==
          TYPE_BRIDGE_STATUS_OK &&
      text_is(text, "Ada");
  ok = phase2_nickname_close(&nickname) == TYPE_BRIDGE_STATUS_OK &&
       ...
       phase2_valzudatetimezutzz_close(&datetime_tz) == TYPE_BRIDGE_STATUS_OK &&
       phase2_valzudatetime_close(&datetime) == TYPE_BRIDGE_STATUS_OK &&
       phase2_valzudate_close(&date) == TYPE_BRIDGE_STATUS_OK &&
       phase2_valzubool_close(&boolean_value) == TYPE_BRIDGE_STATUS_OK && ok;
  return ok && *diagnostics == NULL;
}

static int interaction_actor_is(
    const phase2_interaction *interaction, int expected_kind,
    type_bridge_execution_diagnostics_t **diagnostics) {
  phase2_interaction_actor_player *actor = NULL;
  int ok = phase2_interaction_actor(interaction, &actor, diagnostics) ==
           TYPE_BRIDGE_STATUS_OK;
  if (!ok) {
    return 0;
  }
  ...
  if (expected_kind == 1) {
    ...
    type_bridge_byte_view_t text = {0};
    ok = phase2_interaction_actor_player_as_person(actor, &person, diagnostics) ==
             TYPE_BRIDGE_STATUS_OK &&
         phase2_person_ref_identifier_key(person, &key, diagnostics) ==
             TYPE_BRIDGE_STATUS_OK &&
         phase2_identifier_value(key, &text, diagnostics) ==
             TYPE_BRIDGE_STATUS_OK &&
         text_is(text, "data-ada");
    ok = phase2_identifier_close(&key) == TYPE_BRIDGE_STATUS_OK &&
         phase2_person_ref_close(&person) == TYPE_BRIDGE_STATUS_OK && ok;
  } else {
    phase2_robot_ref *robot = NULL;
    phase2_robotzuid *key = NULL;
    int64_t value = 0;
    ok = phase2_interaction_actor_player_as_robot(actor, &robot, diagnostics) ==
             TYPE_BRIDGE_STATUS_OK &&
         ...
         phase2_robotzuid_value(key, &value, diagnostics) ==
             TYPE_BRIDGE_STATUS_OK &&
         value == 7;
    ok = phase2_robotzuid_close(&key) == TYPE_BRIDGE_STATUS_OK &&
         phase2_robot_ref_close(&robot) == TYPE_BRIDGE_STATUS_OK && ok;
  }
  return phase2_interaction_actor_player_close(&actor) == TYPE_BRIDGE_STATUS_OK &&
         ok && *diagnostics == NULL;
}

int main(void) {
  const char *address = getenv("TYPE_BRIDGE_PHASE2_LIVE_ADDRESS");
  const char *database_name = getenv("TYPE_BRIDGE_PHASE2_LIVE_DATABASE");
  ...
  phase2_person_create *ada_create = NULL;
  phase2_person_create *dana_create = NULL;
  phase2_person *person = NULL;
  phase2_robot_create *robot_positive_create = NULL;
  phase2_robot_create *robot_negative_create = NULL;
  phase2_robot *robot = NULL;
  iid_copy_t iids[10] = {{{0}, 0u}};
  phase2_person_ref *ada_ref = NULL;
  phase2_person_ref *dana_ref = NULL;
  phase2_robot_ref *robot_ref = NULL;
  phase2_interaction_target_player *ada_target = NULL;
  phase2_interaction_target_player *dana_target = NULL;
  phase2_interaction_actor_player *person_actor = NULL;
  phase2_interaction_actor_player *robot_actor = NULL;
  phase2_identifier *interaction_ids[3] = {NULL, NULL, NULL};
  phase2_interaction_create *interaction_creates[3] = {NULL, NULL, NULL};
  phase2_interaction *interaction = NULL;
  phase2_plainzhactivity_participant_player *plain_player = NULL;
  phase2_plainzhactivity_create *plain_create = NULL;
  phase2_plainzhactivity *plain = NULL;
  phase2_event_subject_player *event_subject = NULL;
  phase2_event_create *event_create = NULL;
  phase2_event *event = NULL;
  phase2_event_ref *event_ref = NULL;
  phase2_container_item_player *event_item = NULL;
  phase2_container_create *container_create = NULL;
  phase2_container *container = NULL;
  uint64_t count = 0u;
  ...
  CHECK(password != NULL);
  CHECK(http_port != 0u);
  CHECK(phase2_schema_package_open(&package, &package_diagnostics) ==
            TYPE_BRIDGE_STATUS_OK &&
        package != NULL && package_diagnostics == NULL);
  CHECK(phase2_foreign_schema_package_open(&foreign_package,
                                           &package_diagnostics) ==
            TYPE_BRIDGE_STATUS_OK &&
...

  {
    ...
    phase2_foreign_person_ref *foreign_ref = NULL;
    phase2_plainzhactivity_participant_player *rejected =
        (phase2_plainzhactivity_participant_player *)(uintptr_t)1u;
    CHECK(phase2_foreign_identifier_open(foreign_package, VIEW("data-ada"),
                                         &foreign_key,
                                         &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          phase2_foreign_person_ref_from_key(foreign_package, foreign_key,
                                             &foreign_ref,
                                             &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK);
    CHECK(phase2_plainzhactivity_participant_player_from_person(
              (const phase2_person_ref *)foreign_ref, &rejected,
    ...
    CHECK(type_bridge_execution_diagnostics_close(&diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          diagnostics == NULL &&
          phase2_foreign_person_ref_close(&foreign_ref) ==
              TYPE_BRIDGE_STATUS_OK &&
  ...
  CHECK(phase2_person_database_insert(database, ada_create, NULL, &person,
                                      &diagnostics) == TYPE_BRIDGE_STATUS_OK);
  {
    type_bridge_byte_view_t iid = {0};
    CHECK(phase2_person_iid(person, &iid, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          copy_iid(iid, &iids[0]) && observe_person_scalars(person, &diagnostics));
  }
  CHECK(phase2_person_close(&person) == TYPE_BRIDGE_STATUS_OK);
  CHECK(open_person_create(
  ...
  CHECK(phase2_person_database_insert(database, dana_create, NULL, &person,
                                      &diagnostics) == TYPE_BRIDGE_STATUS_OK);
  {
    type_bridge_byte_view_t iid = {0};
    CHECK(phase2_person_iid(person, &iid, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
  ...
  CHECK(open_robot_create(package, 7, 12, &robot_positive_create,
                          &diagnostics));
  CHECK(phase2_robot_database_insert(database, robot_positive_create, NULL,
                                     &robot, &diagnostics) ==
        TYPE_BRIDGE_STATUS_OK);
  {
    type_bridge_byte_view_t iid = {0};
    phase2_robotzuid *key = NULL;
    int64_t value = 0;
    CHECK(phase2_robot_iid(robot, &iid, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          copy_iid(iid, &iids[2]) &&
          phase2_robot_robotzuid(robot, &key, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          phase2_robotzuid_value(key, &value, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          value == 7 && phase2_robotzuid_close(&key) == TYPE_BRIDGE_STATUS_OK);
  }
  CHECK(phase2_robot_close(&robot) == TYPE_BRIDGE_STATUS_OK);
  CHECK(open_robot_create(package, -7, 13, &robot_negative_create,
                          &diagnostics));
  CHECK(phase2_robot_database_insert(database, robot_negative_create, NULL,
                                     &robot, &diagnostics) ==
        TYPE_BRIDGE_STATUS_OK);
  {
    type_bridge_byte_view_t iid = {0};
  ...
  CHECK(phase2_robot_close(&robot) == TYPE_BRIDGE_STATUS_OK);
  CHECK(phase2_robot_ref_from_iid(package, iid_view(&iids[2]), &robot_ref,
                                  &diagnostics) == TYPE_BRIDGE_STATUS_OK);

  CHECK(phase2_interaction_target_player_from_person(
            ada_ref, &ada_target, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
        phase2_interaction_target_player_from_person(
            ...
            robot_ref, &robot_actor, &diagnostics) == TYPE_BRIDGE_STATUS_OK);
  {
    ...
    phase2_interaction_actor_player *actors[3] = {robot_actor, NULL,
                                                  person_actor};
    phase2_interaction_target_player *targets[3] = {ada_target, dana_target,
                                                     dana_target};
    ...
    for (index = 0u; index < 3u; ++index) {
      ...
      type_bridge_byte_view_t iid = {0};
      CHECK(phase2_identifier_open(package, view_of(keys[index]),
      ...
      CHECK(phase2_interaction_create_open(
                package, &args, &interaction_creates[index], &diagnostics) ==
            TYPE_BRIDGE_STATUS_OK);
      CHECK(phase2_interaction_database_insert(
                database, interaction_creates[index], NULL, &interaction,
                &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
            interaction_actor_is(interaction, expected_kinds[index],
                                 &diagnostics) &&
            phase2_interaction_iid(interaction, &iid, &diagnostics) ==
                TYPE_BRIDGE_STATUS_OK &&
            copy_iid(iid, &iids[4u + index]));
      CHECK(phase2_interaction_close(&interaction) == TYPE_BRIDGE_STATUS_OK);
    }
  }
            ...
            ada_ref, &plain_player, &diagnostics) == TYPE_BRIDGE_STATUS_OK);
  {
    ...
    phase2_person_ref *participant = NULL;
    phase2_identifier *key = NULL;
    type_bridge_byte_view_t text = {0};
    args.struct_size = sizeof(args);
    args.version = PHASE2_CREATE_ARGS_VERSION;
    args.role_participant = plain_player;
    CHECK(phase2_plainzhactivity_create_open(package, &args, &plain_create,
                                             &diagnostics) ==
          TYPE_BRIDGE_STATUS_OK);
    CHECK(phase2_plainzhactivity_database_insert(
              database, plain_create, NULL, &plain, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          phase2_plainzhactivity_iid(plain, &iid, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          copy_iid(iid, &iids[7]) &&
          phase2_plainzhactivity_participant(plain, &hydrated, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          phase2_plainzhactivity_participant_player_as_person(
              hydrated, &participant, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          phase2_person_ref_identifier_key(participant, &key, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          phase2_identifier_value(key, &text, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          text_is(text, "data-ada"));
    CHECK(phase2_identifier_close(&key) == TYPE_BRIDGE_STATUS_OK &&
          phase2_person_ref_close(&participant) == TYPE_BRIDGE_STATUS_OK &&
          phase2_plainzhactivity_participant_player_close(&hydrated) ==
              TYPE_BRIDGE_STATUS_OK &&
          phase2_plainzhactivity_close(&plain) == TYPE_BRIDGE_STATUS_OK);
    CHECK(phase2_plainzhactivity_database_get_by_iid(
              database, iid_view(&iids[7]), NULL, &plain, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          plain != NULL && diagnostics == NULL);
    CHECK(phase2_plainzhactivity_close(&plain) == TYPE_BRIDGE_STATUS_OK);
  }

  CHECK(phase2_event_subject_player_from_person(ada_ref, &event_subject,
                                                 &diagnostics) ==
        TYPE_BRIDGE_STATUS_OK);
  {
    phase2_event_create_args_v1_t args = {0};
    type_bridge_byte_view_t iid = {0};
    args.struct_size = sizeof(args);
    args.version = PHASE2_CREATE_ARGS_VERSION;
    args.role_subject = event_subject;
    CHECK(phase2_event_create_open(package, &args, &event_create,
                                   &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          phase2_event_database_insert(database, event_create, NULL, &event,
                                       &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          phase2_event_iid(event, &iid, &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          copy_iid(iid, &iids[8]));
    CHECK(phase2_event_close(&event) == TYPE_BRIDGE_STATUS_OK);
    CHECK(phase2_event_ref_from_iid(package, iid_view(&iids[8]), &event_ref,
                                    &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          phase2_container_item_player_from_event(event_ref, &event_item,
                                                   &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK);
  }
  {
    ...
    type_bridge_byte_view_t iid = {0};
    phase2_container_item_player *hydrated_item = NULL;
    ...
    CHECK(phase2_container_create_open(package, &args, &container_create,
                                       &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          phase2_container_database_insert(database, container_create, NULL,
                                           &container, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          phase2_container_iid(container, &iid, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          copy_iid(iid, &iids[9]) &&
          phase2_container_item_count(container, &item_count, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          item_count == 1u &&
          phase2_container_item_at(container, 0u, &hydrated_item,
                                   &diagnostics) == TYPE_BRIDGE_STATUS_OK &&
          phase2_container_item_player_as_event(
              hydrated_item, &hydrated_event, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          phase2_event_ref_iid(hydrated_event, &hydrated_iid, &diagnostics) ==
              TYPE_BRIDGE_STATUS_OK &&
          hydrated_iid.length == iids[8].length &&
          memcmp(hydrated_iid.data, iids[8].bytes, iids[8].length) == 0);
    CHECK(phase2_event_ref_close(&hydrated_event) == TYPE_BRIDGE_STATUS_OK &&
          phase2_container_item_player_close(&hydrated_item) ==
              TYPE_BRIDGE_STATUS_OK &&
          phase2_container_close(&container) == TYPE_BRIDGE_STATUS_OK);
  }

  CHECK(phase2_container_database_delete_by_iid(
            database, iid_view(&iids[9]), NULL, &diagnostics) ==
        TYPE_BRIDGE_STATUS_OK);
  CHECK(phase2_event_database_delete_by_iid(database, iid_view(&iids[8]), NULL,
                                            &diagnostics) ==
        TYPE_BRIDGE_STATUS_OK);
  CHECK(phase2_plainzhactivity_database_delete_by_iid(
  ...
  CHECK(phase2_plainzhactivity_database_get_by_iid(
            ...
            TYPE_BRIDGE_STATUS_OK &&
        plain == NULL && diagnostics == NULL);
  CHECK(phase2_plainzhactivity_database_count(database, NULL, &count,
                                              &diagnostics) ==
            TYPE_BRIDGE_STATUS_OK &&
        count == 0u);
  CHECK(phase2_interaction_database_delete_by_iid(
  ...
  CHECK(phase2_interaction_database_delete_by_iid(
        ...
        TYPE_BRIDGE_STATUS_OK);
  CHECK(phase2_robot_database_delete_by_iid(database, iid_view(&iids[3]), NULL,
                                            &diagnostics) ==
        TYPE_BRIDGE_STATUS_OK);
  CHECK(phase2_robot_database_delete_by_iid(database, iid_view(&iids[2]), NULL,
                                            &diagnostics) ==
        TYPE_BRIDGE_STATUS_OK);
  CHECK(phase2_person_database_delete_by_iid(database, iid_view(&iids[1]), NULL,
                                             &diagnostics) ==
        TYPE_BRIDGE_STATUS_OK);
  CHECK(phase2_person_database_delete_by_iid(database, iid_view(&iids[0]), NULL,
                                             &diagnostics) ==
        TYPE_BRIDGE_STATUS_OK);
  CHECK(phase2_container_database_count(database, NULL, &count, &diagnostics) ==
            TYPE_BRIDGE_STATUS_OK &&
        count == 0u);
  CHECK(phase2_event_database_count(database, NULL, &count, &diagnostics) ==
            TYPE_BRIDGE_STATUS_OK &&
        count == 0u);
  CHECK(phase2_interaction_database_count(database, NULL, &count,
                                          &diagnostics) ==
            TYPE_BRIDGE_STATUS_OK &&
        count == 0u);
  CHECK(phase2_robot_database_count(database, NULL, &count, &diagnostics) ==
            TYPE_BRIDGE_STATUS_OK &&
        count == 0u);
  CHECK(phase2_person_database_count(database, NULL, &count, &diagnostics) ==
            TYPE_BRIDGE_STATUS_OK &&
        count == 0u);

  CHECK(phase2_container_create_close(&container_create) ==
            TYPE_BRIDGE_STATUS_OK &&
        phase2_container_item_player_close(&event_item) ==
            TYPE_BRIDGE_STATUS_OK &&
        phase2_event_ref_close(&event_ref) == TYPE_BRIDGE_STATUS_OK &&
        phase2_event_create_close(&event_create) == TYPE_BRIDGE_STATUS_OK &&
        phase2_event_subject_player_close(&event_subject) ==
            TYPE_BRIDGE_STATUS_OK &&
            ...
            TYPE_BRIDGE_STATUS_OK);
  {
    size_t index;
    for (index = 0u; index < 3u; ++index) {
      CHECK(phase2_interaction_create_close(&interaction_creates[index]) ==
                TYPE_BRIDGE_STATUS_OK &&
            phase2_identifier_close(&interaction_ids[index]) ==
                TYPE_BRIDGE_STATUS_OK);
    }
  }
  CHECK(phase2_interaction_actor_player_close(&robot_actor) ==
            TYPE_BRIDGE_STATUS_OK &&
        phase2_interaction_actor_player_close(&person_actor) ==
            TYPE_BRIDGE_STATUS_OK &&
        phase2_interaction_target_player_close(&dana_target) ==
            TYPE_BRIDGE_STATUS_OK &&
        phase2_interaction_target_player_close(&ada_target) ==
            TYPE_BRIDGE_STATUS_OK &&
        phase2_robot_ref_close(&robot_ref) == TYPE_BRIDGE_STATUS_OK &&
        phase2_robot_create_close(&robot_negative_create) ==
            TYPE_BRIDGE_STATUS_OK &&
        phase2_robot_create_close(&robot_positive_create) ==
            TYPE_BRIDGE_STATUS_OK &&
        phase2_person_ref_close(&dana_ref) == TYPE_BRIDGE_STATUS_OK &&
        phase2_person_ref_close(&ada_ref) == TYPE_BRIDGE_STATUS_OK &&
        phase2_person_create_close(&dana_create) == TYPE_BRIDGE_STATUS_OK &&
        phase2_person_create_close(&ada_create) == TYPE_BRIDGE_STATUS_OK);
  ...
  puts("TYPE_BRIDGE_C_PHASE2_LIVE_FACT\trelation_as_player\t{\"owner\":{\"model\":\"container\",\"ref\":\"container-event\"},\"player\":{\"model\":\"event\",\"ref\":\"event-ada\"},\"preserved\":true,\"role\":\"item\"}");
  return 0;
}
