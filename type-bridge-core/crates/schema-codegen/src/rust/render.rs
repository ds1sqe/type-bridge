...
pub(super) fn render(
    ...
    GeneratedPackage::try_new([
        ...
        ("src/lib.rs".to_owned(), render_lib(projection).into_bytes()),
...
fn header() -> &'static str {
    "// Generated by type-bridge from a verified Rust RuntimeProjection. Do not edit.\n// Schema names retain their versioned or explicitly overridden spelling.\n#![allow(non_snake_case, non_camel_case_types)]\n\n"
}

fn render_lib(projection: &RuntimeProjection) -> String {
    let mut output = format!(
        "{}#![forbid(unsafe_code)]\n\npub mod runtime;\npub mod structs;\npub mod reference;\npub mod read;\npub mod tokens;\npub mod declaration;\npub mod create;\npub mod functions;\npub mod schema;\n\npub use create::*;\npub use declaration::*;\n",
        header()
    );
    if !projection.emission().functions().is_empty() {
        output.push_str("pub use functions::*;\n");
    }
    output.push_str(
        "pub use read::*;\npub use reference::*;\npub use runtime::*;\npub use schema::*;\n",
    );
    if !projection.emission().structs().is_empty() {
        output.push_str("pub use structs::*;\n");
    }
    output.push_str("pub use tokens::*;\n");
    output
}
...
fn render_declaration(projection: &RuntimeProjection) -> Result<String, Diagnostic> {
    ...
    if projection
        .models()
        .values()
        .any(|model| !model.query_tokens().roles().is_empty())
    {
        output.push_str("use crate::tokens::*;\n");
    }
...
fn collect_optional_entries(entries: &str) -> String {
    if entries.is_empty() {
        "Vec::new()".to_owned()
    } else {
...
fn render_create(projection: &RuntimeProjection, ordered: bool) -> Result<String, Diagnostic> {
    ...
    for id in projection.emission().model_shells() {
        ...
        for member in &members {
            ...
            let (target, encode) = if member.is_role {
                (
                    &mut enc_roles_code,
                    "__tb_value.clone().into_encoded_reference()?",
                )
            } else {
                (&mut enc_fields_code, "__tb_value.into_encoded_scalar()")
            };
            match (member.container, member.required) {
                (ProjectedContainer::Scalar, true) => {
                    let _ = writeln!(
                        target,
                        "      Some(({token:?}, vec![{{ let __tb_value = self.{mname}.value(); {encode} }}])),"
                    );
                }
                (ProjectedContainer::Scalar, false) => {
                    if member.is_role {
                        let _ = writeln!(
                            target,
                            "      match self.{mname}.as_ref() {{ Some(__tb_value) => Some(({token:?}, vec![{encode}])), None => None }},"
                        );
                    } else {
                        let _ = writeln!(
                            target,
                            "      self.{mname}.as_ref().map(|__tb_value| ({token:?}, vec![{encode}])),"
                        );
                    }
                }
                (ProjectedContainer::Sequence, _) => {
                    let _ = writeln!(
                        target,
                        "      Some(({token:?}, {{ let mut __tb_values = Vec::new(); for __tb_value in self.{mname}.as_slice() {{ __tb_values.push({encode}); }} __tb_values }})),"
                    );
                }
            }
        }

        let enc_fields_code = collect_optional_entries(&enc_fields_code);
        let enc_roles_code = collect_optional_entries(&enc_roles_code);
        let _ = writeln!(
            output,
            "impl IntoEncodedCreate for {name} {{\n  fn into_encoded_create(self) -> Result<EncodedCreate, ValidationError> {{\n    let __tb_fields = {enc_fields_code};\n    let __tb_roles = {enc_roles_code};\n    Ok(EncodedCreate::new({read_name}::TYPE_ID_JSON, __tb_fields, __tb_roles))\n  }}\n}}\n"
        );
        ...
        let _ = writeln!(
            output,
            "impl MaterializeCreate for {name} {{\n  type Schema = crate::AppSchema;\n  fn materialize_create(__tb_value: &DecodedCreate, __tb_path: &ValidationPath) -> Result<Self, ValidationError> {{\n    if __tb_value.type_id_json() != {read_name}::TYPE_ID_JSON {{ return Err(ValidationError::new(__tb_path.path(), \"wrong_concrete_model_type\")); }}\n    let mut __tb_seen = std::collections::BTreeSet::<&str>::new();\n    for (__tb_token, _) in __tb_value.fields() {{\n      if !__tb_seen.insert(__tb_token.as_str()) {{ return Err(ValidationError::new(__tb_path.path(), \"duplicate_scalar_evidence\")); }}\n      if ![{expected_fields}].contains(&__tb_token.as_str()) {{ return Err(ValidationError::new(__tb_path.path(), \"unexpected_field_evidence\")); }}\n    }}\n    __tb_seen.clear();\n    for (__tb_token, _) in __tb_value.roles() {{\n      if !__tb_seen.insert(__tb_token.as_str()) {{ return Err(ValidationError::new(__tb_path.path(), \"duplicate_role_evidence\")); }}\n      if ![{expected_roles}].contains(&__tb_token.as_str()) {{ return Err(ValidationError::new(__tb_path.path(), \"unexpected_role_evidence\")); }}\n    }}\n{decode_members}    Self::try_new({arguments})\n  }}\n}}\n"
        );
    }
    Ok(output)
}
            ...
            for member in &members {
                ...
                match (member.container, member.required) {
                    (ProjectedContainer::Scalar, true) => {
                        let _ = writeln!(
                            target,
                            "      Some(({token:?}, vec![{{ let __tb_value = self.{mname}(); {encode} }}])),"
                        );
                    }
                    (ProjectedContainer::Scalar, false) => {
                        if member.is_role {
                            let _ = writeln!(
                                target,
                                "      match self.{mname}() {{ Some(__tb_value) => Some(({token:?}, vec![{encode}])), None => None }},"
                            );
                        } else {
                            let _ = writeln!(
                                target,
                                "      self.{mname}().map(|__tb_value| ({token:?}, vec![{encode}])),"
                            );
                        }
                    }
                    (ProjectedContainer::Sequence, _) => {
                        let _ = writeln!(
                            target,
                            "      Some(({token:?}, {{ let mut __tb_members = Vec::new(); for __tb_value in self.{mname}() {{ __tb_members.push({encode}); }} __tb_members }})),"
                        );
                    }
                }
            }
            let snapshot_fields = collect_optional_entries(&snapshot_fields);
            let snapshot_roles = collect_optional_entries(&snapshot_roles);
            let _ = writeln!(
                output,
                "impl IntoHydratedSnapshot for {name} {{\n  fn into_hydrated_snapshot(self) -> Result<HydratedRow, ValidationError> {{\n    let __tb_fields = {snapshot_fields};\n    let __tb_values = {snapshot_roles};\n    Ok(HydratedRow::new(Self::TYPE_ID_JSON, self.iid().to_owned(), __tb_fields, __tb_values))\n  }}\n}}\n"
            );
...
fn render_reference(projection: &RuntimeProjection) -> Result<String, Diagnostic> {
    ...
    for id in projection.emission().model_shells() {
        ...
        if members.is_empty() {
            output.push_str("\n    if !__tb_player.keys().is_empty() {\n      return Err(ValidationError::new(__tb_path.join(\"keys\").join_index(0).path(), \"unexpected_reference_key\"));\n    }\n");
        } else {
...
fn render_families(projection: &RuntimeProjection) -> Result<String, Diagnostic> {
    ...
    for id in projection.emission().model_shells() {
        ...
        let _ = writeln!(
            output,
            "// Variants preserve the generated model API without boxing.\n#[allow(clippy::large_enum_variant)]\n#[derive(Clone, Debug, PartialEq)]\npub enum {family_name} {{"
        );
        for descendant in &descendants {
...
fn render_record(
    ...
    if members.len() > 7 {
        output.push_str("  // Constructor parameters follow the schema fields.\n  #[allow(clippy::too_many_arguments)]\n");
    }
    let _ = write!(output, "  pub fn {primary_fn}(");
