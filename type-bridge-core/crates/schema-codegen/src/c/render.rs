...
use type_bridge_contract::projection::{
    ...
    TYPE_BRIDGE_C_ABI_MINOR, TYPE_BRIDGE_C_CREATE_FIELD_MAX, TYPE_BRIDGE_C_CREATE_MEMBER_MAX,
    TYPE_BRIDGE_C_CREATE_ROLE_MAX, TYPE_BRIDGE_PROJECTED_TOKEN_VERSION, TargetIdentifier,
};
use type_bridge_contract::schema::OwnsFactId;
use type_bridge_contract::value::ValueTypeTag;

use crate::{EmbeddedAuthority, GeneratedPackage, invalid};
...
const C_RUNTIME_HEADER_EXTERNAL_IDENTIFIER_COUNT: usize = 180;
const C_RUNTIME_HEADER_MACRO_IDENTIFIER_COUNT: usize = 205;
...
pub(super) fn render(
    ...
    validate_c_projection_limits(projection, prefix)?;
    ...
    GeneratedPackage::try_new([
        ...
        (header_path, render_header(projection, prefix)?.into_bytes()),
        (
...
fn render_header(projection: &RuntimeProjection, prefix: &str) -> Result<String, Diagnostic> {
    validate_generated_symbols(projection, prefix)?;
    ...
    for name in projected_nominal_names(projection)? {
...
fn projected_nominal_names(projection: &RuntimeProjection) -> Result<BTreeSet<String>, Diagnostic> {
...
fn validate_generated_symbols(
    ...
    let mut symbols = projected_nominal_names(projection)?;
...
fn validate_embedded_resource_symbols(
    ...
    let mut symbols = validate_generated_symbols(projection, prefix)?;
    let mut insert = |symbol: String| {
        if !symbols.insert(symbol) {
            return Err(invalid(
                "c_emitter_generated_symbol_collision",
...
fn validate_c_projection_limits(
    ...
    validate_c_translation_inventory(
        generated_external_identifiers(projection, prefix)?.len(),
...
fn validate_c_translation_inventory(
    ...
    let external_identifier_count = C_RUNTIME_HEADER_EXTERNAL_IDENTIFIER_COUNT
    ...
    let macro_count = C_RUNTIME_HEADER_MACRO_IDENTIFIER_COUNT
...
mod tests {
    ...
    fn runtime_export_ledger() -> Vec<&'static str> {
        let body = RUNTIME_EXPORT_LEDGER
            .split_once("const EXPORTED_SYMBOLS: [&str; ")
    ...
    fn frozen_runtime_inventory_matches_public_header_and_export_ledger() {
        ...
        let header_export_count = RUNTIME_HEADER
            ...
            .count();
        let ledger = runtime_export_ledger();
        ...
        assert_eq!(ledger.len(), C_RUNTIME_HEADER_EXTERNAL_IDENTIFIER_COUNT);
        for name in ledger {
            assert!(
    ...
    fn c_translation_external_identifier_inventory_accepts_exact_minimum_and_rejects_one_more() {
        let exact_generated =
            C_TRANSLATION_UNIT_IDENTIFIER_MAX - C_RUNTIME_HEADER_EXTERNAL_IDENTIFIER_COUNT;
        validate_c_translation_inventory(exact_generated, 0)
            .expect("the exact C external-identifier minimum is accepted");
        let error = validate_c_translation_inventory(exact_generated + 1, 0)
    ...
    fn c_translation_macro_inventory_accepts_exact_minimum_and_rejects_one_more() {
        let exact_generated = C_SIMULTANEOUS_MACRO_MAX
            - C_RUNTIME_HEADER_MACRO_IDENTIFIER_COUNT
            - C_SUPPORTED_IMPLEMENTATION_MACRO_RESERVE;
        validate_c_translation_inventory(0, exact_generated)
            .expect("the exact simultaneous-macro minimum is accepted");
        let error = validate_c_translation_inventory(0, exact_generated + 1)
