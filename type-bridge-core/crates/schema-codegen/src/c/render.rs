...
const C_ORDERED_DESCRIPTOR_ABI_MINOR: u32 = 4;
...
fn render_header(
    ...
    let runtime_header = if ordered {
        "typebridge/type_bridge_abi_1_4.h"
    } else {
    ...
    render_projected_token_declarations(&mut output, projection, prefix);
...
fn render_source(
    ...
    render_projected_token_definitions(&mut output, projection, prefix);
...
fn projected_tokens(
    ...
    for (kind, kind_name) in projected_token_kinds() {
...
fn render_projected_token_definitions(
    ...
    let digest = projection
        ...
        .digest()
        .bytes();
    for (kind, kind_name, ordinal) in projected_tokens(projection) {
        let symbol = projected_token_symbol(prefix, kind_name, ordinal);
        let _ = writeln!(
            output,
