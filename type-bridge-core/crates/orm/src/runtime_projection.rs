...
impl InstalledRuntimeProjection {
    ...
    pub fn try_new(projection: RuntimeProjection) -> Result<Self> {
        ...
        for (id, model) in projection.models() {
            let descriptor = match id.kind() {
                ...
                TypeKind::Relation => TypeDescriptor::Relation(relation_descriptor(&projection, model)?),
                TypeKind::Attribute | TypeKind::Struct => continue,
            };
            descriptors.insert(id.clone(), descriptor);
        }
        Ok(Self { projection: Arc::new(projection), descriptors })
    }
    ...
    pub fn descriptor(&self, id: &TypeId) -> Result<&TypeDescriptor> {
        self.descriptors.get(id).ok_or_else(|| OrmError::DescriptorNotFound(format!(
            "{}:{}", kind_name(id.kind()), id.label().as_str()
        )))
    }
...
fn entity_descriptor(projection: &RuntimeProjection, model: &ModelProjection) -> Result<EntityDescriptor> {
    Ok(EntityDescriptor {
        type_name: model.id().label().as_str().to_owned(),
        is_abstract: model.declaration().is_abstract(),
        parent_type: model.declaration().parent().map(|id| id.label().as_str().to_owned()),
...
fn relation_descriptor(projection: &RuntimeProjection, model: &ModelProjection) -> Result<RelationDescriptor> {
    let mut roles = Vec::new();
    for role in model.query_tokens().roles().values() {
        roles.push(RoleDescriptor {
            role_name: role.role().label().as_str().to_owned(),
            player_type_names: role.accepted_players().iter().map(|id| id.label().as_str().to_owned()).collect(),
...
fn owned_attributes(projection: &RuntimeProjection, model: &ModelProjection) -> Result<Vec<OwnedAttributeDescriptor>> {
    model.query_tokens().fields().values().map(|field| {
        let attribute_id = TypeId::new(TypeKind::Attribute, field.id().attribute().label().as_str())
            .map_err(contract_error)?;
        let attribute = projection.models().get(&attribute_id).ok_or_else(|| descriptor_error(
            model.id(), "projected ownership references an absent attribute model",
        ))?;
        let value_type = attribute.declaration().value_type().ok_or_else(|| descriptor_error(
            &attribute_id, "projected attribute omits its value type",
        ))?;
...
fn provider_cardinality(value: Cardinality) -> Result<(u32, Option<u32>)> {
    ...
    let max = value.max().map(u32::try_from).transpose().map_err(|_| OrmError::DescriptorValidation {
        type_name: "<runtime-projection>".into(),
...
mod tests {
    ...
    fn provider_role_player_arrays_decode_once_in_the_orm() {
        ...
        assert!(decode_role_player_attributes(
            &id,
            &descriptors,
