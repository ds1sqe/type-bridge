...
pub struct CanonicalDate { year: i32, month: u8, day: u8 }
...
impl CanonicalDate {
    ...
    pub fn new(year: i32, month: u8, day: u8) -> Result<Self, Diagnostic> {
        ...
        } else { Ok(Self { year, month, day }) }
    }
    /// Return `(year, month, day)`.
    pub const fn components(self) -> (i32, u8, u8) { (self.year, self.month, self.day) }
}
...
impl FromStr for CanonicalDate {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        let (year_month, day) = value.rsplit_once('-').ok_or_else(|| invalid_temporal("date"))?;
        let (year, month) = year_month.rsplit_once('-').ok_or_else(|| invalid_temporal("date"))?;
        let parsed = Self::new(
        ...
        if parsed.to_string() != value { Err(invalid_temporal("date")) } else { Ok(parsed) }
    }
}
...
impl FromStr for CanonicalTime {
    type Err = Diagnostic;
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        if !value.is_ascii() || value.len() < 8 || &value[2..3] != ":" || &value[5..6] != ":" { return Err(invalid_temporal("datetime")); }
...
impl CanonicalDateTimeTz {
    ...
    pub fn new_fixed(local: CanonicalDateTime, zone: TimeZoneDesignator) -> Result<Self, Diagnostic> {
        match &zone {
            TimeZoneDesignator::Utc => Ok(Self { local, zone, effective_offset_seconds: 0 }),
            TimeZoneDesignator::OffsetSeconds(seconds)
                if seconds.unsigned_abs() <= 86_399 && *seconds != 0 =>
            {
                let effective_offset_seconds = *seconds;
                Ok(Self { local, zone, effective_offset_seconds })
            }
    ...
    pub fn new_named_resolved(
        ...
        if name.is_empty()
            || name.len() > 255
            || !name.bytes().all(|byte| byte.is_ascii_alphanumeric() || matches!(byte, b'/' | b'_' | b'-' | b'+'))
    ...
    pub const fn effective_offset_seconds(&self) -> i32 { self.effective_offset_seconds }
...
impl Serialize for CanonicalDateTimeTz {
        ...
        S: Serializer,
    {
        let zone = match &self.zone {
            ...
            TimeZoneDesignator::OffsetSeconds(seconds) => TimeZoneWire::OffsetSeconds { seconds: *seconds },
        ...
        D: Deserializer<'de>,
    {
        ...
        match wire.zone {
            ...
            TimeZoneWire::OffsetSeconds { seconds } if wire.effective_offset_seconds == seconds => {
                Self::new_fixed(wire.local, TimeZoneDesignator::OffsetSeconds(seconds)).map_err(D::Error::custom)
            }
            TimeZoneWire::Named { name } => {
                Self::new_named_resolved(wire.local, name, wire.effective_offset_seconds).map_err(D::Error::custom)
            }
            TimeZoneWire::Utc | TimeZoneWire::OffsetSeconds { .. } => {
                Err(D::Error::custom("timezone offset does not match its authored fixed zone"))
            }
        }
    }
}
...
impl FromStr for CanonicalDateTimeTz {
    type Err = Diagnostic;
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        if !value.is_ascii() { return Err(invalid_temporal("datetime_tz")); }
        if let Some(local) = value.strip_suffix('Z') { return Self::new_fixed(local.parse()?, TimeZoneDesignator::Utc); }
        if value.ends_with(']') { return Err(invalid_temporal("datetime_tz")); }
        ...
        Self::new_fixed(value[..split].parse()?, TimeZoneDesignator::OffsetSeconds(seconds))
    }
}
...
mod tests {
    ...
    fn dates_cover_the_complete_provider_year_domain() {
        for value in ["-262143-01-01", "-0001-12-31", "0000-02-29", "+262142-12-31"] {
            assert_eq!(value.parse::<CanonicalDate>().unwrap().to_string(), value);
        }
        for value in ["-262144-01-01", "+262143-01-01", "+0001-01-01", "1-01-01"] {
            assert!(value.parse::<CanonicalDate>().is_err(), "expected {value:?} to fail");
        }
    }
    ...
    fn timezone_wire_retains_authored_zone_and_resolved_instant() {
        ...
        assert_eq!(serde_json::from_str::<CanonicalDateTimeTz>(&bytes).unwrap(), value);
        ...
        let later_overlap = CanonicalDateTimeTz::new_named_resolved(local, "Europe/London", 0).unwrap();
        assert!(
            value.semantic_utc_nanoseconds()
                < later_overlap.semantic_utc_nanoseconds()
        );
    }
}
