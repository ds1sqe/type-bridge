...
impl CapabilityId {
    ...
    pub fn new(value: impl Into<String>) -> Result<Self, Diagnostic> {
        ...
        if value.len() <= MAX_CAPABILITY_ID_BYTES && segments.len() >= 2 && segments.iter().all(|s| valid_segment(s)) {
            Ok(Self(value))
        } else {
...
impl CapabilitySet {
    ...
    pub const fn new() -> Self { Self(BTreeSet::new()) }
    /// Insert one capability.
    pub fn insert(&mut self, capability: CapabilityId) -> bool { self.0.insert(capability) }
    /// Return whether one capability is present.
    pub fn contains(&self, capability: &CapabilityId) -> bool { self.0.contains(capability) }
    /// Return the number of capabilities.
    pub fn len(&self) -> usize { self.0.len() }
    /// Return whether the set is empty.
    pub fn is_empty(&self) -> bool { self.0.is_empty() }
    /// Iterate in deterministic lexical order.
    pub fn iter(&self) -> impl ExactSizeIterator<Item = &CapabilityId> { self.0.iter() }
    ...
    pub fn ensure_supported_by(&self, available: &Self) -> Result<(), Diagnostic> {
        ...
        Err(Diagnostic::stable(
            ...
            "missing",
            DiagnosticDetailValue::TextList(missing.iter().map(|id| id.as_str().to_owned()).collect()),
        ))
    }
}

impl FromIterator<CapabilityId> for CapabilitySet {
    fn from_iter<T: IntoIterator<Item = CapabilityId>>(iter: T) -> Self { Self(iter.into_iter().collect()) }
}
...
mod tests {
    ...
    fn id(value: &str) -> CapabilityId { CapabilityId::new(value).unwrap() }
    ...
    fn unknown_open_ids_round_trip_in_deterministic_order() {
        ...
        assert_eq!(serde_json::from_slice::<CapabilitySet>(&bytes).unwrap(), set);
    }
    ...
    fn missing_required_capability_has_a_stable_diagnostic() {
        ...
        let error = required.ensure_supported_by(&CapabilitySet::new()).unwrap_err();
