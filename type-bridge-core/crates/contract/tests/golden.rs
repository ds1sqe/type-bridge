...
fn phase1_foundation_has_byte_exact_golden_shapes() {
    ...
    assert_eq!(
        to_canonical_json(&type_id).unwrap(),
        br#"{"kind":"entity","label":"person"}"#
    );
    ...
    assert_eq!(
        to_canonical_json(&capabilities).unwrap(),
        br#"["query.given-multi-row","schema.annotations"]"#
    );

    let cardinality = Cardinality::new(0, None).unwrap();
    assert_eq!(
        to_canonical_json(&cardinality).unwrap(),
        br#"{"kind":"cardinality","max":"unbounded","min":"0"}"#
    );
