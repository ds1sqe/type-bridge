// ---------------------------------------------------------------------------
// TypeSchema interface — mirrors the Rust `TypeSchema` serde JSON shape.
//
// Field names match the Rust struct field names exactly (serde default).
// Inheritance is already resolved by the Rust core before serialization;
// the TypeScript side only deserializes.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// parseSchema — thin wrapper over the `parseSchemaJson` NAPI binding.
// ---------------------------------------------------------------------------
 ...
 */
export function parseSchema(tql: string, native: SchemaParserNative): TypeSchema {
  return JSON.parse(native.parseSchemaJson(tql)) as TypeSchema;
}
