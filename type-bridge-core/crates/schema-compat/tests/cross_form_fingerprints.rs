...
use type_bridge_schema::{
    SchemaDocumentSet, normalize_documents, semantic_schema_fingerprint,
};
...
fn assert_equal_fingerprints(
    context: &str,
    left: &DeclaredSchema,
    right: &DeclaredSchema,
) {
    assert_eq!(
    ...
    assert_eq!(
        semantic_schema_fingerprint(left, &profile())
            .expect("left semantic fingerprint computes"),
...
fn yaml_typeql_and_legacy_toml_share_both_fingerprint_domains() {
    ...
    let typeql = typeql_to_declared(
        document_id("schema/legacy-overlap.tql"),
        LEGACY_TYPEQL,
    )
    .expect("legacy-overlap TypeQL fixture adapts");
    let toml = toml_to_declared(
        document_id("generated/legacy-overlap.tql"),
        LEGACY_TOML,
    )
