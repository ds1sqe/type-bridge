    ...
    for (function, references) in parsed.function_body_references() {
        ...
        for reference in references.references() {
            let reserved = match reference {
                SchemaReference::Label(label) => {
                    is_typebridge_internal_label(label.as_str())
                }
                SchemaReference::Scoped { scope, name } => {
                    is_typebridge_internal_label(scope.as_str())
                        || is_typebridge_internal_label(name.as_str())
                }
                SchemaReference::Function(id) => {
                    is_typebridge_internal_label(id.label().as_str())
                }
            };
            if reserved {
                return Err(failure(
