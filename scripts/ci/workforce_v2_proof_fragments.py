#!/usr/bin/env python3
"""Validate same-run deterministic proof fragments for workforce-v2 reports."""
...
PROOF_SCHEMA_RELATIVE = "tests/contracts/sdk_conformance/workforce-v2/proof-fragment-schema-v1.json"
ALLOWLIST_RELATIVE = "tests/contracts/sdk_conformance/workforce-v2/proof-fragment-allowlist-v1.json"
JOURNEY_RELATIVE = "tests/contracts/sdk_conformance/workforce-v2/journey-v2.json"

FRAGMENT_FORMAT = "typebridge.workforce-v2-proof-fragment/v1"
ALLOWLIST_FORMAT = "typebridge.workforce-v2-proof-fragment-allowlist/v1"
SEMANTIC_PROFILE = "typedb-3.12.1/v1"
REPORT_BINDINGS = frozenset({"python", "node", "rust", "c"})
PROOF_KINDS = frozenset({"direct_runtime", "remote_runtime", "diagnostic", "lifecycle"})
...
def _inspect_fragment_path(path: Path) -> tuple[Path, stat.stat_result]:
...
def main(argv: list[str] | None = None, *, root: Path = ROOT) -> int:
    ...
    try:
        ...
        print(f"workforce-v2 proof fragments rejected: {error}")
        return 1
    if args.observations_json:
        ...
        print(f"validated {len(observations)} workforce-v2 proof lanes for {args.binding}")
    return 0
