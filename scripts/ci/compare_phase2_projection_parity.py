#!/usr/bin/env python3
"""Compare the four provider-free Phase-2 generated projection reports."""
...
REPORT_FORMAT = "typebridge.phase2-projected-parity-report/v1"
SUMMARY_FORMAT = "typebridge.phase2-projected-parity-summary/v1"
...
class ContractError(ValueError):
    """A stable fail-closed Phase-2 parity rejection."""
...
def _canonical_scalar_values(journey: dict[str, Any]) -> dict[str, Any]:
    ...
    if type(ada) is not dict or type(ada.get("fields")) is not dict:
        raise ContractError("invalid_journey_record", "Ada's workforce record is absent")
    fields = ada["fields"]
    field_by_domain = {
        "boolean": "val_bool",
        "date": "val_date",
...
def load_contract() -> Phase2ParityContract:
    """Load the Phase-2 subset without finalizing the broader V3 authority."""

    schema_identity, _ = _source_identity(SCHEMA_RELATIVE, "Phase-2 schema")
    journey_identity, journey_raw = _source_identity(JOURNEY_RELATIVE, "Phase-2 journey")
    journey = _exact_object(
        _json_value(journey_raw, "Phase-2 journey"),
        {
            "format",
            "fixture_id",
            "version",
    ...
    if (
        ...
        raise ContractError("invalid_journey_authority", "workforce-v3 journey identity drifted")
    ...
    if tuple(sorted(observations)) != tuple(sorted(OBSERVATION_REFS)):
        raise ContractError("invalid_observation_ledger", "Phase-2 observation ledger drifted")
    return Phase2ParityContract(
        authority={
...
def expected_report(binding: str, contract: Phase2ParityContract) -> dict[str, Any]:
    """Return the one exact report shape a binding producer must emit."""

    if binding not in REPORT_BINDINGS:
        raise ContractError("unknown_binding", f"unknown Phase-2 binding {binding!r}")
    return {
...
def _load_report(path: Path, contract: Phase2ParityContract) -> tuple[str, dict[str, Any]]:
    ...
    if report != expected:
        raise ContractError(
            "phase2_observation_mismatch",
            f"{binding} report differs from the frozen Phase-2 observations",
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Phase-2 projected parity rejected: {error}", file=sys.stderr)
        return 1
