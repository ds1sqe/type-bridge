#!/usr/bin/env python3
"""Run the four Sdk V4 producers against one exact isolated TypeDB."""
...
COMPARATOR = ROOT / "scripts/ci/compare_sdk_conformance_v4.py"
REPORTS = tuple(f"{binding}-sdk-v4-report.json" for binding in ("python", "node", "rust", "c"))
TESTS = tuple(f"sdk_v4_{binding}_live" for binding in ("python", "node", "rust", "c"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    temporary = Path(tempfile.mkdtemp(prefix="typebridge-sdk-v4-"))
    compose = [
        "docker",
        "compose",
        "-p",
