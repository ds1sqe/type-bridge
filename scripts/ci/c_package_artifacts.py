#!/usr/bin/env python3
"""Build, validate, and consume frozen ABI 1.6 C package artifacts."""
...
import standalone_cli_artifact as shared

ROOT = Path(__file__).resolve().parents[2]
CORE = ROOT / "type-bridge-core"
CONTRACT_PATH = ROOT / "tests/contracts/c-distribution-v1.json"
ABI_PATH = ROOT / "tests/contracts/c-abi.json"
SCHEMA_PATH = ROOT / "tests/contracts/sdk_conformance/sdk-v3/schema-v3.yaml"
LICENSE_PATH = ROOT / "LICENSE"
NOTICE_PATH = CORE / "crates/c/THIRD_PARTY_NOTICES.md"
TARGET = "x86_64-unknown-linux-gnu"
RUNTIME_NAME = "type-bridge-c-runtime"
RUNTIME_LIBRARY = "libtype_bridge_c.so"
PACKAGE_NAME = "tb_sdkv3"
PACKAGE_VERSION = "1.0.0"
ABI_VERSION = "1.6.0"
PUBLICATION_DISPOSITION = "artifact-only-unpublished-unsupported"
...
class ArtifactError(RuntimeError):
    """A C package artifact is incomplete, stale, or unsafe."""
...
def run(
    ...
    *,
    ...
    except shared.ArtifactError as error:
...
def require_command_failure(
    ...
    if result.returncode == 0:
        raise ArtifactError(f"incompatible artifact was accepted: {' '.join(command)}")
...
def read_regular(path: Path, *, maximum: int = MAX_MEMBER_BYTES) -> bytes:
    try:
        ...
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
            raise ArtifactError(f"linked or non-regular input: {path}")
        if metadata.st_size < 0 or metadata.st_size > maximum:
            raise ArtifactError(f"input exceeds its byte budget: {path}")
        body = path.read_bytes()
    except OSError as error:
        raise ArtifactError(f"cannot read {path}: {error}") from error
    if len(body) != metadata.st_size:
        raise ArtifactError(f"input changed while being read: {path}")
    return body
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ArtifactError(f"JSON authority is not an object: {path}")
    return value
...
def toolchain_identity() -> dict[str, str]:
    toolchain = shared.artifact_toolchain()
    return {
...
def safe_archive_files(
    ...
    if len(body) < 10 or body[:3] != b"\x1f\x8b\x08" or body[3] != 0 or body[4:8] != b"\0" * 4:
        raise ArtifactError("archive gzip metadata is not canonical")
        ...
        with tarfile.open(fileobj=io.BytesIO(body), mode="r:gz") as archive:
            for member in archive:
                ...
                if (
                    ...
                    raise ArtifactError(f"archive metadata drifted: {name}")
                if member.size < 0 or member.size > MAX_MEMBER_BYTES:
                    raise ArtifactError(f"archive member exceeds its byte budget: {name}")
                expanded += member.size
                if expanded > MAX_EXPANDED_BYTES:
                    raise ArtifactError("archive exceeds its expanded byte budget")
                stream = archive.extractfile(member)
                if stream is None:
                    raise ArtifactError(f"unreadable archive member: {name}")
                payload = stream.read(member.size + 1)
                if len(payload) != member.size:
                    raise ArtifactError(f"archive member size drifted: {name}")
                files[PurePosixPath(name)] = payload
    except ArtifactError:
        raise
    except (EOFError, OSError, tarfile.TarError) as error:
        raise ArtifactError(f"cannot read archive: {error}") from error
    if tuple(files) != tuple(members):
        raise ArtifactError(f"archive layout/order drifted: {[str(path) for path in files]}")
    return files
...
def inspect_shared_elf(library: Path) -> dict[str, object]:
    ...
    if len(body) < 20 or body[:4] != b"\x7fELF" or body[4:6] != b"\x02\x01":
        raise ArtifactError("runtime is not little-endian ELF64")
    if int.from_bytes(body[18:20], "little") != 62:
        raise ArtifactError("runtime ELF machine is not x86_64")
    ...
    if not needed:
        raise ArtifactError("runtime has no ELF dependency inventory")
    if rpath:
        raise ArtifactError(f"runtime carries a forbidden RPATH/RUNPATH: {rpath}")
    exports = sorted(
    ...
    required = sorted(
        {
            ...
            for name in re.findall(
                ...
                read_regular(CORE / "crates/c/include/typebridge/type_bridge.h"),
            )
        }
    )
    if exports != required:
        raise ArtifactError("runtime exports do not match the current header")
    return {
        "exports": exports,
...
def embedded_json(source: bytes, label: str) -> dict[str, Any]:
    ...
    if not chunks:
        raise ArtifactError(f"generated source omits embedded {label}")
    body = bytes(
    ...
    try:
        ...
        raise ArtifactError(f"embedded {label} is invalid: {error}") from error
    if not isinstance(value, dict) or canonical_json(value)[:-1] != body:
        raise ArtifactError(f"embedded {label} is not canonical JSON")
    return value
...
def build_runtime(output: Path, build_root: Path, *, commit: str, tree: str) -> Path:
    ...
    environment = shared.artifact_build_environment(target_dir)
    run(
        [
            "cargo",
            f"+{toolchain}",
...
find_dependency(TypeBridge 1.6.0 EXACT CONFIG)
...
def build_generation_workspace(root: Path) -> Path:
    ...
    shutil.copy2(SCHEMA_PATH, workspace / "schema/sdk.yaml")
...
def build_generated(output: Path, build_root: Path, *, commit: str, tree: str) -> Path:
    ...
    if original_source.count(old_include) != 1:
        raise ArtifactError("generated source include authority drifted")
    ...
    if (
        ...
        raise ArtifactError("generated projection fingerprint is malformed")
    files: dict[PurePosixPath, bytes] = {
    ...
    manifest = {
        "artifact-id": f"sha256:{identity}",
        "declared-schema-source": "tests/contracts/sdk_conformance/sdk-v3/schema-v3.yaml",
        ...
        "publication-disposition": PUBLICATION_DISPOSITION,
        "required-runtime-abi": {"exact": "1.6.0"},
...
def build(output: Path) -> dict[str, str]:
    if platform.system() != "Linux" or platform.machine() not in {"x86_64", "AMD64"}:
        raise ArtifactError(f"C artifacts require native {TARGET}")
    contract = load_json(CONTRACT_PATH)
    if contract.get("publication_disposition") != PUBLICATION_DISPOSITION:
        raise ArtifactError("frozen publication disposition drifted")
...
def validate_runtime(archive: Path) -> dict[str, Any]:
    ...
    if manifest.get("dependencies") != observed:
        raise ArtifactError("runtime dependency/export inventory drifted")
    if manifest.get("sha256") != sha256(files[PurePosixPath(f"lib/{RUNTIME_LIBRARY}")]):
        raise ArtifactError("runtime library digest drifted")
    if manifest.get("publication-disposition") != PUBLICATION_DISPOSITION:
        raise ArtifactError("runtime publication disposition widened")
    return manifest
...
def validate_generated(archive: Path) -> dict[str, Any]:
    ...
    manifest = json.loads(manifest_body, object_pairs_hook=unique_object)
    if not isinstance(manifest, dict) or canonical_json(manifest) != manifest_body:
        raise ArtifactError("generated package manifest is not canonical JSON")
    if manifest.get("format") != PACKAGE_FORMAT or manifest.get("name") != PACKAGE_NAME:
        raise ArtifactError("generated package identity drifted")
    if manifest.get("filename") != archive.name:
        raise ArtifactError("generated package filename drifted")
    expected_paths = (*PACKAGE_MEMBERS[:5], PACKAGE_MEMBERS[6])
    expected = [member_record(path, files[path]) for path in expected_paths]
    if manifest.get("members") != expected:
        raise ArtifactError("generated package member manifest drifted")
    if manifest.get("payload-sha256") != sha256(canonical_json(expected)):
        raise ArtifactError("generated package payload digest drifted")
    ...
    if (
        ...
        raise ArtifactError("generated embedded compatibility identity drifted")
    digest = projection.get("digest")
    if archive.name != f"{PACKAGE_NAME}-c-{digest}.tar.gz":
        raise ArtifactError("generated archive name is not projection-bound")
    if files[PACKAGE_MEMBERS[6]] != read_regular(LICENSE_PATH):
        raise ArtifactError("generated license template drifted")
    config = files[PACKAGE_MEMBERS[2]].decode("utf-8")
    pc = files[PACKAGE_MEMBERS[4]].decode("utf-8")
    if "find_dependency(TypeBridge 1.6.0 EXACT CONFIG)" not in config:
        raise ArtifactError("generated CMake runtime ABI fence drifted")
    if "type-bridge = 1.6.0" not in pc:
        raise ArtifactError("generated pkg-config runtime ABI fence drifted")
    if manifest.get("publication-disposition") != PUBLICATION_DISPOSITION:
        raise ArtifactError("generated publication disposition widened")
    return manifest
...
def pkg_config_arguments(arguments: Sequence[str], *, env: Mapping[str, str]) -> list[str]:
    # Some pkg-config implementations shell-escape individual UTF-8 bytes.
    # Split the shell words before decoding each reconstructed argument.
    try:
        result = subprocess.run(
        ...
        raise ArtifactError(f"pkg-config failed: {error}") from error
    return [
        word.encode("utf-8", "surrogateescape").decode("utf-8")
        for word in shlex.split(result.stdout.decode("utf-8", "surrogateescape"))
    ]
...
def smoke(runtime_archive: Path, generated_archive: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="type-bridge-c-consumer-") as temporary:
        ...
        for incompatible in ("1.5.0", "2.0.0"):
            ...
            if sentinel.exists():
                raise ArtifactError("incompatible runtime reached package/provider loading")

        pkg_environment = {
            **environment,
            "PKG_CONFIG_PATH": f"{relocated_generated / 'lib/pkgconfig'}:{relocated_runtime / 'lib/pkgconfig'}",
        }
        pkg_sources = pkg_config_arguments(
            ["--variable=generated_source", PACKAGE_NAME],
            env=pkg_environment,
        )
        if len(pkg_sources) != 1:
            raise ArtifactError("pkg-config generated source path is ambiguous")
        pkg_source = pkg_sources[0]
        flags = pkg_config_arguments(["--cflags", "--libs", PACKAGE_NAME], env=pkg_environment)
        ...
        if relocated_runtime.exists() or relocated_generated.exists():
            raise ArtifactError("artifact uninstall left package files behind")
    return {
        "cmake-c17-cpp17": True,
        "generated-artifact-id": generated_manifest_value["artifact-id"],
        "pkg-config-c17": True,
        "relocation": True,
        "runtime-range-rejection": ["1.5.0", "2.0.0"],
        "runtime-artifact-id": runtime_manifest_value["artifact-id"],
        "uninstall": "clean",
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (ArtifactError, shared.ArtifactError) as error:
        print(f"C package artifact rejected: {error}", file=os.sys.stderr)
        return 1
