#!/usr/bin/env python3
"""Build, validate, and consume the frozen standalone CLI artifact archive."""
...
SDK_MIGRATIONS = (
    "sdkv4/0001_initial",
    "sdkv4/0002_expand-display-name",
    "sdkv4/0003_backfill-display-name",
    "sdkv4/0004_contract-legacy-name",
)
ARCHIVE_MEMBERS = (
...
class ArtifactError(RuntimeError):
    """A standalone CLI artifact is incomplete, stale, or unsafe."""
...
def read_regular(path: Path, *, maximum: int, label: str) -> bytes:
    try:
        ...
        raise ArtifactError(f"cannot inspect {label} {path}: {error}") from error
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise ArtifactError(f"{label} is linked or non-regular: {path}")
    if metadata.st_size < 0 or metadata.st_size > maximum:
        raise ArtifactError(f"{label} exceeds its byte budget: {metadata.st_size}")
    try:
        payload = path.read_bytes()
    except OSError as error:
        raise ArtifactError(f"cannot read {label} {path}: {error}") from error
    if len(payload) != metadata.st_size:
        raise ArtifactError(f"{label} changed while it was read: {path}")
    return payload
...
def run(command: Sequence[str], *, cwd: Path = ROOT, env: Mapping[str, str] | None = None) -> str:
    try:
        ...
        raise ArtifactError(f"command failed: {' '.join(command)}: {detail or error}") from error
...
def load_contract() -> dict[str, Any]:
    try:
        ...
        raise ArtifactError(f"cannot read frozen distribution contract: {error}") from error
    if not isinstance(contract, dict):
        raise ArtifactError("frozen distribution contract is not an object")
    return contract
...
def package_version() -> str:
    try:
        ...
        raise ArtifactError(f"cannot read CLI version authority: {error}") from error
    version = manifest.get("package", {}).get("version")
    if not isinstance(version, str) or not version:
        raise ArtifactError("CLI version authority has no package version")
    return version


def artifact_toolchain() -> str:
    try:
        inventory = tomllib.loads(TOOLCHAIN_INVENTORY.read_text(encoding="utf-8"))
    except (OSError, tomllib.TOMLDecodeError) as error:
        raise ArtifactError(f"cannot read artifact toolchain authority: {error}") from error
    ...
    if not isinstance(toolchain, str) or re.fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+", toolchain) is None:
        raise ArtifactError("artifact toolchain authority is malformed")
    return toolchain


def artifact_build_environment(target_directory: Path) -> dict[str, str]:
    """Return the fixed native-artifact build environment.
...
def git_identity() -> tuple[str, str]:
    ...
    if status:
        raise ArtifactError("artifact source has tracked changes; commit them before building")
    commit = run(["git", "rev-parse", "HEAD"])
    tree = run(["git", "rev-parse", "HEAD^{tree}"])
    if HEX40.fullmatch(commit) is None or HEX40.fullmatch(tree) is None:
        raise ArtifactError("Git returned a malformed source identity")
    return commit, tree
...
def inspect_elf(binary: Path) -> dict[str, Any]:
    ...
    if len(payload) < 20 or payload[:4] != b"\x7fELF":
        raise ArtifactError("CLI executable is not ELF")
    if payload[4:6] != b"\x02\x01" or int.from_bytes(payload[18:20], "little") != 62:
        raise ArtifactError("CLI executable is not little-endian ELF64 x86_64")
    dynamic = run(["readelf", "-dW", str(binary)])
    needed = sorted(set(re.findall(r"Shared library: \[([^\]]+)\]", dynamic)))
    if not needed:
        raise ArtifactError("CLI executable has no recorded ELF dependency closure")
    ...
    if interpreter_match is None:
        raise ArtifactError("CLI executable has no ELF interpreter")
    interpreter = interpreter_match.group(1)
    if interpreter != "/lib64/ld-linux-x86-64.so.2":
        raise ArtifactError(f"unexpected ELF interpreter: {interpreter}")
...
def build_manifest(
    *, binary: bytes, dependencies: dict[str, Any], version: str, commit: str, tree: str
    ...
    toolchain = artifact_toolchain()
    return {
        "abi": "not-applicable",
        "artifact-id": f"sha256:{sha256(identity_input)}",
        "dependencies": dependencies,
        "filename": f"type-bridge-cli-artifact-{TARGET}.tar.gz",
        "format": FORMAT,
        "members": [
            {
                "mode": "0755",
...
def safe_archive_files(archive_path: Path) -> dict[PurePosixPath, bytes]:
    ...
    if len(body) < 10 or body[:3] != b"\x1f\x8b\x08" or body[3] != 0 or body[4:8] != b"\0" * 4:
        raise ArtifactError("CLI archive has non-canonical gzip metadata")
        ...
        with tarfile.open(fileobj=io.BytesIO(body), mode="r:gz") as archive:
            for member in archive:
                ...
                if (
                    ...
                    raise ArtifactError(f"CLI archive contains an unsafe path: {name!r}")
                if name in names or name.casefold() in folded:
                    raise ArtifactError(f"CLI archive contains a duplicate/colliding path: {name}")
                names.add(name)
                folded.add(name.casefold())
                if not member.isfile() or member.issym() or member.islnk():
                    raise ArtifactError(f"CLI archive contains a non-regular member: {name}")
                ...
                if (
                    ...
                    raise ArtifactError(f"CLI archive metadata drifted: {name}")
                if member.size < 0 or member.size > MAX_MEMBER_BYTES:
                    raise ArtifactError(f"CLI archive member exceeds its byte budget: {name}")
                expanded += member.size
                if expanded > MAX_EXPANDED_BYTES:
                    raise ArtifactError("CLI archive exceeds its expanded byte budget")
                source = archive.extractfile(member)
                if source is None:
                    raise ArtifactError(f"CLI archive member is unreadable: {name}")
                payload = source.read(member.size + 1)
                if len(payload) != member.size:
                    raise ArtifactError(f"CLI archive member size drifted: {name}")
                files[path] = payload
    except ArtifactError:
        raise
    except (EOFError, OSError, tarfile.TarError) as error:
        raise ArtifactError(f"cannot read CLI archive: {error}") from error
...
def load_manifest(body: bytes) -> dict[str, Any]:
    try:
        ...
        raise ArtifactError(f"artifact manifest is invalid JSON: {error}") from error
...
def _unique_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise ArtifactError(f"artifact manifest has duplicate key: {key}")
        result[key] = value
    return result
...
def validate(archive_path: Path) -> dict[str, Any]:
    ...
    if set(manifest) != required:
        raise ArtifactError(f"artifact manifest fields drifted: {sorted(set(manifest) ^ required)}")
    if manifest["format"] != FORMAT or manifest["name"] != "type-bridge":
        raise ArtifactError("artifact identity drifted")
    if manifest["target"] != TARGET or manifest["filename"] != archive_path.name:
        raise ArtifactError("artifact target or filename drifted")
    if manifest["publication-disposition"] != PUBLICATION_DISPOSITION:
        raise ArtifactError("artifact publication disposition widened")
    if manifest["semantic-profiles"] != SEMANTIC_PROFILES or manifest["abi"] != "not-applicable":
        raise ArtifactError("artifact semantic profile or ABI classification drifted")
    commit = manifest["source-commit"]
    tree = manifest["source-tree"]
    if not isinstance(commit, str) or HEX40.fullmatch(commit) is None:
        raise ArtifactError("artifact source commit is malformed")
    if not isinstance(tree, str) or HEX40.fullmatch(tree) is None:
        raise ArtifactError("artifact source tree is malformed")
    binary = files[PurePosixPath("bin/type-bridge")]
    if manifest["sha256"] != sha256(binary) or manifest["size"] != len(binary):
        raise ArtifactError("artifact executable identity drifted")
    expected_id = sha256(
        canonical_json(
            {
                "source-commit": commit,
    ...
    if manifest["artifact-id"] != f"sha256:{expected_id}":
        raise ArtifactError("artifact identity drifted")
    expected_members = []
    ...
    if manifest["members"] != expected_members:
        raise ArtifactError("artifact payload manifest drifted")
    if files[PurePosixPath("LICENSE")] != LICENSE_PATH.read_bytes():
        raise ArtifactError("artifact license drifted")
    if files[PurePosixPath("THIRD_PARTY_NOTICES.md")] != NOTICE_PATH.read_bytes():
        raise ArtifactError("artifact third-party notice drifted")
    with tempfile.TemporaryDirectory(prefix="type-bridge-cli-validate-") as temporary:
        ...
        if manifest["dependencies"] != dependencies:
            raise ArtifactError("artifact ELF dependency inventory drifted")
        verify_version(binary_path, version=manifest["version"], commit=commit, tree=tree)
    if manifest["version"] != package_version():
        raise ArtifactError("artifact product version drifted")
    toolchain = manifest["toolchain"]
    if not isinstance(toolchain, dict) or toolchain.get("version") != artifact_toolchain():
        raise ArtifactError("artifact toolchain identity drifted")
    return manifest


def build(output_directory: Path) -> Path:
    if platform.system() != "Linux" or platform.machine() not in {"x86_64", "AMD64"}:
        raise ArtifactError(f"artifact build requires native {TARGET}")
    commit, tree = git_identity()
    toolchain = artifact_toolchain()
    target_directory = output_directory / "build"
    environment = artifact_build_environment(target_directory)
    environment.update(
        {
...
def migration_apply_arguments() -> list[str]:
    ...
    for migration_id in SDK_MIGRATIONS:
    ...
    with tempfile.TemporaryDirectory(prefix="type-bridge-cli-consumer-") as temporary:
        ...
        for binding in ("python", "typescript", "rust", "c"):
            ...
            if not history.is_file() or history.is_symlink():
                raise ArtifactError(f"clean CLI did not generate {binding} history authority")
        if sentinel.exists():
            raise ArtifactError(f"clean CLI discovered a forbidden tool: {sentinel.read_text()}")
        shutil.rmtree(relocated)
        if binary.exists():
            raise ArtifactError("clean CLI uninstall left its executable behind")
    return {
...
def connected_smoke(
    ...
    *,
    ...
    if re.fullmatch(r"[a-z][a-z0-9_]{2,62}", database) is None:
        raise ArtifactError("connected-smoke database name is not a safe isolated identifier")
    manifest = validate(archive_path)
    files = safe_archive_files(archive_path)
    with tempfile.TemporaryDirectory(prefix="type-bridge-cli-live-") as temporary:
        root = Path(temporary)
        installed = root / "installed artifact"
        workspace = root / "sdk history"
        home = root / "empty home"
        shims = root / "forbidden tools"
        sentinel = root / "forbidden-tool-used"
        installed.mkdir()
        ...
        if source.count(expected_environment) != 1:
            raise ArtifactError("sdk connected environment authority drifted")
        ...
        environment.update(
            {
                "TYPEBRIDGE_SDK_V4_USERNAME": username,
                "TYPEBRIDGE_SDK_V4_PASSWORD": password,
            }
        )
        ...
        if binary.exists():
            raise ArtifactError("connected CLI uninstall left its executable behind")
    return {
        "artifact-id": manifest["artifact-id"],
        "database": database,
        "explicit-credentials": True,
        "history-length": len(SDK_MIGRATIONS),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if contract.get("archive_layouts", {}).get("cli") != [
            ...
            raise ArtifactError("frozen CLI archive layout drifted")
        ...
        print(f"standalone CLI artifact rejected: {error}", file=sys.stderr)
        return 1
