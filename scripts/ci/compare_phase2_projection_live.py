...
SUMMARY_FORMAT = "typebridge.phase2-projected-live-summary/v1"
SEMANTIC_PROFILE = "typedb-3.12.1/v1"
REPORT_BINDINGS = ("python", "node", "rust", "c")
REPORT_OUTPUT_ENV = "TYPE_BRIDGE_PHASE2_LIVE_REPORT"
...
class ContractError(ValueError):
    """A stable fail-closed Phase-2 live-contract rejection."""
        ...
        for key, item in value.items():
            ...
            if normalized in FORBIDDEN_REPORT_KEYS:
                raise ContractError(
                    ...
                    f"{path}.{key} is outside the Phase-2 live contract",
                )
...
def _source_identity(label: str) -> tuple[dict[str, str], bytes]:
    ...
    raw = _regular_bytes(ROOT / relative, f"Phase-2 {label}", MAX_AUTHORITY_BYTES)
...
def load_contract() -> Phase2LiveContract:
    ...
    return Phase2LiveContract(authority=authority, observations=observations)


def expected_report(binding: str, contract: Phase2LiveContract) -> dict[str, Any]:
...
def _load_report(path: Path, contract: Phase2LiveContract) -> tuple[str, dict[str, Any]]:
    ...
    if report != expected_report(binding, contract):
        raise ContractError(
            "phase2_live_observation_mismatch",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Phase-2 projected live parity rejected: {error}", file=sys.stderr)
        return 1
