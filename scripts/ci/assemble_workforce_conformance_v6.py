...
def _predecessors(values: list[bytes], binding: str) -> list[dict[str, Any]]:
    ...
    for version, raw in enumerate(values, 1):
        ...
        if not isinstance(report, dict) or raw != conformance.canonical_json_bytes(report):
...
def assemble_report(
    ...
    *,
    ...
    if evidence["producer"] != catalog["candidate_report_producers"][binding]:
    ...
    if (
        phase4.get("format") != "typebridge.c-artifact-phase4-report/v1"
        or phase4.get("publication-disposition") != "candidate-only-unpublished-unsupported"
    ):
        reject("invalid_phase4_report", "Phase 4 aggregate identity is invalid")
    conformance.exact_sha(phase4_sha256, "Phase 4 aggregate")
    artifacts = phase4.get("artifacts")
    if not isinstance(artifacts, dict):
        reject("invalid_phase4_report", "Phase 4 artifact inventory is missing")
    ...
    surface = {
        ...
        "candidate-id": f"sha256:{surface_sha}",
        "sha256": surface_sha,
    }
    if binding == "c":
        ...
        if hashlib.sha256(generated_surface).hexdigest() != surface["sha256"]:
            reject("c_generated_surface_drift", "C surface bytes differ from Phase 4 package")
    ...
    report = {
        ...
        "phase4_report_sha256": phase4_sha256,
        "artifacts": artifacts,
...
def main() -> int:
    ...
    try:
        ...
        phase4 = artifact_journey.load_canonical_report(arguments.phase4)
        ...
        if {manifest["source-commit"] for manifest in candidate_manifests} != {
        ...
        report = assemble_report(
            ...
            phase4=phase4,
            phase4_sha256=hashlib.sha256(arguments.phase4.read_bytes()).hexdigest(),
        ...
        print(f"workforce-v6 assembly rejected [{code}]: {error}", file=sys.stderr)
        return 1
    return 0
