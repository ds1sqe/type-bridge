#!/usr/bin/env python3
"""Shared source and observation validation for SDK proof fragments."""
...
class ProofContract:
    proof_schema_relative: str
    allowlist_relative: str
    journey_relative: str
    fragment_format: str
    allowlist_format: str
    proof_kinds: frozenset[str]
    max_results: int

    def _validate_lane(self, value: Any, context: str) -> ObservationLane:
        if (
        ...
        if not OBSERVATION_REF_RE.fullmatch(observation_ref) or proof_kind not in self.proof_kinds:
            raise ProofFragmentError(
                "invalid_allowlist", f"{context} is not a valid observation lane"
            )
        return value

    def proof_fragment_authority(self, root: Path, binding: str) -> dict[str, ProducerAuthority]:
        ...
        raw = _regular_source_bytes(root, self.allowlist_relative)
        try:
            value = json.loads(
                raw,
                object_pairs_hook=_reject_duplicate_keys,
        ...
        if (
            allowlist["format"] != self.allowlist_format
            or allowlist["semantic_profile"] != SEMANTIC_PROFILE
        ):
        ...
        if not isinstance(producers, list) or not producers:
            raise ProofFragmentError(
                "invalid_allowlist", f"{binding} allowlist is empty or malformed"
            )
        ...
        for producer_index, item in enumerate(producers):
            ...
            if not isinstance(results, list) or not 1 <= len(results) <= self.max_results:
                raise ProofFragmentError(
    ...
    def proof_fragment_lanes(self, root: Path, binding: str) -> set[ObservationLane]:
        """Return the exact committed deterministic lane set for one binding."""

        return {
            lane
            for _, result_authority in self.proof_fragment_authority(root, binding).values()
    ...
    def load_proof_fragments(
        ...
        *,
        ...
        producer_authority = self.proof_fragment_authority(root, expected_binding)
        committed_lanes = {
        ...
        if allowed_lanes is None:
            ...
            allowed = {self._validate_lane(lane, "allowed lane") for lane in allowed_lanes}
        ...
        expected_contract = {
            "proof_schema": source_identity(root, self.proof_schema_relative),
            "allowlist": source_identity(root, self.allowlist_relative),
            "journey": source_identity(root, self.journey_relative),
        }
        ...
        for path in paths:
            ...
            if fragment["format"] != self.fragment_format:
            ...
            if fragment["semantic_profile"] != SEMANTIC_PROFILE:
                raise ProofFragmentError(
                    "profile_mismatch", f"fragment {path} has the wrong profile"
                )
            if fragment["run_nonce"] != run_nonce:
                raise ProofFragmentError(
                    "run_nonce_mismatch", f"fragment {path} is from another run"
                )
            ...
            if contract != expected_contract:
                raise ProofFragmentError(
                    "contract_mismatch", f"fragment {path} binds another contract"
                )
            ...
            if not isinstance(results, list) or not 1 <= len(results) <= self.max_results:
                raise ProofFragmentError(
                    "invalid_results", "fragment results must contain 1..16 items"
                )
            ...
            for index, item in enumerate(results):
                ...
                if not isinstance(observation_ref, str) or not OBSERVATION_REF_RE.fullmatch(
                    ...
                    raise ProofFragmentError(
                        "invalid_result", f"results[{index}] has an invalid ref"
                    )
                if proof_kind not in self.proof_kinds:
                    raise ProofFragmentError(
                        "invalid_result", f"results[{index}] has an invalid kind"
                    )
                if not isinstance(test_id, str) or not TEST_ID_RE.fullmatch(test_id):
                    raise ProofFragmentError(
                ...
                if lane in observations:
                    raise ProofFragmentError(
                        "duplicate_lane", f"proof lane {lane!r} was emitted twice"
                    )
        ...
        if actual != allowed:
            ...
            raise ProofFragmentError(
                "lane_coverage_mismatch",
                f"proof lanes differ: missing={missing!r}, extra={extra!r}",
            )
        if producer_ids != set(producer_authority):
            raise ProofFragmentError(
                "producer_coverage_mismatch",
