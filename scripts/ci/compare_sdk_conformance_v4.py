#!/usr/bin/env python3
"""Validate and compare four real Sdk V4 reports."""
...
CATALOG_RELATIVE = "tests/contracts/sdk_conformance/sdk-v4/catalog-v4.json"
JOURNEY_RELATIVE = "tests/contracts/sdk_conformance/sdk-v4/journey-v4.json"
REPORT_SCHEMA_RELATIVE = "tests/contracts/sdk_conformance/sdk-v4/report-schema-v4.json"
OBSERVATION_SCHEMA_RELATIVE = "tests/contracts/sdk_conformance/sdk-v4/observation-schema-v4.json"
EXPECTED_BINDINGS = ("python", "node", "rust", "c")
EXPECTED_TRANSITIONS = (
    "sdk.runtime.database-administration",
    "sdk.migration.rollback",
    "sdk.migration.backfill",
    "sdk.migration.runtime-facade",
)
...
EXPECTED_CATALOG_FINGERPRINT = "efa3249f25fb2decaa0fef24c99d3a9884942d2e7926b3545955eb21eee7accb"


class ContractError(ValueError):
    """A stable fail-closed Sdk V4 rejection."""
    ...
    if reference == "bound_database_administration":
        ...
        for field in ["apply_order", "rollback_order"]:
            ...
            if (
                ...
                or not values
                or not all(
                    isinstance(value, str) and value.startswith("sdkv4/") for value in values
                )
            ):
                _reject(
                    "invalid_observation_shape",
...
def compare_reports(paths: list[Path], root: Path = ROOT) -> dict[str, Any]:
    ...
    if authority_state != "finalized":
        _reject(
            ...
            "Sdk V4 authority state is outside the closed vocabulary",
        )
    ...
    return {
        "format": "typebridge.sdk-v4-comparison/v1",
...
def main() -> int:
    ...
    try:
        ...
        print(f"sdk-v4 conformance rejected [{error.code}]: {error}", file=sys.stderr)
        return 1
