#!/usr/bin/env python3
"""Build and validate candidate-CLI-generated Workforce V6 language surfaces."""
...
FORMAT = "typebridge.workforce-v6-generated-surface/v1"
...
def manifest_for(
    ...
    return {
        ...
        "cli-candidate-id": cli_manifest["candidate-id"],
        "members": members,
...
def validate_surface(path: Path, binding: str) -> dict[str, Any]:
    ...
    except (UnicodeDecodeError, json.JSONDecodeError, cli.CandidateError) as error:
    ...
    candidate = manifest["cli-candidate-id"]
    if (
        not isinstance(candidate, str)
        or not candidate.startswith("sha256:")
        or len(candidate) != 71
        or any(c not in "0123456789abcdef" for c in candidate[7:])
    ):
        reject("invalid_surface_candidate", "surface CLI candidate ID is invalid")
    expected = [
...
def build(cli_archive: Path, output: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="typebridge-v6-surfaces-") as temporary:
        ...
        if result.returncode != 0:
            reject("surface_generation_failed", f"candidate CLI generation failed: {result.stdout}")
        ...
        for binding, directory in BINDING_DIRECTORIES.items():
            ...
            destination = output / f"type-bridge-{binding}-generated-workforce-v6.tar.gz"
            ...
            summary[binding] = {
                ...
                "sha256": digest,
                "candidate-id": f"sha256:{digest}",
            }
        return {
            "format": "typebridge.workforce-v6-generated-surface-set/v1",
            "source-commit": cli_manifest["source-commit"],
            "cli-candidate-id": cli_manifest["candidate-id"],
            "surfaces": summary,
            "publication-authority": False,
...
def main() -> int:
    ...
    except (SurfaceError, cli.CandidateError) as error:
        code = getattr(error, "code", "surface_failure")
        print(f"Workforce V6 surface rejected [{code}]: {error}", file=sys.stderr)
        return 1
    return 0
