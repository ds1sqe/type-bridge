#!/usr/bin/env python3
"""Run and assemble all four real Sdk V3 artifact reports."""
...
def run_phase_reports(directory: Path, environment: dict[str, str]) -> None:
    run(
        [
            sys.executable,
            "scripts/ci/run_projected_parity.py",
            "--output",
            str(directory / "projected-parity"),
        ],
        env=environment,
    )
    ...
    run(
        [
            sys.executable,
            "scripts/ci/run_projected_live.py",
            "--output",
            str(directory / "projected-live"),
        ],
    ...
    run(
        [
            sys.executable,
            "scripts/ci/run_manager_filter_live.py",
            "--output",
            str(directory / "manager"),
        ],
...
def run_artifacts(directory: Path, environment: dict[str, str]) -> None:
    ...
    run(
        [sys.executable, "scripts/ci/validate_sdk_v3_atomic_generation.py", str(atomic)],
        env=environment,
    )
    run(
        [sys.executable, "scripts/ci/validate_sdk_v3_field_identity.py", str(field_identity)],
        env=environment,
    )
...
def fragment_environment(
    ...
    return environment | {
        "TYPE_BRIDGE_SDK_V3_PROOF_FRAGMENT": str(destination),
        "TYPE_BRIDGE_SDK_V3_PROOF_RUN_NONCE": nonce,
    }
...
def run_proof_fragments(directory: Path, environment: dict[str, str], nonce: str) -> None:
    ...
    data_tests = {
        ...
        "node": (
            "type-bridge-node",
            "runtime_projection::tests::sdk_v3_node_data_plane_fragment",
        ),
    ...
    for binding in BINDINGS:
        ...
        run(
            [
                sys.executable,
                "scripts/ci/sdk_v3_proof_fragments.py",
                "--binding",
                binding,
                "--run-nonce",
                nonce,
...
def assemble(directory: Path, environment: dict[str, str], nonce: str) -> tuple[Path, ...]:
    ...
    for binding in BINDINGS:
        run(
            [
                sys.executable,
                "scripts/ci/compose_sdk_v3_live_observations.py",
                "--binding",
                binding,
                "--projected-parity",
        ...
        run(
            [
                sys.executable,
                "scripts/ci/assemble_sdk_conformance_v3.py",
                "--binding",
                binding,
                "--live-observations",
                str(live / f"{binding}.json"),
    ...
    run(
        [
            sys.executable,
            "scripts/ci/compare_sdk_conformance_v3.py",
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="typebridge-sdk-v3-") as value:
        ...
        print(f"Sdk V3 artifact run failed: {error}", file=sys.stderr)
        return 1
