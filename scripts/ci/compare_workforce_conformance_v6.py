#!/usr/bin/env python3
"""Validate and compare four artifact-bound Workforce V6 candidate reports."""
...
CATALOG = "tests/contracts/sdk_conformance/workforce-v6/catalog-v6.json"
...
def exact_candidate_id(value: Any, label: str) -> None:
    if not isinstance(value, str) or not value.startswith("sha256:"):
        reject("invalid_candidate_identity", f"{label} is not a SHA-256 candidate ID")
...
def validate_observation(index: int, value: Any) -> None:
    ...
    if index == 3:
        item = exact_keys(
            value,
            {"candidate-id", "offline_commands", "relocated", "sdk_independent", "sha256"},
            "standalone CLI observation",
        )
        exact_sha(item["sha256"], "standalone CLI digest")
        exact_candidate_id(item["candidate-id"], "standalone CLI candidate ID")
...
def validate_report(report: dict[str, Any], root: Path = ROOT) -> None:
    ...
    if report["producer"] != catalog["candidate_report_producers"][report["binding"]]:
    ...
    exact_sha(report["phase4_report_sha256"], "Phase 4 report")
    artifacts = exact_keys(
        report["artifacts"], {"cli", "generated-package", "runtime"}, "artifact set"
    )
    for name, artifact in artifacts.items():
        item = exact_keys(artifact, {"candidate-id", "sha256"}, f"{name} artifact")
        exact_sha(item["sha256"], f"{name} artifact")
        exact_candidate_id(item["candidate-id"], f"{name} candidate ID")
    surface = exact_keys(
        report["generated_surface"], {"format", "candidate-id", "sha256"}, "generated surface"
    )
    exact_sha(surface["sha256"], "generated surface")
    exact_candidate_id(surface["candidate-id"], "generated surface candidate ID")
    ...
    if (
        results[3]["observation"]["candidate-id"] != artifacts["cli"]["candidate-id"]
...
def compare_reports(paths: list[Path], root: Path = ROOT) -> dict[str, Any]:
    ...
    return {
        "format": "typebridge.workforce-v6-comparison/v1",
        "authority_state": "candidate" if pending else "finalized",
...
def main() -> int:
    ...
    try:
        ...
        print(f"workforce-v6 conformance rejected [{error.code}]: {error}", file=sys.stderr)
        return 1
