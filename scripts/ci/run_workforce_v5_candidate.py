#!/usr/bin/env python3
"""Run and assemble all four real Workforce V5 candidate reports."""
...
PYTHON_PROVIDER = ROOT / "tests/integration/schema/test_generated_workforce_v5_codec.py"
NODE_PROVIDER = ROOT / "tests/integration/schema/test_generated_node_workforce_v5_codec.py"
C_PROVIDER = ROOT / "tests/integration/schema/test_generated_c_workforce_v5_codec.py"
...
def write_cleanup(path: Path, binding: str) -> None:
    path.write_bytes(
        conformance.canonical_json_bytes(
            {
                "binding": binding,
                "format": "typebridge.workforce-v5-cleanup-evidence/v1",
...
def provider_free(directory: Path, environment: dict[str, str]) -> None:
    python_env = environment | {
        "TYPE_BRIDGE_WORKFORCE_V5_PYTHON_CORPUS": str(directory / "python-corpus.json"),
        "TYPE_BRIDGE_WORKFORCE_V5_PYTHON_OPERATIONAL_EVIDENCE": str(
    ...
    node_env = environment | {
        "TYPE_BRIDGE_WORKFORCE_V5_NODE_CORPUS": str(directory / "node-corpus.json"),
        "TYPE_BRIDGE_WORKFORCE_V5_NODE_OPERATIONAL_EVIDENCE": str(
            directory / "node-operational.json"
        ),
    }
    run(
        ["uv", "run", "pytest", "-q", "-m", "integration", str(NODE_PROVIDER)],
        env=node_env,
    )

    rust_env = environment | {
        "TYPE_BRIDGE_WORKFORCE_V5_RUST_CORPUS": str(directory / "rust-corpus.json"),
        "TYPE_BRIDGE_WORKFORCE_V5_RUST_OPERATIONAL_EVIDENCE": str(
            directory / "rust-operational.json"
        ),
    }
    run(
        [
    ...
    c_env = environment | {
        "TYPE_BRIDGE_WORKFORCE_V5_C_CORPUS": str(directory / "c-corpus.json"),
        "TYPE_BRIDGE_WORKFORCE_V5_C_OPERATIONAL_EVIDENCE": str(directory / "c-operational.json"),
    }
    run(
    ...
    run(
        [
            sys.executable,
            str(ROOT / "scripts/ci/compare_workforce_v5_corpora.py"),
            *(
                item
    ...
    run(
        [
            sys.executable,
            str(ROOT / "scripts/ci/compare_workforce_v5_operational.py"),
            *(
                item
                for binding in BINDINGS
                for item in (
...
def live(directory: Path, environment: dict[str, str]) -> None:
    python_env = environment | {
        "USE_DOCKER": "false",
        "TYPE_BRIDGE_WORKFORCE_V5_PYTHON_EVIDENCE": str(directory / "python-live.json"),
    }
    run(
        [
            "uv",
    ...
    node_env = environment | {
        "TYPE_BRIDGE_WORKFORCE_V5_NODE_EVIDENCE": str(directory / "node-live.json"),
    ...
    rust_env = environment | {
        "TYPE_BRIDGE_WORKFORCE_V5_RUST_EVIDENCE": str(directory / "rust-live.json"),
    ...
    c_env = environment | {
        "TYPE_BRIDGE_WORKFORCE_V5_C_EVIDENCE": str(directory / "c-live.json"),
...
def assemble(directory: Path, output: Path, environment: dict[str, str]) -> None:
    ...
    for binding in BINDINGS:
        ...
        run(
            [
                sys.executable,
                "scripts/ci/compose_workforce_v5_evidence.py",
                "--binding",
                binding,
                "--run-nonce",
                nonce,
        ...
        run(
            [
                sys.executable,
                "scripts/ci/assemble_workforce_conformance_v5.py",
                "--binding",
                binding,
                "--run-nonce",
                nonce,
    ...
    run(
        [
            sys.executable,
            "scripts/ci/compare_workforce_conformance_v5.py",
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="typebridge-workforce-v5-") as value:
        ...
        print(f"Workforce V5 candidate run failed: {error}", file=sys.stderr)
        return 1
