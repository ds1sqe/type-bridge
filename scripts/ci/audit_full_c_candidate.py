#!/usr/bin/env python3
"""Independently audit one final FULL-C candidate without granting release authority."""
...
import compare_workforce_conformance as v1  # noqa: E402
import compare_workforce_conformance_v2 as v2  # noqa: E402
import compare_workforce_conformance_v3 as v3  # noqa: E402
import compare_workforce_conformance_v4 as v4  # noqa: E402
import compare_workforce_conformance_v5 as v5  # noqa: E402
import compare_workforce_conformance_v6 as v6  # noqa: E402
...
DOCUMENTATION_FENCES = {
    "tests/contracts/c-distribution-v1.json": (
        "candidate-only-unpublished-unsupported",
        '"public_supported": []',
    ),
    "docs/development/testing.md": ("not a public C support or distribution",),
    "type-bridge-core/crates/schema-codegen/README.md": ("unpublished, unsupported foundation",),
}
...
def validate_documentation(root: Path = ROOT) -> list[dict[str, Any]]:
    ...
    for relative, fences in DOCUMENTATION_FENCES.items():
        ...
        if any(fence not in body for fence in fences):
            reject("unsupported_documentation_claim", f"candidate-only fence drifted in {relative}")
...
def audit(
    *,
    ...
    phase4 = artifact_journey.load_canonical_report(regular(phase4_path, "Phase 4 report"))
    artifact_journey.validate_phase4_report(
        phase4,
        regular(provider, "provider-free report"),
        regular(live, "live report"),
        regular(cli, "CLI candidate"),
        regular(runtime, "runtime candidate"),
        regular(generated, "generated package candidate"),
    )
    ...
    candidate_manifest = load_unique(
        security_evidence / "candidate-manifest.json", "candidate artifact manifest"
    )
    source_commit = candidate_manifest.get("source-commit")
    if not isinstance(source_commit, str) or len(source_commit) != 40:
        reject("candidate_source_drift", "candidate source commit is invalid")
    if any(
        report.get("source_commit") != source_commit
        for report in [load_unique(path, "V6 report") for path in report_paths(reports_root, 6)]
    ):
        reject("candidate_source_drift", "V6 reports and artifacts bind different sources")
    ...
    result = {
        ...
        "candidate_set_id": candidate_manifest["candidate-set-id"],
        "inputs": {
            ...
            "phase4_sha256": sha256(phase4_path),
            "security_manifest_sha256": sha256(security_evidence / "candidate-manifest.json"),
    ...
    summary = (
        "# FULL-C candidate audit\n\n"
        f"- Result: accepted candidate\n"
        f"- Source commit: `{source_commit}`\n"
        f"- Candidate set: `{candidate_manifest['candidate-set-id']}`\n"
        f"- Capabilities: {len(capabilities)}/44 accepted for C\n"
        f"- Workforce reports: V1–V6 independently validated\n"
