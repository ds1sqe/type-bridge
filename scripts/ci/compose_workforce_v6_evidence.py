#!/usr/bin/env python3
"""Compose measured surface, predecessor, ledger, and Phase 4 facts for V6 assembly."""
...
import assemble_workforce_conformance_v6 as assembler
...
def compose(
    *,
    ...
    if (
        ...
        reject("invalid_phase4_report", "Phase 4 aggregate is not a complete candidate report")
    artifacts = phase4.get("artifacts")
    if not isinstance(artifacts, dict) or set(artifacts) != {"cli", "generated-package", "runtime"}:
        reject("invalid_phase4_report", "Phase 4 artifact set is not exact")
    ...
    if binding == "c":
        if surface_consumer != {
            ...
            reject(
                "surface_consumer_identity_drift", "C surface report is not exact Phase 4 evidence"
            )
    else:
        expected_checks = {
        ...
        if (
            ...
            or surface_consumer.get("cli-candidate-id") != artifacts["cli"]["candidate-id"]
...
def main() -> int:
    ...
    try:
        ...
        print(f"workforce-v6 composition rejected [{code}]: {error}", file=sys.stderr)
        return 1
    return 0
