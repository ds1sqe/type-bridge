#!/usr/bin/env python3
"""Produce and compare four Phase-2 reports on one exact TypeDB 3.12.1 fixture."""
...
COMPARATOR = ROOT / "scripts/ci/compare_phase2_projection_live.py"
PYTHON_PRODUCER = CORE / "crates/schema-codegen/tests/acceptance/phase2_live_check.py"
NODE_PRODUCER = CORE / "crates/schema-codegen/tests/typescript_acceptance/phase2_live_check.mjs"
RUST_PRODUCER = CORE / "crates/schema-codegen/tests/rust_acceptance/phase2_live.rs"
C_HARNESS = CORE / "crates/schema-codegen/tests/c_phase2_live.rs"
C_CONSUMER = CORE / "crates/schema-codegen/tests/c_phase2_live/consumer.c"
C_SETUP = CORE / "crates/schema-codegen/tests/c_phase2_live/setup.rs"

ADDRESS_ENV = "TYPE_BRIDGE_PHASE2_LIVE_ADDRESS"
HTTP_PORT_ENV = "TYPE_BRIDGE_PHASE2_LIVE_HTTP_PORT"
REPORT_ENV = "TYPE_BRIDGE_PHASE2_LIVE_REPORT"
DATABASE_ENV = "TYPE_BRIDGE_PHASE2_LIVE_DATABASE"
PYTHON_PACKAGE_ENV = "TYPE_BRIDGE_PHASE2_PYTHON_PACKAGE_ROOT"
NODE_PACKAGE_ENV = "TYPE_BRIDGE_PHASE2_NODE_PACKAGE_ROOT"
REPOSITORY_ENV = "TYPE_BRIDGE_PHASE2_REPOSITORY_ROOT"
...
if action not in {"assert-absent", "cleanup"} or not names:
    raise SystemExit("invalid Phase-2 database guard invocation")
port = int(raw_port)
failures = []
for name in names:
...
def _load_live_contract() -> ModuleType:
    spec = importlib.util.spec_from_file_location("phase2_projection_live_contract", COMPARATOR)
    if spec is None or spec.loader is None:
        raise RunnerError("the committed Phase-2 live comparator cannot be loaded")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    try:
        spec.loader.exec_module(module)
    except (OSError, RuntimeError, ValueError) as error:
        raise RunnerError(f"the committed Phase-2 live comparator rejected: {error}") from error
    return module
...
def _required_fixture(environment: Mapping[str, str]) -> Fixture:
    ...
    if present:
        raise RunnerError(f"runner-owned Phase-2 live environment must be unset: {present}")
...
def _require_tools() -> None:
    if os.name != "posix":
        raise RunnerError("the Phase-2 live fan-in requires POSIX process-group isolation")
    for executable in ("cargo", "maturin", "npm", "node", "gcc", "clang"):
        if shutil.which(executable) is None:
            raise RunnerError(f"required Phase-2 live tool is unavailable: {executable}")
...
def _prepare(layout: Layout) -> None:
    ...
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode):
        raise RunnerError("the Phase-2 live stage root must be a real directory")
    layout.python.mkdir()
    layout.node.mkdir()
    ...
    if len(set(reports)) != 4 or len(set(databases)) != 4:
        raise RunnerError("Phase-2 live report paths and database names must be unique")
    ...
    return (
        ...
        CommandSpec(
            ...
            (
                ...
                "--test",
                "rust_phase2_live",
                "generated_rust_phase2_live_producer_runs_when_explicitly_configured",
                "--",
                "--exact",
                "--nocapture",
            ),
        ...
        CommandSpec(
            ...
            (
                ...
                "--test",
                "c_phase2_live",
                "generated_c17_phase2_live_subset_round_trips_exact_3_12_1",
                "--",
                "--ignored",
                "--exact",
                "--nocapture",
...
def _run(command: CommandSpec) -> subprocess.CompletedProcess[str]:
    ...
    print(f"Phase-2 live: {command.label}", file=sys.stderr, flush=True)
...
def run(environment: Mapping[str, str] = os.environ) -> str:
    ...
    _require_tools()
    with tempfile.TemporaryDirectory(prefix="typebridge-phase2-live-") as temporary:
        ...
        try:
            ...
            if owns_database_names:
                ...
                raise RunnerError(
                    f"Phase-2 live fan-in was interrupted by {type(error).__name__}{cleanup}"
                ) from error
            raise
        try:
