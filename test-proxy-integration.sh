#!/usr/bin/env bash
# Helper script to run proxy integration tests with Docker or Podman
# Auto-detects container tool (prefers podman, falls back to docker)
# Override with: CONTAINER_TOOL=docker ./test-proxy-integration.sh
...
echo -e "${GREEN}Running proxy integration tests with ${CONTAINER_TOOL:-auto-detect}...${NC}"
